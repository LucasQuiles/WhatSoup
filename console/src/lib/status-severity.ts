...
export function statusSeverity(status: Status): StatusSeverity {
  if (status === 'online') return 'ok';
  if (status === 'degraded' || status === 'unknown') return 'warn';
  return 'crit';
}

export function statusNeedsAttention(status: Status): boolean {
...
export function statusWashClass(status: Status): string {
  const severity = statusSeverity(status);
...
export function statusAlertMessage(status: Status, lastSessionStatus?: string | null): string {
