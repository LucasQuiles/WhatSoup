/**
 * transport-identity (T5 b-07…b-10) — the console-wide home for transport→channel
...
/** Baileys-era fallback for surfaces that MUST render a shape (the fleet glyph
 *  column can't draw "unknown"): a line predating the generic transport block
 *  is WhatsApp when it carries the legacy health.whatsapp block, otherwise the
 *  neutral sms bars — never a wrong-channel silhouette. The legacy key read
 *  lives here with the other one, so this file stays the only allowlisted
 *  reader (hygiene.no-health-whatsapp-key-read). */
export function baileysFallbackChannel(line: LineInstance | undefined): 'wa' | 'sms' {
