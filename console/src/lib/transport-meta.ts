...
import { canonicalizeAppleIdEmail } from '../../../src/lib/appleid.ts'
...
export const TRANSPORT_MAP: Readonly<Record<TransportKind, TransportEntry>> = {
  ...
  twilio: {
    ...
    cardLabel: 'SMS (Twilio)',
    cardDescription: 'Cloud-hosted SMS through Twilio.',
    adminIdLabel: 'SMS Access Phones',
    adminIdPlaceholder: 'Enter phone number',
    adminIdHelper: 'Stored in the compatibility field adminPhones. All entries are pre-approved for ordinary SMS. Alerts are not broadcast: startup uses the first configured phone, and approval alerts may use a recent matching conversation. SMS sender IDs are spoofable and cannot issue admin commands.',
    validateAdminId: (value) => validatePhoneIdentityInput(value),
    normalizeAdminId: (value) => normalizePhoneIdentityInput(value),
  },
  signal: {
  ...
  imessage: {
    ...
    validateAdminId: (value) => validatePhoneIdentityInput(value) || canonicalizeAppleIdEmail(value) !== null,
    normalizeAdminId: (value) => {
      const appleId = canonicalizeAppleIdEmail(value)
      return appleId ?? normalizeWirePhoneIdentity(value)
    },
  },
}
