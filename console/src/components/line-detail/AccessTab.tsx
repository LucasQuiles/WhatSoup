...
export function AccessTab({ access, lineName }: { access: AccessEntry[]; lineName: string }) {
  ...
  const renderItem = (entry: AccessEntry, showActions: 'pending' | 'allowed' | 'blocked') => (
    <div
      key={entry.subjectId}
      className={`flex items-center gap-3 hover:bg-d3 c-hover py-[var(--sp-2h)] px-[var(--sp-4)] c-border-b ${showActions === 'pending' ? 'bg-[var(--s-warn-wash)]' : ''} ${showActions === 'blocked' ? 'opacity-60' : ''}`}
    >
      {/* Avatar */}
      <div
      ...
      {/* Status badge */}
      <span
        className="font-mono font-medium flex-shrink-0 text-[var(--font-size-sm)] py-0 px-[var(--sp-2)] rounded-sm"
        style={{
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-3" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(var(--avatar-lg), 1fr))' }}>
        <div className="c-card overflow-hidden">
          <div
            className="c-toolbar c-border-b c-col-header text-t4"
          >
