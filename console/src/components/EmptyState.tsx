...
const EmptyState: FC<EmptyStateProps> = ({
  ...
  return (
    ...
    >
      {resolvedIcon && (
        <motion.div
          className={`${isError ? 'text-s-crit' : 'text-t5'} mb-4 w-[var(--icon-empty)] h-[var(--icon-empty)]`}
      ...
      <motion.div
        className={`font-sans font-semibold ${isError ? 'text-s-crit' : 'text-t3'} text-[var(--font-size-lg)] mb-[var(--sp-1)]`}
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
