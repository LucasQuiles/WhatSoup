...
const ConfigStep: FC<ConfigStepProps> = ({ data, onChange, errors, onSkip }) => {
  ...
  return (
    <div className="flex flex-col gap-[var(--sp-4)]">
      ...
      {activeTab === 'behavior' && (
        <div className="flex flex-col gap-[var(--sp-4)]">
          ...
          <div>
            ...
            <div
              className="flex items-center justify-center cursor-pointer border-[var(--b2)] rounded-sm p-[var(--sp-4)] bg-d3 mb-[var(--sp-2)]"
              style={{
                borderWidth: 'var(--bw)', borderStyle: 'dashed',
              }}
            >
              <input
                type="file"
            ...
            {(Object.entries(CATEGORY_LABELS) as [string, string][]).map(([cat, catLabel]) => {
              ...
              return (
                <div key={cat} className="mt-[var(--sp-3)]">
                  <div className="text-[var(--font-size-xs)] text-t4 uppercase mb-[var(--sp-1)] tracking-[var(--tracking-wide)]">
                    {catLabel}
                  </div>
                  {plugins.map(plugin => {
                    ...
                    return (
                      <label
                        key={plugin.key}
                        className="flex items-start cursor-pointer gap-[var(--sp-2)] py-[var(--sp-1)]"
                      >
                        <input
                          type="checkbox"
                          checked={isEnabled}
            ...
            <div className="flex">
              {SEARCH_MODES.map((mode) => (
                <button
                  key={mode}
                  type="button"
                  className={`c-btn py-[var(--sp-2)] px-[var(--sp-4)] text-[var(--font-size-data)] cursor-pointer border-[var(--b2)] border-solid ${
                    pineconeSearchMode === mode
                      ? 'bg-[var(--wizard-accent)] text-d0'
                      : 'bg-d3 text-t3'
                  } ${
                    mode === 'Memory'
                      ? 'rounded-l-sm'
      ...
      {onSkip && (
        <div className="flex justify-end mt-[var(--sp-4)] pt-[var(--sp-3)] border-t-[var(--bw)] border-solid border-b1">
          <button
            type="button"
