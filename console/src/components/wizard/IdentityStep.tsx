...
const IdentityStep: FC<IdentityStepProps> = ({ data, onChange, errors, nameLocked }) => {
  ...
  const adminPhonesHelperText = transport === 'twilio'
    ? adminPhones.length === 0
      ? transportMeta.adminIdHelper
      : `${adminPhones.length} notification ${adminPhones.length === 1 ? 'phone' : 'phones'} stored in adminPhones. These phones are pre-approved for ordinary SMS but cannot issue admin commands.`
    : adminPhones.length === 0
      ? transportMeta.adminIdHelper
