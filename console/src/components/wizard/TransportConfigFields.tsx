...
import { slugAgentWorkspaceName } from '../../lib/agent-cwd'
...
const TransportConfigFields: FC<TransportConfigFieldsProps> = ({
  ...
  if (transport === 'twilio') {
    const senderKind = config.messagingServiceSid !== undefined ? 'messaging-service' : 'phone'
    const authTokenService = `whatsoup-twilio-${slugAgentWorkspaceName(value(data.name))}`
    return (
      <section aria-labelledby="twilio-transport-config" className="flex flex-col gap-[var(--sp-3)] rounded-md border border-border-subtle bg-surface-inset p-[var(--sp-4)]">
        ...
        <Field label="Twilio auth-token keyring service" helper="The token stays in the OS keyring; only its service name is saved." error={errors['twilioConfig.authTokenService']}>
          {(id) => <TextInput id={id} value={authTokenService} readOnly aria-readonly="true" error={Boolean(errors['twilioConfig.authTokenService'])} />}
        </Field>
  ...
  if (transport === 'signal') {
    const endpointKind = config.tcpPort !== undefined || config.tcpHost !== undefined ? 'tcp' : 'socket'
    return (
  ...
  const rawBackend = value(config.backend)
  const backend = config.backend === undefined
    ? 'bluebubbles'
    : rawBackend === 'imsg' || rawBackend === 'bluebubbles'
      ? rawBackend
      : ''
  return (
    <section aria-labelledby="imessage-transport-config" className="flex flex-col gap-[var(--sp-3)] rounded-md border border-border-subtle bg-surface-inset p-[var(--sp-4)]">
      ...
      <Field label="iMessage backend" error={errors['imessageConfig.backend']}>
        {(id) => (
          <SelectInput
            id={id}
            value={backend}
            error={Boolean(errors['imessageConfig.backend'])}
      ...
      {backend === 'imsg' ? (
        ...
        <>
          <Field label="BlueBubbles URL" helper="Use HTTPS unless the server is on loopback." error={errors['imessageConfig.bluebubblesUrl']}>
          ...
          <Field label="BlueBubbles password keyring service" helper="Use whatsoup-bluebubbles-&lt;line-name&gt;; the password stays in the OS keyring and is bound to this line." error={errors['imessageConfig.bluebubblesPasswordService']}>
            {(id) => <TextInput id={id} value={value(config.bluebubblesPasswordService)} onChange={(event) => update({ bluebubblesPasswordService: event.target.value })} placeholder="whatsoup-bluebubbles-my-line" error={Boolean(errors['imessageConfig.bluebubblesPasswordService'])} />}
          </Field>
        </>
      ) : null}
    </section>
