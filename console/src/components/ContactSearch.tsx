    ...
    <div className="flex flex-col gap-[var(--sp-2)]">
      <div className="flex items-center gap-[var(--sp-2)]">
        ...
        <input
          ...
          className="c-input flex-1 py-[var(--sp-1)] px-[var(--sp-2)] text-data"
        />
        <button
          type="button"
      ...
      {searched && results.length === 0 && (
        <div className="text-t5 font-mono text-center p-[var(--sp-2)] text-xs">
          No contacts found
        </div>
      )}
        ...
        <div className="flex flex-col gap-[var(--bw)] max-h-[var(--contact-search-max-h)] overflow-y-auto">
          {results.map((c) => (
            <button
              key={c.jid}
              type="button"
              className="flex items-center gap-2 text-left c-hover cursor-pointer rounded-sm py-[var(--sp-1h)] px-[var(--sp-2)] text-data"
              onClick={() => onSelect?.(c)}
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                {c.number && (
                  <div className="font-mono text-t5 text-xs">
                    {c.number}
                  </div>
                )}
              </div>
