    ...
    return (
      <div className="relative" ref={containerRef}>
        <button
          ...
          className="w-full flex items-center justify-between font-sans text-t1 hover:bg-d4 cursor-pointer bg-d3 c-toolbar c-hover text-[var(--font-size-body)] min-h-[var(--toolbar-h)] c-border-b"
        >
        ...
        {open && (
          <div
            className="absolute top-full left-0 right-0 z-10 max-h-64 overflow-auto scrollbar-hide bg-d6 rounded-b-md shadow-[var(--shadow-md)] border-[var(--bw)] border-solid border-[var(--b2)] border-t-0"
          >
            {lines.map(line => (
              <button
                key={line.name}
  ...
  return (
    <div className="relative" ref={containerRef}>
      <button
        type="button"
        onClick={() => setOpen(!open)}
        className="flex items-center gap-2 font-mono cursor-pointer c-hover text-t1 bg-d4 text-[var(--font-size-label)] tracking-[var(--tracking-pill)] py-[var(--sp-0h)] px-[var(--sp-3)] rounded-sm border-[var(--bw)] border-solid border-[var(--b2)]"
      >
        {currentLine && <StatusDot status={currentLine.status} size="sm" />}
      ...
      {open && (
        <div
          className="absolute top-full left-0 mt-1 z-20 max-h-64 overflow-auto min-w-[var(--dropdown-min-w)] bg-d6 rounded-md shadow-[var(--shadow-md)] border-[var(--bw)] border-solid border-[var(--b2)]"
        >
          {lines.map(line => (
            <button
              key={line.name}
