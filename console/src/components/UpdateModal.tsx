  ...
  return (
    ...
    >
      <div
        ...
        className="w-[var(--panel-confirm)] bg-d2 border border-[var(--b2)] rounded-lg shadow-[var(--shadow-lg)]"
        style={{
          maxWidth: '90%',
          borderWidth: 'var(--bw)',
          borderStyle: 'solid',
      ...
      >
        ...
        >
          ...
          <button onClick={handleClose} aria-label="Close" type="button" className="c-btn c-btn-ghost">
            <X size={16} />
          </button>
        </div>
        ...
        <div className="py-[var(--sp-4)] px-[var(--sp-5)]">
          ...
          {phase === 'confirm' && (
            <div className="flex flex-col gap-[var(--sp-4)]">
              ...
              <div className="flex justify-end gap-[var(--sp-2)]">
                <button onClick={handleClose} aria-label="Close" type="button" className="c-btn c-btn-ghost">Cancel</button>
                <button onClick={startUpdate} type="button" className="c-btn c-btn-primary">
                  <Download size={14} />
                  Update
                </button>
              </div>
          ...
          {(phase === 'updating' || phase === 'restarting-fleet') && (
            <div className="flex flex-col gap-[var(--sp-2)]">
              {steps.map(s => (
                <div key={s.step} className="flex items-center gap-[var(--sp-2)] py-[var(--sp-1)]">
                  {stepIcon(s.status)}
                  <span className={`font-mono text-[var(--font-size-data)] ${s.status === 'skip' ? 'text-t5' : 'text-t2'}`}>
                    {STEP_LABELS[s.step] ?? s.step}
                  </span>
                  {s.message && s.status !== 'error' && (
                    <span className="text-t5 font-mono text-[var(--font-size-xs)]">
                      {s.message}
                    </span>
                  )}
                </div>
              ...
              {phase === 'restarting-fleet' && (
                <div className="flex items-center gap-[var(--sp-2)] py-[var(--sp-2)]">
          ...
          {phase === 'error' && error && (
            <div className="flex flex-col gap-[var(--sp-3)]">
              ...
              <div className="flex justify-end">
                <button onClick={handleClose} aria-label="Close" type="button" className="c-btn c-btn-ghost">Close</button>
              </div>
            </div>
          )}
          ...
          {phase === 'restart-instances' && (
            <div className="flex flex-col gap-[var(--sp-3)]">
                ...
                {lines.map(line => {
                  ...
                  return (
                    ...
                    >
                      ...
                      />
                      <span className="font-mono text-t2 text-[var(--font-size-data)] flex-1">
                        {line.name}
                      </span>
              ...
              <div className="flex justify-end gap-[var(--sp-2)] pt-[var(--sp-2)]">
                <button onClick={handleClose} aria-label="Close" type="button" className="c-btn c-btn-ghost">Skip</button>
                <button
          ...
          {/* Phase: done */}
          {phase === 'done' && (
            <div className="flex items-center justify-center gap-[var(--sp-2)] py-[var(--sp-4)]">
