  ...
  if (is24h) {
    ...
    return (
      ...
      >
        ...
        {/* 24h bar chart */}
        <div
          className="flex items-end gap-[var(--bw-accent)] h-[var(--avatar-lg)]"
        >
          {hourly.map((value, h) => (
            <div
              key={h}
        ...
        <div className="flex mt-[var(--bw-accent)]">
          {HOURS.map((h) => (
            <div
              key={h}
              className="flex-1 text-t5 font-mono leading-tight text-[var(--font-size-xs)] text-center"
            >
              {h % 3 === 0 ? formatHour(h) : ''}
            </div>
          ))}
    ...
    >
      ...
      >
        ...
        {DAYS.map((day, di) => (
          <>
            <div
              key={`d-${di}`}
              className="text-t4 font-mono leading-snug text-[var(--font-size-xs)] pr-[var(--sp-1)] text-right"
            >
              {day}
            </div>
            {HOURS.map((h) => {
              ...
              return (
                <div
                  ...
                  className="rounded-sm h-[var(--sep-h)]"
                  style={{
      ...
      >
        ...
        {[0, 0.25, 0.5, 0.75, 1].map((ratio) => (
          <div
            key={ratio}
            className="rounded-sm w-[var(--sp-3)] h-[var(--sp-3)]"
            style={{
