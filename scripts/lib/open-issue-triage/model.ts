...
export type IssueClassification =
  "leaf" | "tracker" | "duplicate" | "stale" | "measurement-only";
...
const uniqueTextListSchema = textListSchema.superRefine((values, context) => {
  if (new Set(values).size !== values.length) {
    context.addIssue({
      code: z.ZodIssueCode.custom,
      message: "list must not contain duplicates",
    });
  }
});

const matchedBySchema = sortedUniqueStringArray(
  z.enum([
    "issue-reference",
    "closing-reference",
    "touched-path",
...
const pullRequestOverlapSchema = z
  .object({
  ...
  .strict()
  .superRefine((overlap, context) => {
    if (
      overlap.url !==
      `https://github.com/LucasQuiles/WhatSoup/pull/${overlap.number}`
    ) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["url"],
    ...
    for (const field of ["head_ref", "base_ref"] as const) {
      if (overlap[field] === "redacted/private-ref") {
        context.addIssue({
          ...
          path: [field],
          message:
            "privacy-withheld pull request refs must be represented as null",
        });
      }
    }
  });

const pullRequestOverlapListSchema = z
  .array(pullRequestOverlapSchema)
  .superRefine((values, context) => {
    if (!isSortedUnique(values, (left, right) => left.number - right.number)) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        message:
          "pull request overlaps must be sorted by number without duplicates",
      });
    }
  });

const partialFindingSchema = z
  .object({
...
const partialFindingListSchema = z
  .array(partialFindingSchema)
  .superRefine((values, context) => {
    if (
      !isSortedUnique(values, (left, right) => compareUtf8(left.key, right.key))
    ) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
...
const issueSchema = z
  .object({
    ...
    lead_verification_obligations: uniqueTextListSchema,
  })
  .strict();
...
const expectedReceiptSnapshotSchema = receiptSnapshotSchema
  .omit({ updated_at: true })
  .strict();

const titleDeltaSchema = z
  .object({
...
const batchStartedWithoutHashSchema = z
  .object({
    ...receiptEnvelopeShape,
    ...
    issue_numbers: issueNumberListSchema.refine(
      (values) => values.length > 0,
      "issue_numbers must be nonempty",
    ),
...
const targetVerifiedWithoutHashSchema = z
  .object({
    ...targetReceiptShape,
...
const targetUnknownWithoutHashSchema = z
  .object({
    ...targetReceiptShape,
...
const batchCompletedWithoutHashSchema = z
  .object({
    ...receiptEnvelopeShape,
...
function isStrictNoOp(receipt: ParsedTargetReceiptWithoutHash): boolean {
  return (
    receipt.before.body_sha256 === receipt.expected_after.body_sha256 &&
    receipt.before.title_sha256 === receipt.expected_after.title_sha256 &&
    JSON.stringify(receipt.before.labels) ===
      JSON.stringify(receipt.expected_after.labels) &&
...
function validateReceiptSemantics(
  ...
  if (receipt.receipt_type === "batch_completed") {
    if (
      receipt.verified_count + receipt.unknown_count !==
      receipt.target_count
    ) {
      addIssue(
        "verified_count plus unknown_count must equal target_count",
        "target_count",
      );
    }
    if (receipt.unknown_count !== 0) {
      addIssue(
        "a completed batch cannot contain unknown targets",
        "unknown_count",
      );
    }
    return;
  ...
  if (receipt.operation_result === "no-op" && !isStrictNoOp(receipt)) {
    addIssue(
      "no-op requires exact before/after snapshots and empty deltas",
      "operation_result",
    );
  }
}

const receiptWithoutHashSchema = receiptWithoutHashUnion.superRefine(
  validateReceiptSemantics,
);

const batchStartedSchema = batchStartedWithoutHashSchema
  .extend({ receipt_sha256: sha256Schema })
  .strict();
const targetVerifiedSchema = targetVerifiedWithoutHashSchema
  .extend({ receipt_sha256: sha256Schema })
  .strict();
const targetUnknownSchema = targetUnknownWithoutHashSchema
  .extend({ receipt_sha256: sha256Schema })
  .strict();
const batchCompletedSchema = batchCompletedWithoutHashSchema
  .extend({ receipt_sha256: sha256Schema })
  .strict();

const mutationReceiptSchema = z
  .discriminatedUnion("receipt_type", [
    batchStartedSchema,
...
export type MutationReceiptWithoutHash = z.infer<
...
export function parseLedger(text: string): MutationReceipt[] {
  ...
  if (/^[\t\r\n ]*$/.test(text)) {
    throw new Error(
      "an empty ledger must use the canonical empty string or one-LF seed",
    );
  }
  ...
  const receipts = lines.map((line, index) => {
    ...
    try {
      ...
      throw new Error(
        `ledger line ${index + 1} violates the receipt schema: ${String(error)}`,
        {
          cause: error,
        },
      );
  ...
  for (const [index, receipt] of receipts.entries()) {
    ...
    if (receipt.sequence !== expectedSequence) {
      throw new Error(
        `ledger line ${index + 1} has a non-consecutive global sequence`,
      );
    }
    if (receipt.previous_receipt_sha256 !== expectedPreviousHash) {
      throw new Error(
        `ledger line ${index + 1} has a broken previous_receipt_sha256 chain`,
      );
    }
  }
  ...
  for (const [index, receipt] of receipts.entries()) {
    ...
    if (receipt.receipt_type === "batch_started") {
      if (active !== null) {
        throw new Error(
          `ledger line ${lineNumber} starts a batch after an unfinished batch`,
        );
      }
      if (
        priorCompletedBatchId !== null &&
        receipt.batch_id === priorCompletedBatchId
      ) {
        throw new Error(
          `ledger line ${lineNumber} reuses the prior completed batch_id`,
        );
      }
      active = {
    ...
    if (active === null) {
      throw new Error(
        `ledger line ${lineNumber} appears outside an active batch`,
      );
    }
    if (receipt.batch_id !== active.batchId) {
      throw new Error(
        `ledger line ${lineNumber} changes batch_id inside an active batch`,
      );
    }
    if (receipt.operation_id !== active.operationId) {
      throw new Error(
        `ledger line ${lineNumber} changes operation_id inside an active batch`,
      );
    }
    if (receipt.pinned_main_revision !== active.pinnedMainRevision) {
      throw new Error(
        `ledger line ${lineNumber} changes pinned_main_revision inside an active batch`,
      );
    }
    if (receipt.planned_at !== active.plannedAt) {
      throw new Error(
        `ledger line ${lineNumber} changes planned_at inside an active batch`,
      );
    }
    ...
    if (active.pendingRecovery !== null) {
      if (receipt.receipt_type !== "target_verified") {
        throw new Error(
          `ledger line ${lineNumber} must exactly recover the pending target_unknown`,
        );
      }
    ...
    if (receipt.receipt_type === "batch_completed") {
      if (active.targetIndex !== active.issueNumbers.length) {
        throw new Error(
          `ledger line ${lineNumber} completes before all declared targets`,
        );
      }
      if (
        receipt.target_count !== active.issueNumbers.length ||
        receipt.verified_count !== active.targetIndex ||
        receipt.unknown_count !== 0
      ) {
        throw new Error(
          `ledger line ${lineNumber} has completion counts inconsistent with the batch`,
        );
      }
      if (active.verifiedResults.includes("planned")) {
        throw new Error(
          `ledger line ${lineNumber} completes a batch containing a planned target`,
        );
      }
...
function validateTargetDelta(
  ...
  if (
    receipt.title_delta.before_sha256 !== receipt.before.title_sha256 ||
    receipt.title_delta.after_sha256 !== receipt.expected_after.title_sha256 ||
    receipt.title_delta.changed !==
      (receipt.title_delta.before_sha256 !== receipt.title_delta.after_sha256)
  ) {
    throw new Error(
      `ledger line ${lineNumber} has an inconsistent title_delta`,
    );
  }

  const beforeLabels = new Set(receipt.before.labels);
  const expectedLabels = new Set(receipt.expected_after.labels);
  const expectedAdds = receipt.expected_after.labels.filter(
    (label) => !beforeLabels.has(label),
  );
  const expectedRemoves = receipt.before.labels.filter(
    (label) => !expectedLabels.has(label),
  );
  if (
    JSON.stringify(receipt.label_delta.add) !== JSON.stringify(expectedAdds) ||
    JSON.stringify(receipt.label_delta.remove) !==
      JSON.stringify(expectedRemoves)
  ) {
    throw new Error(
      `ledger line ${lineNumber} has an inconsistent label_delta`,
    );
  }
}
...
function validateTargetRecovery(
  ...
  const exactRecovery =
    unknown.issue_number === verified.issue_number &&
    unknown.issue_node_id === verified.issue_node_id &&
    JSON.stringify(canonicalize(unknown.before)) ===
      JSON.stringify(canonicalize(verified.before)) &&
  ...
  if (!exactRecovery) {
    throw new Error(
      `ledger line ${lineNumber} does not exactly recover the pending target snapshot and deltas`,
    );
  }
  ...
  if (
    unknown.operation_result === "refused-concurrent-update" &&
    !isStrictNoOp(verified)
  ) {
    throw new Error(
      `ledger line ${lineNumber} must recover refused-concurrent-update as a strict no-op`,
    );
  }
}
...
function compareFindings(
  ...
  const issueComparison =
    (left.issue_number ?? -1) - (right.issue_number ?? -1);
...
function findImplementationCycles(registry: OpenIssueRegistry): number[] {
  const openIssueNumbers = new Set(
    registry.issues.map((issue) => issue.issue_number),
  );
  ...
  for (const issue of registry.issues) {
    edges.set(
      ...
      issue.implementation_after_issue_numbers.filter(
        (number) =>
          number !== issue.issue_number && openIssueNumbers.has(number),
      ),
    );
  }
  ...
  for (const issueNumber of [...openIssueNumbers].sort(
...
export function validateRegistry(
  ...
  if (
    registry.inventory.draft_pull_request_count >
    registry.inventory.open_pull_request_count
  ) {
    issues.push(
      finding(
        null,
  ...
  for (const issue of registry.issues) {
    issueCounts.set(
      issue.issue_number,
      (issueCounts.get(issue.issue_number) ?? 0) + 1,
    );
  }
  ...
  for (const issue of registry.issues) {
    ...
    const recommendedLabelsAreKnown = issue.recommended_labels.every((label) =>
      liveLabelSet.has(label),
    );
    const addIssueFinding = (
      code: string,
      field: string,
      message: string,
    ): void => {
    ...
    if (
      issue.url !==
      `https://github.com/LucasQuiles/WhatSoup/issues/${issue.issue_number}`
    ) {
      issues.push(
        finding(
          issue.issue_number,
    ...
    if (issue.classification === "leaf") {
      if (issue.owner_boundary === null || issue.owner_boundary.length === 0) {
        addIssueFinding(
          "leaf-owner-boundary-required",
          "owner_boundary",
          "leaf issues require one owner boundary",
        );
      }
      if (issue.affected_paths.length === 0) {
        addIssueFinding(
          "leaf-affected-paths-required",
          "affected_paths",
          "leaf issues require affected paths",
        );
      }
    ...
    if (
      ...
      issue.evidence_state !== "contradicted"
    ) {
      addIssueFinding(
    ...
    if (
      issue.classification === "measurement-only" &&
    ...
    if (issue.proposed_cohort_id !== null) {
      ...
      if (
        issue.pull_request_overlaps.some(
          (overlap) =>
            overlap.disposition === "open" && overlap.assessment === "owns",
        )
      ) {
        addIssueFinding(
    ...
    if (
      issue.pull_request_owner_pr_number !== null &&
      issue.pull_request_overlaps.some(
