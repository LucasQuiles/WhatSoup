...
const REASONS = deepFreeze([
  ...
  defineReason({
    ...
    implementationState: 'implemented',
    messageTemplate: {
      summary: 'Legacy Git graft metadata prevents trustworthy outgoing-ref ancestry evaluation.',
      guidance: ['Reconcile the repository graft precondition, then regenerate exact-ref evidence; do not bypass or silently delete repository metadata.'],
    },
  }),
  defineReason({
    ...
    implementationState: 'implemented',
    messageTemplate: {
      summary: 'Repository control metadata could not be resolved safely for outgoing-ref evaluation.',
      guidance: ['Repair the repository control-path precondition, then regenerate exact-ref evidence; do not guess or rewrite control metadata automatically.'],
    },
  }),
