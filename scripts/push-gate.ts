...
export const CURATED_TEST_PATHS = [
  ...
  'tests/eslint-rules/require-env-justification.test.ts',
  ...
  'tests/scripts/fault-taxonomy-source-coverage-guard.test.ts',
  ...
  'tests/scripts/platform-pattern-check.test.ts',
...
export const BRANCH_STEPS: readonly GateStep[] = [
  ...
  guard('guard:observation-contract'),
  ...
  guard('guard:png-estate'),
  guard('guard:fault-taxonomy-coverage'),
  ...
  guard('guard:process-tree-diagnostics'),
...
export const CI_ONLY_GUARDS: ReadonlyArray<{ readonly name: string; readonly reason: string }> = [
  ...
  { name: 'guard:png-estate:staged', reason: 'pre-commit staged-PNG hook (#2219); the whole-tree ratchet half is the gate step guard:png-estate' },
