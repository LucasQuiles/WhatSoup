...
export class OutboundQueue implements IOutboundQueue {
  ...
  private startTyping(): void {
    ...
    this.isTyping = true;
    // setTyping failures are debug-level: the indicator is cosmetic, and during a
    // socket outage the send path itself raises the actionable errors — warn here
    // would repeat every TYPING_REFRESH_MS for the outage's duration.
    this.messenger.setTyping?.(this.deliveryJid, true).catch((err) => log.debug({ err }, 'outbound-queue: setTyping(true) rejected'));
    ...
    this.typingRefreshInterval = setInterval(() => {
      ...
      this.messenger.setTyping?.(this.deliveryJid, true).catch((err) => log.debug({ err }, 'outbound-queue: setTyping(true) refresh rejected'));
    }, TYPING_REFRESH_MS);
  }
  ...
  private stopTyping(notify = true): void {
    ...
    if (notify) {
      this.messenger.setTyping?.(this.deliveryJid, false).catch((err) => log.debug({ err }, 'outbound-queue: setTyping(false) rejected'));
    }
  }
    ...
    this.chain = this.chain
      .then(async () => {
        while (this.sendQueue.length > 0) {
          ...
          if (this.isTyping) {
            this.messenger.setTyping?.(chunk.chatJid, true).catch((err) => log.debug({ err }, 'outbound-queue: setTyping(chunk) rejected'));
          }
        }
        this.sending = false;
      })
