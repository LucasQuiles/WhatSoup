...
export interface SessionManagerOptions {
  ...
  /** Canonical provider config overrides generated by the shared MCP adapter. */
  providerMcpConfigArgs?: readonly string[];
  /** Fail-closed host-local proof check, evaluated immediately before spawn.
   *  Returns a ProviderAdmission record binding the validated provider identity
   *  (resolved binary path + sha256 digests) to prevent TOCTOU swaps between
   *  admission and execution. */
  providerCanaryAdmission?: () => ProviderAdmission | Promise<ProviderAdmission>;
}
...
export function buildChildEnv(
  ...
  if (baseOpts?.providerCredentials === 'omit') return env;
...
export const __provider_switch_for_test = {
  ...
  getProviderArgs(
    ...
    return resolveProviderArgs(provider, systemPrompt, cwd, resumeSessionId, model, pluginDirs, providerConfig, providerMcpConfigArgs);
  },
...
export class SessionManager {
  ...
  private readonly providerMcpConfigArgs: readonly string[];
  private providerAdmission: ProviderAdmission | null = null;
  private readonly providerCanaryAdmission: (() => ProviderAdmission | Promise<ProviderAdmission>) | undefined;
  ...
  constructor(opts: SessionManagerOptions) {
    ...
    this.providerMcpConfigArgs = opts.providerMcpConfigArgs ?? [];
    this.providerCanaryAdmission = opts.providerCanaryAdmission;
