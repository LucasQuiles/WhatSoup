/**
 * Service-lifecycle handlers for the fleet ops API.
 *
 * Extracted from `src/fleet/routes/ops.ts` (#2239, slice 3/5). Owns the
 * handlers that change with the service-manager contract: restart, stop,
 * delete (stop + disable + state removal, idempotent on an already-absent
 * service) and create (validate, write config and workspace files, enable,
 * roll back on failure).
 *
 * Re-exported from `ops.ts` as a migration shim so existing callers and tests
 * are unchanged. Validation helpers shared with PATCH come from ops-config.ts.
 */
...
import { systemdUnitName } from '../platform.ts';
...
function cleanupPartial(name: string, extras?: ExtraRecord[]): void {
  ...
  for (const d of dirs) {
    try { fs.rmSync(d, { recursive: true, force: true }); } catch { /* intentional: best-effort removal of a partial instance; cleanup must never throw */ }
  }
  if (extras) {
    for (const e of extras) {
      ...
      } catch { /* intentional: swallow — cleanup must never throw */ }
    }
  }
}
