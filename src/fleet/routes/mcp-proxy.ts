 ...
 *
 * Most handlers are generated by `mcpGet` / `mcpWithBody` factories to
...
/** Factory for GET handlers — no request body, args derived from path params. */
function mcpGet<P extends { name: string }>(
  toolName: string,
...
export const handleGetScheduledById = mcpGet<{ name: string; id: string }>(
  'get_scheduled',
...
export const handleCancelScheduledById = mcpGet<{ name: string; id: string }>(
  'cancel_scheduled',
  p => ({ id: parseInt(p.id, 10) }),
);
...
export const handleGetGroups = mcpGet('list_groups', undefined, { timeout: 15_000 });

// GET /api/lines/:name/groups/:jid — get group metadata.
export const handleGetGroupDetail = mcpGet<{ name: string; jid: string }>(
  'get_group_metadata',
...
export const handleLeaveGroup = mcpGet<{ name: string; jid: string }>(
  'group_leave',
...
export const handleGetGroupInvite = mcpGet<{ name: string; jid: string }>(
  'get_group_invite_link',
  p => ({ jid: decodeURIComponent(p.jid) }),
);

// POST /api/lines/:name/groups/:jid/invite/revoke — revoke group invite link.
export const handleRevokeGroupInvite = mcpGet<{ name: string; jid: string }>(
  'group_revoke_invite',
  p => ({ jid: decodeURIComponent(p.jid) }),
);
...
export const handleGetGroupRequests = mcpGet<{ name: string; jid: string }>(
