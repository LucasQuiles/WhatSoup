/**
 * Message handlers for the fleet ops API.
 *
 * Extracted from `src/fleet/routes/ops.ts` (#2239, slice 4/5). Owns the
 * handlers that change with the message model: send (chatJid xor to, JID
 * normalization, MCP-socket then health-port routing), access update,
 * mark-read, and save contact.
 *
...
export async function handleSend(
  ...
  if (instance.type === 'passive' && instance.socketPath) {
    ...
    } catch { /* intentional: fall through to HTTP — a failed MCP socket send retries via the health-port route below */ }
  }
...
export async function handleSaveContact(
  ...
  if (instance.socketPath && fs.existsSync(instance.socketPath)) {
    try {
      ...
      respondMcp(res, result, 200);
      return;
    } catch { /* intentional: fall through to HTTP proxy — the no-MCP 503 below reports the failure */ }
  }
