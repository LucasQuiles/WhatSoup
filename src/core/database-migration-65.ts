// Migration 65 — append-only continuity-gap closures.
//
...
//
// Rollback: once this migration is recorded, a binary whose ceiling is 64
...
export function runMigration65(db: DatabaseSync): void {
  db.exec('SAVEPOINT migration_65');
  try {
    db.exec(CREATE_TABLE);
    db.exec(CREATE_TRIGGERS);
    db.exec('RELEASE migration_65');
  } catch (err) {
    db.exec('ROLLBACK TO migration_65');
    db.exec('RELEASE migration_65');
    throw err;
  }
}
