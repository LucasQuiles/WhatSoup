 ...
 *    sqlite_master, SAVEPOINT, deferred FKs, double-copy through a _v60
 ...
 */
export function runMigration60(db: DatabaseSync): void {
  db.exec(`
  ...
  if (invalid.length > 0) {
    throw new Error(
      'migration 60 abort: capability_obligations.creation_reason holds value(s)'
  ...
  if (!table.sql.includes(oldCheck)) {
    throw new Error(
      'migration 60 abort: capability_obligations DDL does not contain the expected migration-58 creation_reason CHECK — refusing a blind rebuild',
    );
  }
  ...
  const tempDdl = rebuiltDdl.replace(
    ...
    'CREATE TABLE capability_obligations_v60',
  );
  const columns = (
  ...
  db.exec('SAVEPOINT migration_60');
  try {
    for (const object of [...recreateOrder].reverse()) {
      db.exec(`DROP ${object.type === 'view' ? 'VIEW' : 'TRIGGER'} IF EXISTS ${object.name}`);
    }
    db.exec(tempDdl);
    db.exec(`INSERT INTO capability_obligations_v60 (${columnList}) SELECT ${selectList} FROM capability_obligations`);
    db.exec('DROP TABLE capability_obligations');
    db.exec(rebuiltDdl);
    db.exec(`INSERT INTO capability_obligations (${columnList}) SELECT ${columnList} FROM capability_obligations_v60`);
    db.exec('DROP TABLE capability_obligations_v60');
    ...
    db.exec('RELEASE migration_60');
  } catch (error) {
    db.exec('ROLLBACK TO migration_60');
    db.exec('RELEASE migration_60');
    throw error;
  }
}
