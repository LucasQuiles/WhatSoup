/**
 * User-facing provider error formatter.
 *
 * Converts raw LLM provider error strings into concise, user-readable copy.
 * Handles the common cases: JSON error payloads, leading HTTP status codes,
 * Cloudflare/CDN HTML error pages, generic provider internal errors, and
 * malformed streaming fragments. Returns a clean fallback for unparseable
 * input.
 *
 * This is the single place that raw provider error text is translated into
 * user-visible copy. Every error path in the chat and agent runtimes should
 * route through here before sending a message to the user.
 */

/** UTF-16-safe truncation to avoid splitting surrogate pairs. */
function truncateUtf16Safe(text: string, maxChars: number): string {
  if (text.length <= maxChars) return text;
  const codeUnits = text.slice(0, maxChars);
  // Back up past a trailing high surrogate (0xD800–0xDBFF) so we don't split a pair.
  const last = codeUnits.charCodeAt(codeUnits.length - 1);
  if (last >= 0xd800 && last <= 0xdbff) {
    return codeUnits.slice(0, -1);
  }
  return codeUnits;
}
...
const HTTP_STATUS_PREFIX_RE = /^(?:http\s*)?(\d{3})(?::\s*|\s+)(.+)$/is;
...
function extractLeadingHttpStatus(raw: string): { code: number; rest: string } | null {
  const match = raw.match(/^(?:http\s*)?(\d{3})(?::\s*|\s+)([\s\S]+)?$/i);
...
function isCloudflareOrHtmlErrorPage(raw: string): boolean {
  ...
  return (
    ...
    HTML_CLOSE_RE.test(status.rest)
  );
}
...
/**
 * Format a raw provider error string into concise, user-readable copy.
 *
 * Handles:
 * - Malformed streaming fragments → clean message
 * - Generic provider internal errors (OpenAI-style) → clean message
 * - Cloudflare/CDN HTML error pages → "temporarily unavailable" message
 * - HTTP status prefixes ("429 Too Many Requests: ...") → "HTTP 429: ..."
 * - JSON error payloads → structured "HTTP 429 rate_limit_exceeded: ..."
 * - Unparseable input → truncated raw text (max 600 chars)
 *
 * @param raw - The raw error string from the provider SDK or caught exception.
 * @returns User-facing error text, never empty.
 */
export function formatProviderErrorForUser(raw?: string): string {
  const trimmed = (raw ?? '').trim();
  if (!trimmed) return UNKNOWN_ERROR_MESSAGE;
  ...
  if (isHtmlChallenge) {
    return (
      ...
      'This usually means a CDN or gateway blocked the request. ' +
...
/**
 * Convenience: accept an unknown thrown value and format it for user display.
 * Uses `errorMessage()` for the initial string conversion, then applies the
 * provider error formatter.
 */
export function formatThrownErrorForUser(err: unknown): string {
  if (err == null) return formatProviderErrorForUser(undefined);
  const raw = err instanceof Error ? err.message : String(err);
  return formatProviderErrorForUser(raw);
}
