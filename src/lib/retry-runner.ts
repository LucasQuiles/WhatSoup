...
export interface RetryOptions {
  ...
  /** Observation hook fired before each retry sleep. */
  onRetry?: (info: {
    attempt: number;
    delayMs: number;
    error: unknown;
...
export async function retryAsync<T>(
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      // never undercuts the server's stated minimum. Otherwise symmetric jitter
      // spreads load across concurrent callers.
      const delayMs = retryAfterMs !== null
        ? jitteredDelayPositive(retryAfterMs, 0, maxMs, jitterFraction)
