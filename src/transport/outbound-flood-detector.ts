...
export class OutboundFloodDetector {
  ...
  private prune(key: string, arr: number[], nowMs: number): void {
    ...
    if (arr.length === 0) this.alerted.delete(key); // drained — re-arm
  }
}

/** Minimal structural view of the Baileys socket the counter wraps. */
export interface SendMessageSeam {
  sendMessage(jid: string, content: unknown, options?: unknown): unknown;
}

export interface InstallOutboundFloodCounterOptions {
  /** Clock source (default `Date.now`). Injected for deterministic tests. */
  now?: () => number;
  /** Called once per flood, on the rising edge only (de-duped by the detector). */
  onFlood?: (result: OutboundFloodRecordResult) => void;
}

/**
 * Wrap `sock.sendMessage` so EVERY outbound send is counted before delegating.
 * All tiers — text, media, poll, raw — funnel through `sock.sendMessage`
 * (connection.ts sendMessage/sendRaw/sendMedia/sendPollMessage), so this single
 * seam sees them all; the fleet-feed log parser sees only the text line, which
 * is why this in-bot counter is authoritative.
 *
 * Read-only: it records then delegates, and any detector failure is swallowed —
 * detection must NEVER break a real send. Composable with PR-F's governor
 * wrapper at the same seam (connection.ts:681): both wrap-and-delegate, so
 * order is immaterial. If PR-F lands first, its wrapper can call `record()`
 * inline instead of installing a second wrapper. Returns a restore function.
 */
export function installOutboundFloodCounter(
  sock: SendMessageSeam,
  detector: OutboundFloodDetector,
  opts: InstallOutboundFloodCounterOptions = {},
): () => void {
  const original = sock.sendMessage.bind(sock);
  const now = opts.now ?? (() => Date.now());
  sock.sendMessage = function outboundFloodCounted(jid: string, content: unknown, options?: unknown) {
    try {
      const result = detector.record(jid, now());
      if (result.tripped) opts.onFlood?.(result);
    } catch {
      // Read-only counter: detection must never break a real send.
    }
