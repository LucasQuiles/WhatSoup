...
interface BbMessage {
  ...
  /**
   * BlueBubbles reaction kind. Server versions and response paths expose
   * either the iMessage chat.db numeric code or a named string. Numeric codes
 ...
 *
 * Numeric codes 2000-2005 are tapback adds (love/like/dislike/laugh/
 * emphasize/question in that order); 3000-3005 are the removal counterparts.
 * Named strings use the outbound `/message/react` shape.
 */
function reactionTypeToEmoji(raw: number | string | null | undefined): { emoji: string; remove: boolean } | null {
  ...
  // String responses share the outbound reaction-name vocabulary.
  const s = raw.trim();
  if (s === '') return null;
...
/** Remove iMessage's text-part qualifier (`p:<index>/`) from a target GUID. */
function normalizeAssociatedMessageGuid(raw: string): string {
  return /^p:\d+\//.test(raw) ? raw.replace(/^p:\d+\//, '') : raw;
}
...
function normalizeMessage(msg: BbMessage): InboundImessage | null {
  ...
  if (typeof msg.associatedMessageGuid === 'string' && msg.associatedMessageGuid !== '') {
    ...
    const reactionTargetGuid = normalizeAssociatedMessageGuid(msg.associatedMessageGuid);
    if (parsed !== null && reactionTargetGuid !== '') {
      return {
        guid: msg.guid,
        from,
