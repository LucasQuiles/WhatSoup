...
SECRET_EVIDENCE = "\n".join(
    [
        ...
        "FAIL provider_probe ana-bot: provider=claude-cli target=primary command=/Users/testuser/bin/claude failure_class=provider_auth_required rc=1 output=sk-secretshouldnotleak provider_auth_context=headless_login_keychain_blocked provider_probe_signal=headless_auth_probe_blocked status=advisory_inconclusive",
        "provider_probe ew-bot: provider=opencode-cli target=fallback command=/Users/testuser/bin/opencode status=ok rc=0",
        "FAIL runtime_manifest deploy/scripts/bot-errors-health-check.py: missing_marker=sk-secretshouldnotleak path=/Users/testuser/private/deploy/scripts/bot-errors-health-check.py",
...
def event_fixture() -> dict:
    return {
        ...
        "instance": "ana-bot",
        "source": "daily-health",
        "summary": "BOT ERRORS provider failure for ana-bot sk-secretshouldnotleak",
        "evidence": SECRET_EVIDENCE,
        "process": {"pid": 123, "cwd": "/Users/testuser/private", "argv": ["/Users/testuser/bin/python", "--token", "sk-secretshouldnotleak"]},
