...
if [ ! -f "$HOME/.config/whatsoup/bot-errors.env" ]; then
  ...
  bot_errors_socket="${BOT_ERRORS_SOCKET_PATH:-$HOME/.local/state/whatsoup/instances/$bot_errors_instance/whatsoup.sock}"
  ...
  bot_errors_health_profile="${BOT_ERRORS_HEALTH_PROFILE:-$REPO_ROOT/deploy/health-profiles/$bot_errors_host.json}"
  umask 077
  {
