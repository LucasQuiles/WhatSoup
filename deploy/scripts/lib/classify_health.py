...
def classify(d: object) -> str:
    ...
    if not isinstance(status, str) or status not in {"healthy", "degraded", "unhealthy"}:
        return "fields"
    # A malformed or self-contradictory recovery_debt contract takes the
    # caller's non-mutating "fields" path, like any other unreadable field.
    if recovery_debt_issue(d) is not None:
        return "fields"
    if not isinstance(tc, dict) or "model_usable" not in tc:
        return "fields"
