...
AUTOCLOSE_PROTECTED_SOURCES = {
    ...
    "instance_logged_out",
    # iMessage transports: a dead BlueBubbles Server or an imsg daemon that
    # lost its Messages-framework link means the monitoring path itself is
    # down — silence is not proof of repair.
    "bluebubbles_connection_lost",
    "imessage_device_delinked",
}
...
_TRANSIENT_TRANSPORT_SIGNATURES = (
    ...
    "stream errored out",
    # iMessage transports: BlueBubbles Server unreachable and imsg daemon
    # socket/timeout failures. These ride the same transient retry budget as
    # the WA blips — a momentarily dead backend must not burn the permanent
    # dead-letter cap and strand deliverable alerts.
    "bluebubbles http error",
    "bluebubbles connection lost",
    "imsg socket error",
    "imsg rpc timeout",
)
