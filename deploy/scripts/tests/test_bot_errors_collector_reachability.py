...
class TestPreflightTcpProbeConfirmsOffline:
    """BLOCKER 3 — transient Tailscale control-plane flip false-positive storm:

    Tailscale's ``Online`` field reflects the coordination server's view. During a
    brief control-plane / DERP reconnect the local node can flip every peer to
    ``Online=False`` simultaneously even though direct peer-to-peer paths still
    work (proven in prod 2026-06-16: a direct SSH to a flipped peer succeeded
    while status showed offline). The old ``preflight_remote_unreachable``
    trusted ``Online is False``
    absolutely, skipped SSH, and paged a CRITICAL storm across every reachable
    host. Fix: when a peer reports offline, confirm with a fast TCP probe to the
    SSH port across its Tailscale IPs before declaring it unreachable.
    """

    def _summary(self, ips: list[str]) -> dict[str, Any]:
        return {
            "status": "found",
            "online": False,
            "hostName": "transithost",
            "tailscaleIPs": ips,
        }

    def test_tcp_probe_function_exists(self, tmp_state):
        state_dir, outbox_dir = tmp_state
        mod = _load_mod_with_dirs(state_dir, outbox_dir)
        assert hasattr(mod, "tcp_probe_reachable") and callable(mod.tcp_probe_reachable)

    def test_offline_peer_with_reachable_tcp_probe_is_not_skipped(self, tmp_state):
        """Offline-in-control-plane but TCP-reachable → preflight must allow SSH."""
        state_dir, outbox_dir = tmp_state
        mod = _load_mod_with_dirs(state_dir, outbox_dir)
        summary = self._summary(["100.0.0.1"])
        with patch.object(mod, "tailscale_peer_summary", return_value=summary), \
             patch.object(mod, "tcp_probe_reachable", return_value=True) as probe:
            result = mod.preflight_remote_unreachable("transithost")
        assert result is None, "TCP-reachable offline peer must NOT be skipped"
        probe.assert_called_once()

    def test_offline_peer_with_failed_tcp_probe_is_skipped(self, tmp_state):
        """Offline AND TCP-unreachable → preflight must still skip (genuine outage)."""
        state_dir, outbox_dir = tmp_state
        mod = _load_mod_with_dirs(state_dir, outbox_dir)
        summary = self._summary(["100.0.0.1"])
        with patch.object(mod, "tailscale_peer_summary", return_value=summary), \
             patch.object(mod, "tcp_probe_reachable", return_value=False):
            result = mod.preflight_remote_unreachable("transithost")
        assert result is summary, "Offline + TCP-unreachable peer must be skipped"

    def test_online_peer_never_probes(self, tmp_state):
        """Online peers take the existing fast path — no probe call."""
        state_dir, outbox_dir = tmp_state
        mod = _load_mod_with_dirs(state_dir, outbox_dir)
        summary = {"status": "found", "online": True, "tailscaleIPs": ["100.0.0.1"]}
        with patch.object(mod, "tailscale_peer_summary", return_value=summary), \
             patch.object(mod, "tcp_probe_reachable", return_value=True) as probe:
            result = mod.preflight_remote_unreachable("onlinehost")
        assert result is None
        probe.assert_not_called()

    def test_tcp_probe_connects_to_open_port(self, tmp_state):
        """Real probe: a listening socket on 127.0.0.1 is detected as reachable."""
        state_dir, outbox_dir = tmp_state
        mod = _load_mod_with_dirs(state_dir, outbox_dir)
        ...
        try:
            with patch.dict(os.environ, {"BOT_ERRORS_REACHABILITY_PROBE_PORT": str(port)}):
                assert mod.tcp_probe_reachable({"tailscaleIPs": ["127.0.0.1"]}) is True
        finally:
            srv.close()

    def test_tcp_probe_fails_on_closed_port(self, tmp_state):
        """Real probe: an unbound port on 127.0.0.1 is not reachable."""
        state_dir, outbox_dir = tmp_state
        mod = _load_mod_with_dirs(state_dir, outbox_dir)
        # Reserve then immediately close a port so it is almost certainly unbound.
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        s.bind(("127.0.0.1", 0))
        port = s.getsockname()[1]
        s.close()
        with patch.dict(os.environ, {
            "BOT_ERRORS_REACHABILITY_PROBE_PORT": str(port),
            "BOT_ERRORS_REACHABILITY_PROBE_TIMEOUT_SECONDS": "0.3",
        }):
            assert mod.tcp_probe_reachable({"tailscaleIPs": ["127.0.0.1"]}) is False

    def test_tcp_probe_tries_multiple_ips(self, tmp_state):
        """Probe falls through a dead IP to a live one."""
        state_dir, outbox_dir = tmp_state
        mod = _load_mod_with_dirs(state_dir, outbox_dir)
        srv = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        srv.bind(("127.0.0.1", 0))
        ...
        try:
            with patch.dict(os.environ, {
                ...
                # First IP is TEST-NET-1 (RFC 5737) — refuses/times out fast;
                # second IP is the live listener.
                assert mod.tcp_probe_reachable(
                    {"tailscaleIPs": ["192.0.2.1", "127.0.0.1"]}
                ) is True
        finally:
            srv.close()

    def test_tcp_probe_disabled_when_port_zero(self, tmp_state):
        """Kill-switch: probe port 0 disables probing (offline peers skip as before)."""
        state_dir, outbox_dir = tmp_state
        mod = _load_mod_with_dirs(state_dir, outbox_dir)
        with patch.dict(os.environ, {"BOT_ERRORS_REACHABILITY_PROBE_PORT": "0"}):
            assert mod.tcp_probe_reachable({"tailscaleIPs": ["127.0.0.1"]}) is False

    def test_tcp_probe_handles_missing_or_bad_ips(self, tmp_state):
        """No IPs / non-list / junk entries → False, never raises."""
        state_dir, outbox_dir = tmp_state
        mod = _load_mod_with_dirs(state_dir, outbox_dir)
        assert mod.tcp_probe_reachable({}) is False
        assert mod.tcp_probe_reachable({"tailscaleIPs": None}) is False
        assert mod.tcp_probe_reachable({"tailscaleIPs": ["", 123, None]}) is False

    def test_probe_port_invalid_env_falls_back_to_22(self, tmp_state):
        """Malformed BOT_ERRORS_REACHABILITY_PROBE_PORT → default 22, no raise."""
        state_dir, outbox_dir = tmp_state
        mod = _load_mod_with_dirs(state_dir, outbox_dir)
        with patch.dict(os.environ, {"BOT_ERRORS_REACHABILITY_PROBE_PORT": "notanint"}):
            assert mod.reachability_probe_port() == 22

    def test_probe_timeout_invalid_env_falls_back_to_default(self, tmp_state):
        """Malformed BOT_ERRORS_REACHABILITY_PROBE_TIMEOUT_SECONDS → 2.0, no raise."""
        state_dir, outbox_dir = tmp_state
        mod = _load_mod_with_dirs(state_dir, outbox_dir)
        with patch.dict(os.environ, {"BOT_ERRORS_REACHABILITY_PROBE_TIMEOUT_SECONDS": "abc"}):
            assert mod.reachability_probe_timeout() == 2.0
