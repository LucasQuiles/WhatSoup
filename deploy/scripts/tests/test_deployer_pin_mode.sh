...
head_led=$(python3 -c "import json;print('deadbeefdeadbeefdeadbeefdeadbeefdeadbeef' in json.load(open('$tmp/ledger.json'))['approved_heads'])")
...
head_count=$(python3 -c "import json;a=json.load(open('$tmp/ledger.json'))['approved_heads'];print(a.count('deadbeefdeadbeefdeadbeefdeadbeefdeadbeef'))")
...
cat > "$tmp/partial-manifest-head-ledger.json" <<JSON
{"schemaVersion":1,"files":[{"path":"deploy/scripts/lib/bot_errors_redaction.py","sha256":"1448da21ae9b598d4cafb342fc1c7aa042141ec3db7fee11c8ff1368cf94812f"}]}
JSON
echo '{"approved_f10":[],"approved_heads":"not-a-list"}' > "$tmp/bad-head-ledger.json"
if bash "$D" pin "$tmp/partial-manifest-head-ledger.json" "$tmp/bad-head-ledger.json" deadbeefdeadbeefdeadbeefdeadbeefdeadbeef > "$tmp/bad-head-ledger.log" 2>&1; then
  echo "PIN_TEST_FAIL accepted malformed head ledger"; cat "$tmp/bad-head-ledger.log"; exit 1
fi
partial_head_ledger_head=$(python3 -c "import json;print(json.load(open('$tmp/partial-manifest-head-ledger.json')).get('expected_head_sha'))")
if [ "$partial_head_ledger_head" != "None" ]; then
  echo "PIN_TEST_FAIL partial manifest was stamped before head-ledger validation"; cat "$tmp/bad-head-ledger.log"; exit 1
fi
if [ "$head" = "deadbeefdeadbeefdeadbeefdeadbeefdeadbeef" ] && [ "$led" = "True" ] && [ "$head_led" = "True" ] && [ "$count" = 1 ] && [ "$head_count" = 1 ]; then
  echo "PIN_TEST_PASS"
else
  echo "PIN_TEST_FAIL head=$head led=$led head_led=$head_led count=$count head_count=$head_count"; exit 1
fi
