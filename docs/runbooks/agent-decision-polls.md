...
- **Inspection**: `sqlite3 <db-path> "SELECT map_key, chat_jid, source, resolution, datetime(closes_at/1000,'unixepoch') AS closes, datetime(hard_closes_at/1000,'unixepoch') AS hard_closes FROM pending_polls ORDER BY created_at;"` lists every persisted poll with its soft/hard expiry. The runtime DB path follows the XDG runtime layout (`~/.local/state/whatsoup/<instance>/...`).
