...
| soup/no-raw-form-control | shadow | scoped-error per dir | P2 | deterministic inventory gate compares live shadow ESLint output to generated `console/design-raw-form-control-inventory.json`; current generated manifest is 9 total = 9 consumer-migration + 0 primitive self-hits |
...
- **Purpose:** `input`, `select`, `textarea` render through the promoted FormControl primitive
  ...
  counted select consumer is `console/src/components/line-detail/ConfigEditDialog.tsx`; the former
  GroupDetailModal select is now routed through `SelectInput`, not a button-class residue.
...
- **FP strategy:** directory scoping plus the deterministic inventory gate. Current generated
  manifest is exactly 9 consumer hits. D4.2 intentionally cleared the former 5 form-kit self-hits
  ...
  D4.3o cleared `Inbox` by routing its message composer through `TextArea`.
