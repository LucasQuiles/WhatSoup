...
| soup/no-raw-field-label | shadow | scoped-error (non-primitives) | P2 | 24 raw `c-field-label` labels across 7 files (2026-06-19); fall-only shadow ratchet keyed in `lint-shadow-baseline.json`; burn down by migrating hand-rolled `<label className="c-field-label">` to the `Field` wrapper (`components/primitives/FormControl.tsx`), then promote to scoped-error |
