...
| soup/no-raw-form-control | shadow | scoped-error per dir | P2 | deterministic inventory gate compares live shadow ESLint output to generated `console/design-raw-form-control-inventory.json`; current generated manifest is 20 total = 20 consumer-migration + 0 primitive self-hits |
...
- **FP strategy:** directory scoping plus the deterministic inventory gate. Current generated
  manifest is exactly 20 consumer hits. D4.2 intentionally cleared the former 5 form-kit self-hits
  ...
  `TextInput` and auth-method radios through `RadioField`.
