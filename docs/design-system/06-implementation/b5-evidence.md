...
| Test integrity | **PASS with named caveats** | 42 hook tests fresh-green; appended-whole describe blocks per directives §3. Suppressions: 2 total, both with rationale + expiry 2026-12-31 (`use-exit-presence.ts:169`; `b5-inert-toast.test.tsx` fire-once harness). The D7 packet's strong-claim audit items stand acknowledged, with two since FIXED: the browser file header no longer claims the jsdom-only opener-refocus proof, and the reduced-motion browser test now closes through `rerender(open=false)` instead of `cleanup()` so it fails if a closing dwell remains. The toast liveness browser test also proves the body-level dismiss button path by clicking it and waiting for alert removal. Still standing: the browser-motion keyframe-name asserts are `if (closingShell)`-conditional with unconditional bounded-removal terminals; the jsdom closing-phase tests carry documented dual-outcome conditionals (`primitives-modal.test.tsx` — rAF timing makes both outcomes valid in jsdom), with deterministic versions in the hook suite under stubbed durations and fake timers; one explicitly labeled INCONCLUSIVE test pins the jsdom AnimationEvent limit. No assertion inversions; no deleted coverage (toast suites byte-stable through the portal move) |
| A11y / reduced motion | **PASS** | Inert is behaviorally proven in-engine (focus refusal + post-close refocusability, `b5-inert-toast.test.tsx`; live row 13). Toast live region (`role="alert"`/`aria-live`, Toast.tsx) portals outside the inert subtree so alerts are never silenced; the same browser proof clicks the portal dismiss button and observes alert removal. Focus trap untouched (`use-dismissable` consumed as-is — packet §5.3 double-protection holds); restoration ordering pinned (C-B5-4). Reduced motion is off-and-instant both ways with one SSOT (CSS media kill + zero-duration read; current reduce-context browser proof closes the modal through `open={false}` and observes no shell dwell). Drawer exception is an a11y POSITIVE: inerting `#root` would have inerted the inline drawer itself and contradicted the squeeze rule's live-table contract |
