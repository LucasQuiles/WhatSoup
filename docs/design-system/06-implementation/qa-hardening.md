...
| design-lint-fixtures run against emptied/renamed/parse-broken rule sources | fails closed (`EMPTY_RULE_SET`, exit 1) — `check-design-lint-fixtures.mjs` flags when implemented+stub+registered rule lists all parse to zero, so the fixture-coverage checker can no longer vacuously PASS over an empty rule set. Negative fixture in `tests/scripts/design-lint-fixtures.test.ts` |
