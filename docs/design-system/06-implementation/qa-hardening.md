...
**Harness note (reduced-motion and exit-presence proof):** `vitest.browser.config.ts` supplies `reducedMotion: 'reduce'` through the Playwright provider's `contextOptions`, the surface the provider actually reads. The reduced browser lane asserts the media query matches, a real production modal shell computes `animationName: none`, and close removes it without waiting for animation. The separate motion lane explicitly requests `no-preference`, asserts the media query does not match, and requires non-null production modal, backdrop, and drawer elements with exact enter-animation names and positive computed durations. Its close proofs require the modal shell/backdrop and drawer shell/scrim after `open=false`, `data-state="closing"`, exact exit-animation names and positive durations, non-interactive closing backdrop/scrim, then condition-based removal after animation completion. `useExitPresence` retains the previously open shell across the falling edge so its layout effect can inspect the authored exit duration; zero-duration closes clear before paint. These checks replace the prior conditional `b5-exit-motion` assertions that could pass after the shell had already disappeared.

**Harness note (motion-lane context made explicit; reduced-lane pins; replaced suite removed):** `vitest.browser.motion.config.ts` now passes `reducedMotion: 'no-preference'` through the Playwright provider's `contextOptions` — the surface the provider actually reads — instead of relying on the absence of a context override, so the motion lane's no-preference claim above is enforced by configuration rather than default behavior. The reduced lane's `b5-inert-toast` suite additionally pins its context (asserts the `prefers-reduced-motion: reduce` media query matches) and its no-animation ground truth (the open production modal computes `animationName: none`, with `console/src/index.css` imported so the computation reads real stylesheet values). The superseded conditional `b5-exit-motion` suite is deleted; its replacement proofs are the exit-presence checks described in the note above. No visual, contrast, or landmark contract changed.
...
**Harness note (provider-model catalogue consumer — use-fleet mock completion):** the editable provider-model input adds a `Settings → ProviderModelInput → use-fleet.useProviderModels` import edge, so `tests/browser/viewport-matrix.test.tsx`'s explicit-shape `use-fleet` factory now enumerates that hook (`data/isPending/isError`). Without the export, strict browser module loading fails before test discovery; with the hook-shaped stub, the existing viewport assertions run unchanged (browser suite green, 9 files / 161 tests). No visual, contrast, layout, or assertion contract changed.
...
**Harness note (react-router 8.3.0 consolidation):** the `react-router-dom` → `react-router` package consolidation (CSRF fix GHSA-7v8v-6qfh-68rf) re-points the browser harness's module surface without changing any proof: `tests/browser/smoke.test.tsx`'s Pattern-C `vi.mock` and its interception test now target `react-router` (the v8 root package exports the full former `-dom` surface), the other browser suites (`a11y-contracts`, `keyboard-proofs`, `perf-budget`, `viewport-matrix`) flip import specifiers only, and `vitest.browser.config.ts` re-aims its resolve alias + `deps.inline` at the consolidated package — the old `react-router-dom` node_modules path no longer exists after the bump, and a stale alias fails every browser session at module fetch. No visual, contrast, landmark, motion, or assertion contract changed — specifier/alias re-pointing only (console suite 224/225 with the single failure proven pre-existing at base; tsc clean).
