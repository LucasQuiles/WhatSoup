...
**Harness note (reduced-motion context proof):** `vitest.browser.config.ts` now supplies `reducedMotion: 'reduce'` through the Playwright provider's `contextOptions`, the surface the provider actually reads. The reduced browser lane asserts the media query matches and a real production modal shell computes `animationName: none`. The separate motion lane explicitly requests `no-preference`, asserts the media query does not match, and requires non-null production modal, backdrop, and drawer elements with their exact enter-animation names and positive computed durations. The prior `b5-exit-motion` checks were conditional and could pass after the shell disappeared, so they were replaced rather than reported as exit proof. Exit dwell remains a separate blocker: `useExitPresence` currently derives `mounted` false on the `open=false` render before its effect can inspect the shell and establish `closingActive`; this harness repair makes no exit-motion completion claim.
...
**Harness note (v3.5 T5 b-03 — fleet suite re-pin to the mockup content grid):** the viewport-matrix Fleet (SoupKitchen) suite moves off the legacy tailwind idiom (`flex-col lg:flex-row`, lg = 1024px) onto the v3.5 mockup SSOT: `.fleet-content` is a CSS grid (`1fr` + `--fleet-activity-w` 320px) that stacks to a single column under the mockup's `@media (max-width:1100px)`. Legs now pin the boundary exactly — 1101px side-by-side (2 columns, KPI strip 5-up), 1100px stacked (the `max-width` query matches at the boundary; KPI strip 2-up), 1099px stacked (grid + KPI 2-up). The tailwind-class candidate-scan fallbacks are gone; assertions read computed `grid-template-columns` on `.fleet-content`/`.fleet-kpis` directly, so no stale-pin false-passes survive the idiom change. 48/48 viewport-matrix tests green locally (Chromium).
...
**Harness note (v3.5 T5 b-05 — Skills Hub suite + the mockup's own 900px breakpoint):** the viewport matrix gains a 4-leg Skills Hub suite pinning the filters rail at skills-hub.html's `@media (max-width:900px)` — the THIRD distinct SSOT breakpoint in the program (chrome/fleet 1100px, agents 1000px, skills 900px): rail visible at 196px at 901px, hidden at the 900px boundary, hidden at 899px, plus the results-column scroll ownership leg (`.skills-page` clips, `.skills-main` owns `overflow-y:auto` at 1440×500). The page renders via `wrapPage(<SkillsHub />)` — catalog data is a static in-repo SSOT (`lib/plugin-catalog.ts`), so no hook mocks are needed beyond the existing factories. 50/50 viewport-matrix tests green locally (Chromium).

**Harness note (v3.5 T5 b-06 — Dream Lab suite + the mockup's own 980px breakpoint):** the viewport matrix gains a 5-leg Dream Lab suite: queue+review side-by-side at 981px, stacked with the queue border swap at the 980px boundary (the FOURTH distinct SSOT breakpoint — chrome/fleet 1100px, agents 1000px, dream 980px, skills 900px), stacked at 979px, review-pane scroll ownership at 1440×500, and the bead acceptance item — the proposed-diff column caps at 72ch, pinned via a probe element bound to the `.dream-diff` recipe (the honest-empty page renders no diff block today). The page renders via `wrapPage(<DreamLab />)`; the live queue is source-free (no Dream API), so no fixture mocks are needed. 51/51 viewport-matrix tests green locally (Chromium).

**Harness note (v3.5 T5 b-07 — Inbox suite + the mockup's own 760px breakpoint):** the viewport matrix replaces the retired v3 container-query legs with an 11-leg v3.5 Inbox suite: context-pane collapse boundary at the mockup 1100px viewport threshold (1101 visible / 1100 hidden / 1000 hidden), the CSS-only master-detail switch at 760px (`data-mobile-detail` mirrors React selection state — the FIFTH distinct SSOT breakpoint: chrome/fleet 1100px, agents 1000px, dream 980px, skills 900px, inbox 760px), the bead acceptance item pinned as computed geometry (caps, input, and Send all measure exactly 36px — `var(--inbox-composer-h)`), and the no-horizontal-overflow sweep. jsdom contracts (`inbox-page.test.tsx`, 27 tests) carry a disposition record for the v3 pins they replace: search lane and contact actions left the surface per the mockup SSOT (the v3 DM access path was also broken — 'number' vs the 'phone'|'group' validator); mark-read moved from a manual button to on-open against the same real route. `inbox-unified.test.ts` (18 tests) pins the pure plane helpers, incl. `normalizeTypingJid` mirroring `toConversationKey` (device-strip, `_at_g.us`, foreign domains, garbage→null). The v3 DD-8 ink pin moved to `ThreadPane`'s thead sub lane (sole line·kind rendering, -v35 quiet register). 47/47 viewport-matrix tests green locally (Chromium).

**Harness note (v3.5 T5 b-08 — Deployments suite at the mockup's shared 1000px):** the viewport matrix gains a 5-leg Deployments suite: summary strip + card-body grids at 4 columns at 1440×900 and 1001px, 2 columns at the 1000px boundary (the mockup's own threshold, shared with the agents surface) and 999px, plus the bead acceptance leg — fits 1440×900 with no horizontal overflow while `.deploy-main` owns internal scroll. The surface renders ONE real deployment (the admin lane has zero multi-host runtime basis per v35/05): jsdom contracts (8 page + 8 lib) pin the live wiring (lines/agents/issues/uptime from `/api/lines` + `/api/version` + `/livez`), the honest no-hub KPI (`—` + "no org hub API"), the degraded worst-of state pill, the retired-update affordance, and the pair card as a disclosed designed-state (placeholder code, both actions disabled with reasons). 51/51 viewport-matrix tests green locally (Chromium).

**Harness note (v3.5 T5 b-09 — Settings suite at the mockup's own 800px):** the viewport matrix gains a Settings suite: 190px section nav + content grid above 800px, horizontal-scroll nav at the 800px boundary (the SIXTH distinct SSOT breakpoint: chrome/fleet 1100px, agents 1000px, dream 980px, skills 900px, inbox 760px, settings 800px), the fits-1440×900 acceptance leg, and the swatch-sync contract (aria-pressed follows the live theme; previews re-scope -v35 tokens through per-swatch `data-theme`, zero raw hex). jsdom contracts pin: nav==sections 1:1 (the wave-4b law), the honest omissions (no workspace-name/admin-identity rows — no backend fields), disabled-with-note controls (auto swatch, reduced-motion, export, reset), the REAL alert-mute backend end-to-end (fleet silences list/mute/unsilence with invalidation), provider-key set/verify/revoke, and the session lock row. The a11y landmark mock graduated for /settings. 51/51 viewport-matrix tests green locally (Chromium); 13/13 jsdom contracts green.

**Harness note (v3.5 T5 b-10 — Journey suites + a reduced-motion harness finding):** the viewport matrix gains Journey legs: splash proof-triptych 3→1 columns at the mockup's own 700px boundary (701/700), journey card radius computed at `--r-journey-lg` 16px (the register acceptance item), and the ceremony PLAY contract computed in Chromium — glow `journey-glowplay` ≤0.8s with iteration count 1, avatar `journey-pop` count 1 (13-§2). The 13-§3 removal media block is source-pinned in `tests/console/ceremony-motion-contract.test.ts` (parse-verified: single iteration, ends at opacity 0, radial accent, hatch-scoped class). **Harness finding, surfaced not fixed:** the suite's `context.reducedMotion: 'reduce'` option (vitest.browser.config.ts:97) is INERT in this provider version — `matchMedia('(prefers-reduced-motion: reduce)')` does NOT match inside browser tests (probe-proven), and the vitest page wrapper exposes no per-test media emulation. Suites that believed they ran under reduce (b5) were not; that investigation is its own bead. jsdom: 24 contracts + the retargeted v3 landing suite (17, disposition recorded — it caught and fixed a skipped heading level in my proof cards). The D1.3 landmark suite now runs on a one-line fleet: zero lines became genuinely first-run with the redirect, which is correct behavior outside that suite's subject. 133/133 browser legs green (Chromium).

**Harness note (v3.5 T5 b-13 integration — the reduced-motion fix relocates the ceremony play contract):** #2158 corrected the browser lane's reduced-motion wiring (factory-level `contextOptions`, not `instances[].context`), which made `tests/browser/` genuinely reduced-motion for the first time. That immediately falsified b-10's ceremony PLAY leg, which had been asserting `animation-name: journey-glowplay` in a lane that now (correctly) removes animation — the leg only ever passed because the option was inert. Both halves of 13-§2/§3 are now proven where each is true: the REMOVAL law is computed in the reduce lane (`viewport-matrix.test.tsx` — glow and avatar resolve to `none`), and the PLAY contract moved to the no-reduce motion lane (`tests/browser-motion/ceremony-play.test.tsx` — `journey-glowplay` ≤0.8s, iteration count 1, `journey-pop` count 1). The pair is its own falsifier: the same elements compute differently in the two lanes, so a regression in either direction fails one of them. Class note for future motion legs: a browser-lane test asserting computed animation MUST import `console/src/index.css`, or every `animation-name` resolves to `none` and the assertion passes or fails for the wrong reason (cost one debugging cycle here). Both CI lanes are already wired (quality.yml runs `test:browser` then `test:browser:motion`).

**Harness note (v3.5 T5 b-11 — Motion system):** the v3.5 motion law (13-ceremony-motion) is now the enforced budget: EXACTLY ONE ambient loop product-wide (`ambient-disc` — opacity 1→.35→1, 2400ms ease-in-out, on the Badge live status disc), the `.lift` idiom (180ms, within the 160–200ms budget), ceremony scoped to journey.css (b-10). The v3 loop family is retired at the root: breathe-ring halo, legacy breathe, typing-bounce (static dots now), and the skeleton shimmer loop (static `.skeleton-bar` placeholders — the waiver's perceived-performance rationale preserved without the loop). Waivers WVR-005/006 closed with their subjects gone; design-regression check 13's sanctioned set tightened to `ambient-disc` ONLY; the ds-hardening ambient-budget inventory now permits exactly that one loop in exactly motion.css. Contract pins in `tests/console/motion-system-contract.test.ts` (spec-exact loop, single-infinite scan, retirement anti-regression, lift budget, reduce removal). Skeleton/line-detail/design-guard suites retargeted with the retirement on-record. The acceptance gate of 13-§5 passes: one ambient loop ✓, ceremony contract (b-10) ✓, reduce removal ✓, no unsanctioned gradients/glow ✓ (check 13 + regression blocking set green).

**Harness note (v3.5 T5 b-12 — Perf instrumentation + report-only lane):** the 19-performance-budget §2 instrumentation ships: WS throughput meter (per-line/global 1s-window rates against the §1 caps, counters on `window.__soupPerf`, over-cap as sampled debug notes — the meter reports, never alters transport behavior), long-task observer (PerformanceObserver, sampled 1%, engine-absent no-op), and the dev-mode Profiler callback. The CI `perf-lane` job runs report-only (`continue-on-error`) because the §1 numbers were never owner-approved (G2 pending): it measures bundle entry gzip (130KB vs 250KB), cold FCP and warm FCP against the provisional budgets, and records the two deferred legs (200-line mount, event-storm frame cost — they need a fixture-fed headless harness). `PERF_LANE_ENFORCE=1` flips it blocking on sign-off. Harness facts recorded: `performance.getEntriesByType('paint')` returned an EMPTY buffer in the Playwright/vite-preview harness — the lane sources FCP via `addInitScript` + PerformanceObserver `buffered:true` (the stable path). The vite `outDir` is repo-root `dist/`, not `console/dist/`. 5/5 meter contracts green.

**Harness note (T5 b-12 deferred legs landed):** the 200-line mount and event-storm frame-cost legs ship as `tests/browser/perf-budget.test.tsx` — real Chromium measurements via React Profiler `actualDuration` (mount render cost against the <50ms re-render class, 10 single-line storm updates against the <16ms frame budget), with 200 fixture rows proven in-DOM so the measurement is never vacuous. The legs report under the PROVISIONAL §1 numbers (owner sign-off pending) and flip to hard assertions with `PERF_LANE_ENFORCE=1`. Also on this branch: the perf module is import-safe in non-window environments (module-load window touch broke node-env test imports), and the perf-lane readiness hardened (local vite binary, 60s window, server log on failure) after its first CI run died on cold npx resolution. **Reduced-motion harness debt fixed at the root (separate PR #2158):** `instances[].context.reducedMotion` was never read by the provider — the factory-level `contextOptions` is the correct spelling; reduce now actually matches inside browser tests (128/128 green with it ACTIVE). 130/130 browser tests green.

**Harness note (reduced-motion context — the inert-option debt fixed):** the browser suite believed it ran under `prefers-reduced-motion: reduce` via `instances[].context.reducedMotion` — a key the `@vitest/browser-playwright` provider never reads, so the media query did NOT match inside browser tests (probe-proven: `matchMedia` returned false). The provider takes `BrowserContextOptions` at the FACTORY level: `playwright({ contextOptions: { reducedMotion: 'reduce' } })`. With the correct key the probe passes and the full browser suite stays green (128/128) with reduce ACTUALLY active — the assertions were written to hold in both states. The 13-ceremony-motion removal law is now computed-proof in Chromium (b-10's source-pin fallback can upgrade on its branch). Recorded: the earlier `context` spelling was silently ignored — an option-shape failure worth grepping for in any future browser config.
