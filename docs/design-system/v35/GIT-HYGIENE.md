...
| Worktree `.worktrees/soup-v35-design` | local `WhatSoup` checkout | Design-docs working tree (docs-only) | registered; working files reconcile on first write-capable session (branch tip advanced via object-store commits from a sandboxed session) |
...
4. **Mirror discipline.** the `agents/q` mirror (no remote) is a scratch mirror only — every file must
...
5. **Worktree accountability.** Program-owned: `.worktrees/soup-v35-design` (registered,
   provisioned, push-capable). The ~10 worktrees under the `agents/q` checkout (restart-reliability,
