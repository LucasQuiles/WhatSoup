...
Every entry below was verified against the section linked. States the program discussed but
**no spec defines** (stale, disconnected, read-only) are not in this table; they appear in
§3 as findings.
...
| 3 | **disconnected is a QA case with no console-level spec home**: line-level disconnection is covered by the Badge taxonomy (unreachable / unlinked), but a console-transport-loss state (failed polling/socket, whole-surface disconnect) has no defined rendering | [qa-hardening §4](../06-implementation/qa-hardening.md); [badge.md](components/badge.md#the-shape-law) covers line status only | debt-register entry; decide whether the error-with-remedy composites plus the feed cover transport loss or a connection-status surface needs spec'ing; record the ruling |
