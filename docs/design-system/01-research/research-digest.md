...
5. **Navigation.** Keep the shallow IA (4 pages + line-detail tabs). Chrome sits 1–2 contrast notches below content (Linear). Tab switches are instant. Consistency rule (DRUIDS): one filter pattern, one time-range control, one search pattern everywhere. A command palette (Linear/Raycast lineage) is a natural v3+ candidate but **out of scope** for the reskin program — note for backlog only. **Superseded (user-approved):** the navigation *chrome* is now a vertical **left rail**, not the original horizontal top bar — the showcase left-rail treatment was promoted to spec (decision-log "top bar → left rail"; layout-density §9). The shallow IA itself is unchanged (still Fleet/Inbox/Ops + line-detail — no new pages); only the chrome shape moved. The "chrome recedes 1–2 notches" posture is preserved (the rail uses `surface-inset`).
