...
| `arch.env-read-justification` | ast | block‡ | eslint, ci | Direct `process.env` access outside the typed-config seam (`src/config.ts`) needs an `env-allowed:` justification or a reviewed allowlist row; the suppression-immune lexical ratchet (`tests/scripts/env-read-allowlist.test.ts`) blocks in CI, the eslint ring is a warn-only editor-latency mirror, and both consume `eslint-rules/env-read-allowlist.json` (#2192 slice 5a; per-site markers arrive with 5b). |
