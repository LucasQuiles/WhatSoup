...
| P0-7 | **Pin-update race during in-flight heal** — pin loaded at top of cycle, used unchanged through `deploy()`; a concurrent re-pin deploys the old bundle. (Deploy-side verify now resolves its expected hashes from `deploy/bot-errors-runtime-manifest.json` at each invocation rather than a hardcoded list — but `deploy()` still reads that manifest once at its own startup, with no re-check inside the lock, so the race this row describes is unchanged.) | `bot-errors-selfcheck.py:772–884`; `whatsoup-bot-errors-deploy.sh:40-53,74` | Re-read + re-verify the pin **inside the lock** before `deploy()`; abort with `current_changed` if `head_sha` changed. |
