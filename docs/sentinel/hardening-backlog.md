...
| P0-4 | **Q token single-use NOT enforced** — `redeemedAt` is read but **never written** by the worker → token replay within the TTL window. | `bot-errors-sentinel.py:911,986`; Q-worker ack flow | Worker writes `redeemedAt` into `qRemediation` + `save_state`; sentinel's `q_remediation_redeemed` already honors it. |
