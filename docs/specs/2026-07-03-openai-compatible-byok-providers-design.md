...
| providerConfig validation | `src/core/agent-config-validator.ts:614-649, 656-674` | `baseUrl` must be valid http(s) URL; `apiKeyService` must be a known service; `apiKeyService` without `baseUrl` rejected as inert; opencode-cli baseUrl requires a resolvable model (API providers exempt) | Config errors fail loud at load, before any traffic | — |
