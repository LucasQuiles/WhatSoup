...
- `FALLBACK STATUS` replies with the effective provider, the window expiry as an ISO timestamp (`none` when on the primary), the process-local fallback turn counters (`served` / `empty`, reset on restart), and the ordered fallback chain readiness when a chain is configured.
