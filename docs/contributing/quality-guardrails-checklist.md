...
| PNG estate ratchet | `npm run guard:png-estate` | #2219 Option A: `artifacts/` PNGs are untracked by policy and the tracked-PNG census (count and bytes, `docs/design-system` + `docs/screenshots`) may only shrink, and each tracked `docs/screenshots` PNG is at most 640 KiB; the `--staged` variant (pre-commit, CI-only) rejects any staged `artifacts/` PNG, any new PNG over 100 KiB, and any changed PNG over 100 KiB unless it is a tracked `docs/screenshots` PNG replaced by one no larger than its committed version and at most 640 KiB. Refuses INCONCLUSIVE (exit 2) on an empty scan root. | yes |
