...
Status: active ledger. Updated: 2026-06-14T21:18Z.
...
- GitHub PR queue is volatile; re-run `gh pr list --state open` before merge or cleanup. Latest read at 2026-06-14T21:17Z showed no open PRs after PR #852 merged.
- Audited docs/merge hygiene checkpoints: PR #848 merged as `2954b695`; PR #849 merged as `35b18970`; PR #850 merged as `470e768b`; PR #851 merged as `cb1bedad`; PR #852 merged as `556919e4` and records the mock-data coverage slice.
...
- Current classifier at 2026-06-14T21:18Z: primary checkout is on `docs/post-852-hygiene-ledger-20260614` from `origin/main` head `556919e4`; local `main` is owned by a peer worktree. Merged-local refs outside the current branch are attached to dirty peer worktrees or still have unclassified local state, so they are not deletion candidates.
