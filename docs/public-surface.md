...
| `cli:npm.reconcile-launchd-restart-policy` | `npm run reconcile-launchd-restart-policy` | `package.json` | beta | active | Dry-run by default; `--apply` validates a generated plist identity, then atomically reloads and starts/restarts one named macOS instance, failing closed when bootout, reload, or rollback fails |
