...
| `cli:npm.guard-png-estate` | `npm run guard:png-estate` | `package.json` | stable | active | Shrink-only ratchet on the tracked-PNG census (#2219 Option A): count and total bytes bounded by the post-migration baseline, artifacts/ must hold zero tracked PNGs; each reducing PR lowers the baseline in the same change |
| `cli:npm.guard-png-estate-staged` | `npm run guard:png-estate:staged` | `package.json` | stable | active | Pre-commit half of the png-estate policy: reject any staged PNG under artifacts/ (untracked by policy) and any staged new/changed PNG over 100 KiB (compress or host externally), except a tracked docs/screenshots/ PNG replaced by one no larger than its committed version and at most 640 KiB |
