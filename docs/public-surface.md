...
| `http:health.send` | `POST /send` | `src/core/health.ts:907` | stable | active | Send a text message |
...
| `http:health.access` | `POST /access` | `src/core/health.ts:1241` | stable | active | Allow / block contact or group |
| `http:health.mark-read` | `POST /mark-read` | `src/core/health.ts:1349` | stable | active | Zero unread + chatModify |
| `http:health.typing` | `GET /typing` | `src/core/health.ts:1419` | stable | active | Currently-composing JIDs from presence cache |
| `http:health.heal` | `POST /heal` | `src/core/health.ts:1142` | stable | active | Inject Type-3 repair report |
| `http:health.agent-compact` | `POST /agent/compact` | `src/core/health.ts:1053` | stable | active | Out-of-band compaction; requires `chatJid` for per-chat / shared scopes |
