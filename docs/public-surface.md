...
| `http:health.access` | `POST /access` | `src/core/health.ts:909` | stable | active | Allow / block contact or group |
| `http:health.mark-read` | `POST /mark-read` | `src/core/health.ts:1001` | stable | active | Zero unread + chatModify |
| `http:health.typing` | `GET /typing` | `src/core/health.ts:1071` | stable | active | Currently-composing JIDs from presence cache |
| `http:health.heal` | `POST /heal` | `src/core/health.ts:824` | stable | active | Inject Type-3 repair report |
| `http:health.agent-compact` | `POST /agent/compact` | `src/core/health.ts:735` | stable | active | Out-of-band compaction; requires `chatJid` for per-chat / shared scopes |
