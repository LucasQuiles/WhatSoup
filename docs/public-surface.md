...
| `http:health.send` | `POST /send` | `src/core/health.ts:620` | stable | active | Send a text message |
...
| `http:health.access` | `POST /access` | `src/core/health.ts:940` | stable | active | Allow / block contact or group |
| `http:health.mark-read` | `POST /mark-read` | `src/core/health.ts:1032` | stable | active | Zero unread + chatModify |
| `http:health.typing` | `GET /typing` | `src/core/health.ts:1102` | stable | active | Currently-composing JIDs from presence cache |
| `http:health.heal` | `POST /heal` | `src/core/health.ts:855` | stable | active | Inject Type-3 repair report |
| `http:health.agent-compact` | `POST /agent/compact` | `src/core/health.ts:766` | stable | active | Out-of-band compaction; requires `chatJid` for per-chat / shared scopes |
