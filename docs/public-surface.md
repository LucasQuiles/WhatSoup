...
| `cli:npm.validate-private-operation-record` | `npm run validate-private-operation-record` | `package.json` | beta | active | Read-only schema-v1 host-operation receipt validation; `schema` publishes closed registries and command schemas, while `validate --record <absolute-path> --format json` exits 0 valid / 1 actionable / 2 read failure with content-free JSON errors |
