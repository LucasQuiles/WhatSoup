...
|---|---|---|---|---|---|
| `http:fleet.providers.list` | `GET /api/providers` | `src/fleet/index.ts:374` | beta | active | Provider catalog (id, displayName, type, needsApiKey, providerConfig fields) derived from `PROVIDER_IDS` |
...
| `http:fleet.feed` | `GET /api/feed` | `src/fleet/index.ts:372` | stable | active | Activity feed across instances |
| `http:fleet.typing` | `GET /api/typing` | `src/fleet/index.ts:371` | stable | active | Currently-typing indicators |
| `http:fleet.directories.check` | `GET /api/directories/check?path=...` | `src/fleet/index.ts:373` | stable | active | Path writable probe |
...
| `http:fleet.lid-mappings.list` | `GET /api/lid-mappings` | `src/fleet/index.ts:433` | stable | active | List cross-instance LID mappings |
| `http:fleet.lid-mappings.sync` | `POST /api/lid-mappings/sync` | `src/fleet/index.ts:434` | stable | active | Sync mappings between instances |
| `http:fleet.silences.list` | `GET /api/fleet/silences` | `src/fleet/index.ts:368` | beta | active | List active fleet-wide alert silences with typed `availability`, `readBasis`, `observedAt`, and revision metadata. A bounded last-known-good view is marked stale and never treated as current. |
| `http:fleet.silences.add` | `POST /api/fleet/silence` | `src/fleet/index.ts:369` | beta | active | Add a silence rule (instance, duration). Requires a current strict registry read; stale LKG is `409` and no usable basis is `503` through the closed `fleet-error-v1` response. |
| `http:fleet.silences.remove` | `DELETE /api/fleet/silence/:name` | `src/fleet/index.ts:370` | beta | active | Remove a named silence rule; requires the same current strict registry read as add. |
| `http:fleet.auth-ticket.mint` | `POST /api/auth-ticket` | `src/fleet/index.ts:868`, `src/fleet/auth-ticket.ts` | stable | active | Mint short-lived API/SSE ticket (root Bearer, or console session cookie + same-origin proof). Loopback TCP source only (C2). |
| `http:fleet.console-session.create` | `POST /api/console-session` | `src/fleet/index.ts:831` | beta | active | Console unlock: loopback source only (C2), validates the root token + same-origin proof, sets an HttpOnly `SameSite=Strict` session cookie (24h TTL; `Secure` when the unlock arrives over TLS); the browser never holds the root token |
| `http:fleet.console-session.delete` | `DELETE /api/console-session` | `src/fleet/index.ts:855` | beta | active | Console lock/logout: revokes the presented session and clears the cookie |
| `http:fleet.ws-ticket.mint` | `POST /api/ws-ticket` | `src/fleet/index.ts:895`, `src/fleet/ws-ticket.ts` | stable | active | Mint short-lived WebSocket ticket (root Bearer, or console session cookie + same-origin proof). Loopback TCP source only (C2). |
| `http:fleet.signals.ingest` | `POST /api/signals` | `src/fleet/index.ts:912`, `src/fleet/routes/signals.ts` | alpha | active | Byte-stable idempotent incident signal ingestion. Producer Bearer credential (pre-gate carve-out: fleet root/tickets do NOT authorize production). 201 receipt / 200 + `Idempotent-Replay: true` / 409 identity conflict; 32 KiB body cap; per-producer rate limit. |
| `http:fleet.producers.register` | `POST /api/producers` | `src/fleet/index.ts:916`, `src/fleet/routes/signals.ts` | alpha | active | Register an incident producer (root Bearer). Returns the single-use enrollment secret; the plaintext appears only in this response. |
| `http:fleet.producers.credential.mint` | `POST /api/producers/:id/credential` | `src/fleet/index.ts:921`, `src/fleet/routes/signals.ts` | alpha | active | Mint a producer credential: enrollment-secret exchange, or rotation with the current credential (24h overlap window). |
| `http:fleet.producers.credential.revoke` | `DELETE /api/producers/:id/credential` | `src/fleet/index.ts:921`, `src/fleet/routes/signals.ts` | alpha | active | Revoke a producer and its credentials (root Bearer). Returns 204. |
