...
| `cli:npm.agent-lease` | `npm run agent:lease` | `package.json` | experimental | active | Quarantined writer/lineage-lease precursor for multi-agent Git work; it is not CP-WA1 authority and must not authorize mutation until continuous takeover exclusivity, exact workspace accounting, verified producer identity, and protected commit transition are proven; exit 0 OK, 1 BLOCK, 2 INCONCLUSIVE |
...
| `cli:npm.drift-classify` | `npm run drift:classify` | `package.json` | beta | active | Local native report-only receipt-invalidation observation from the canonical exact-object classifier; requires full base/observed OIDs and the digest of the exact manifest bytes read from the caller-supplied base OID. It is not `ci-control-result-v1`: a later protected adapter must add trusted policy, producer, precondition, terminal-attempt, and aggregation evidence. Exits 0 for unchanged or explicitly proven disjoint advisory observations, 1 for a proven path conflict, and 2 when prior assurance requires reconciliation or is inconclusive; it is not merge authority |
| `cli:npm.guard-drift-coverage` | `npm run guard:drift-coverage` | `package.json` | beta | active | Report-only CI self-check that the exact HEAD tree is covered by canonical manifest risk rules. Missing coverage is INCONCLUSIVE; this guard does not authorize drift |
...
| `cli:npm.guard-hooks-installed` | `npm run guard:hooks-installed` | `package.json` | beta | active | Report-only native diagnostic binding the configured `.husky` directory and its four reviewed entrypoint/helper files to `HEAD` with an approved absolute Git launcher; unexpected entries or unavailable evidence are inconclusive, and it neither rewrites configuration nor provides remote enforcement |
| `cli:npm.ci-ref-policy` | `npm run ci:ref-policy -- --help` | `package.json` | beta | active | Safe package help boundary for the report-only outgoing-ref policy canary; full evaluation follows the displayed silent invocation so npm does not echo bounded remote input, manifest-named deletion remains inconclusive until protected private ref binding exists, new branches and release tags remain inconclusive until protected trusted-base provenance exists, and the command is neither invoked by the active hook nor authorized to permit transport |
...
| `cli:npm.verify-release` | `npm run verify:release` | `package.json` | beta | active | Source-readiness verifier only; does not provide artifact, SBOM, provenance, attestation, or deployment assurance; surface still settling |
...
| `cli:npm.ci-classify` | `npm run ci:classify -- --event <event> --candidate <oid> --base <oid> --manifest-digest <digest> --json` | `package.json` | beta | active | Report-only exact-Git-object canary: validates the supplied manifest digest against manifest bytes reread from the caller-supplied base OID; pull-request and merge-group events also require `--merge <oid>`. It is not independently protected policy or aggregate authorization. |
