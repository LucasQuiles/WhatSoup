...
| `cli:npm.continuity-review-proof-pack` | `npm run continuity:review-proof-pack` | `package.json` | beta | active | Read-only manifest-backed continuity review proof pack from an already-migrated `bot.db`; writes redacted proof plus hash/size manifest and fails closed on missing schema, unmigrated DBs, or secret-like artifacts |
