...
| `cli:npm.guard-platform-patterns` | `npm run guard:platform-patterns` | `package.json` | stable | active | Enforce cross-platform portability: block hardcoded platform binaries, unguarded /proc///sys///run/ access, systemctl without launchctl fallback, and GNU-only shell flags; 4 mechanical guards with 45 baselined violations |
