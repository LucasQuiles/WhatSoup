...
Canonical route table: [`src/fleet/index.ts:366-435`](../src/fleet/index.ts) (`ROUTES`
...
|---|---|---|---|---|---|
| `http:fleet.providers.list` | `GET /api/providers` | `src/fleet/index.ts:373` | beta | active | Provider catalog (id, displayName, type, needsApiKey, providerConfig fields) derived from `PROVIDER_IDS` |
| `http:fleet.providers.models` | `GET /api/providers/:name/models` | `src/fleet/index.ts:374`, `src/fleet/routes/providers.ts` | beta | active | Typed provider-native catalogue receipt from the shared runtime resolver. `ok` includes exact IDs, source, and capture age; unavailable sources return a classified reason; unknown execution providers return 404 before probing. |
| `http:fleet.credentials.set` | `PUT /api/credentials/:service` | `src/fleet/index.ts:375`, `src/fleet/routes/credentials.ts` | beta | active | Set or rotate a provider key (`{"value": string}`, single-line, ≤4096 bytes). Closed allowlist; never reads back. Returns `{ ok, service, backend, envShadowed }`. |
| `http:fleet.credentials.delete` | `DELETE /api/credentials/:service` | `src/fleet/index.ts:376`, `src/fleet/routes/credentials.ts` | beta | active | Remove a stored provider key. Returns `{ ok, service, envShadowed, inUse }`; 404 when no key was stored. |
| `http:fleet.credentials.verify` | `POST /api/credentials/:service/verify` | `src/fleet/index.ts:377`, `src/fleet/routes/credentials.ts` | beta | active | One live list-models probe using the stored key (never carried in the request). 10s per-service cooldown (429); returns `{ ok, service, status, envShadowed }`. |
| `http:fleet.credentials.read-rejected` | `GET /api/credentials/:service` | `src/fleet/index.ts:378`, `src/fleet/routes/credentials.ts` | beta | active | Always `405`: credentials are write-only and never read back. |
| `http:fleet.lines.list` | `GET /api/lines` | `src/fleet/index.ts:379` | stable | active | List instances + health |
| `http:fleet.lines.create` | `POST /api/lines` | `src/fleet/index.ts:380` | stable | active | Create instance |
| `http:fleet.lines.get` | `GET /api/lines/:name` | `src/fleet/index.ts:390` | stable | active | Instance detail + config |
| `http:fleet.lines.delete` | `DELETE /api/lines/:name` | `src/fleet/index.ts:389` | stable | active | Stop + cleanup |
| `http:fleet.lines.exists` | `GET /api/lines/:name/exists` | `src/fleet/index.ts:381` | stable | active | Registration probe |
| `http:fleet.lines.provider-status` | `GET /api/lines/:name/provider-status` | `src/fleet/index.ts:382` | beta | active | Per-instance primary/fallback provider, key presence (boolean), resolved primary model, custom-endpoint visibility (`endpointHost` — URL host only — and `apiKeyService` on BOTH `primary` and `fallback`, attributed to the role whose provider actually consumes `providerConfig`: `openai-api`/`anthropic-api` either role, `opencode-cli` as primary only; for the API providers `apiKeyService` is the explicit `providerConfig` override (`null` = no override), for `opencode-cli` it is the model-prefix-derived service `keyPresent` checks; `null` when not consumed by that role's provider or unparseable on disk; readable by any fleet-token or `api`-audience ticket holder — the key-service name identifies the OS-keyring entry, an accepted observability tradeoff), fallback reason/model/reset/probe state, fallback window/counter state, effective provider, active fallback-chain entry, chain eligibility, and line reachability |
| `http:fleet.lines.config-update` | `PATCH /api/lines/:name/config` | `src/fleet/index.ts:404` | stable | active | Update `config.json` |
| `http:fleet.lines.auth-sse` | `GET /api/lines/:name/auth` | `src/fleet/index.ts:405` | stable | active | QR-code SSE stream (legacy; instances with a configured `accountScopeId` refuse with 409 and use the pairing saga routes below) |
| `http:fleet.lines.pairing-preflight` | `POST /api/lines/:name/pairing/preflight` | `src/fleet/index.ts:406`, `src/fleet/routes/ops-auth.ts` | alpha | active | Side-effect-free pairing plan (latch/tree/generation/lease/operation state). accountScopeId-configured instances only; header-borne Bearer required, cross-site refused. |
| `http:fleet.lines.pairing-apply` | `POST /api/lines/:name/pairing/apply` | `src/fleet/index.ts:407`, `src/fleet/routes/ops-auth.ts` | alpha | active | Idempotent evidence-preserving pairing saga (stop, fenced lease, quarantine, fresh-generation pairing, V2 receipt, latch supersession). Same authenticity requirements as preflight. |
| `http:fleet.lines.pairing-status` | `GET /api/lines/:name/pairing/status` | `src/fleet/index.ts:408`, `src/fleet/routes/ops-auth.ts` | alpha | active | Pairing plan + operation record + last helper event (qr/pairing_code/connected). Same authenticity requirements as preflight. |
| `http:fleet.lines.restart` | `POST /api/lines/:name/restart` | `src/fleet/index.ts:402` | stable | active | Restart unit |
| `http:fleet.lines.stop` | `POST /api/lines/:name/stop` | `src/fleet/index.ts:403` | stable | active | Stop unit |
| `http:fleet.lines.send` | `POST /api/lines/:name/send` | `src/fleet/index.ts:398` | stable | active | Send message; accepts `chatJid` or alias `to` + optional `profile` |
| `http:fleet.lines.access-update` | `POST /api/lines/:name/access` | `src/fleet/index.ts:400` | stable | active | Update access control |
| `http:fleet.lines.access-view` | `GET /api/lines/:name/access` | `src/fleet/index.ts:396` | stable | active | View access list |
| `http:fleet.lines.mark-read` | `POST /api/lines/:name/mark-read` | `src/fleet/index.ts:401` | stable | active | Zero unread + chatModify |
| `http:fleet.lines.contacts-save` | `POST /api/lines/:name/contacts` | `src/fleet/index.ts:399` | stable | active | Save contact |
| `http:fleet.lines.contacts-search` | `GET /api/lines/:name/contacts/search` | `src/fleet/index.ts:430` | stable | active | Search saved contacts |
| `http:fleet.lines.chats` | `GET /api/lines/:name/chats` | `src/fleet/index.ts:391` | stable | active | List chats |
| `http:fleet.lines.messages` | `GET /api/lines/:name/messages` | `src/fleet/index.ts:392` | stable | active | Fetch messages |
| `http:fleet.lines.messages-search` | `GET /api/lines/:name/messages/search` | `src/fleet/index.ts:393` | stable | active | Full-text search |
| `http:fleet.lines.metrics` | `GET /api/lines/:name/metrics` | `src/fleet/index.ts:395` | stable | active | 24h / 7d / 30d |
| `http:fleet.lines.logs` | `GET /api/lines/:name/logs` | `src/fleet/index.ts:397` | stable | active | Instance logs |
| `http:fleet.lines.scheduled.list` | `GET /api/lines/:name/scheduled` | `src/fleet/index.ts:409` | stable | active | List scheduled; filter `?status=` |
| `http:fleet.lines.scheduled.create` | `POST /api/lines/:name/scheduled` | `src/fleet/index.ts:410` | stable | active | Schedule a new message |
| `http:fleet.lines.scheduled.cancel-query` | `DELETE /api/lines/:name/scheduled` | `src/fleet/index.ts:411` | stable | active | Cancel by `?id=` |
| `http:fleet.lines.scheduled.get` | `GET /api/lines/:name/scheduled/:id` | `src/fleet/index.ts:412` | stable | active | Single scheduled |
| `http:fleet.lines.scheduled.update` | `PUT /api/lines/:name/scheduled/:id` | `src/fleet/index.ts:413` | stable | active | Update scheduled |
| `http:fleet.lines.scheduled.cancel` | `DELETE /api/lines/:name/scheduled/:id` | `src/fleet/index.ts:414` | stable | active | Cancel scheduled |
| `http:fleet.lines.groups.list` | `GET /api/lines/:name/groups` | `src/fleet/index.ts:415` | stable | active | List groups |
| `http:fleet.lines.groups.create` | `POST /api/lines/:name/groups` | `src/fleet/index.ts:416` | stable | active | Create group |
| `http:fleet.lines.groups.detail` | `GET /api/lines/:name/groups/:jid` | `src/fleet/index.ts:428` | stable | active | Group detail |
| `http:fleet.lines.groups.leave` | `DELETE /api/lines/:name/groups/:jid` | `src/fleet/index.ts:429` | stable | active | Leave group |
| `http:fleet.lines.groups.subject` | `PUT /api/lines/:name/groups/:jid/subject` | `src/fleet/index.ts:417` | stable | active | Update subject |
| `http:fleet.lines.groups.description` | `PUT /api/lines/:name/groups/:jid/description` | `src/fleet/index.ts:418` | stable | active | Update description |
| `http:fleet.lines.groups.participants` | `POST /api/lines/:name/groups/:jid/participants` | `src/fleet/index.ts:419` | stable | active | Add/remove/promote/demote |
| `http:fleet.lines.groups.settings` | `PUT /api/lines/:name/groups/:jid/settings` | `src/fleet/index.ts:420` | stable | active | Announce / locked |
| `http:fleet.lines.groups.invite` | `GET /api/lines/:name/groups/:jid/invite` | `src/fleet/index.ts:421` | stable | active | Fetch invite code |
| `http:fleet.lines.groups.invite-revoke` | `POST /api/lines/:name/groups/:jid/invite/revoke` | `src/fleet/index.ts:422` | stable | active | Revoke + rotate |
| `http:fleet.lines.groups.ephemeral` | `PUT /api/lines/:name/groups/:jid/ephemeral` | `src/fleet/index.ts:423` | stable | active | Disappearing-message duration |
| `http:fleet.lines.groups.member-add-mode` | `PUT /api/lines/:name/groups/:jid/member-add-mode` | `src/fleet/index.ts:424` | stable | active | Toggle who can add members |
| `http:fleet.lines.groups.join-approval` | `PUT /api/lines/:name/groups/:jid/join-approval` | `src/fleet/index.ts:425` | stable | active | Toggle join-approval requirement |
| `http:fleet.lines.groups.requests.list` | `GET /api/lines/:name/groups/:jid/requests` | `src/fleet/index.ts:426` | stable | active | List pending join requests |
| `http:fleet.lines.groups.requests.update` | `POST /api/lines/:name/groups/:jid/requests` | `src/fleet/index.ts:427` | stable | active | Approve / reject |
| `http:fleet.feed` | `GET /api/feed` | `src/fleet/index.ts:371` | stable | active | Activity feed across instances |
| `http:fleet.typing` | `GET /api/typing` | `src/fleet/index.ts:370` | stable | active | Currently-typing indicators |
| `http:fleet.directories.check` | `GET /api/directories/check?path=...` | `src/fleet/index.ts:372` | stable | active | Path writable probe |
| `http:fleet.metrics` | `GET /api/metrics` | `src/fleet/index.ts:394` | stable | active | Fleet-wide metrics |
| `http:fleet.version` | `GET /api/version` | `src/fleet/index.ts:431` | stable | active | Build version |
| `http:fleet.update` | `POST /api/update` | `src/fleet/index.ts:432` | stable | active | Retired in-place update: performs no mutation in any mode; returns a typed `update-by-release-deploy-required` refusal over the SSE error channel. Update by deploying a new release; read availability via `GET /api/version`. |
| `http:fleet.lid-mappings.list` | `GET /api/lid-mappings` | `src/fleet/index.ts:433`, `src/fleet/routes/lid-sync.ts` | stable | active | List cross-instance LID mappings |
| `http:fleet.lid-mappings.sync` | `POST /api/lid-mappings/sync` | `src/fleet/index.ts:434`, `src/fleet/routes/lid-sync.ts` | stable | active | Sync mappings between instances |
| `http:fleet.silences.list` | `GET /api/fleet/silences` | `src/fleet/index.ts:367` | beta | active | List active fleet-wide alert silences with typed `availability`, `readBasis`, `observedAt`, and revision metadata. A bounded last-known-good view is marked stale and never treated as current. |
| `http:fleet.silences.add` | `POST /api/fleet/silence` | `src/fleet/index.ts:368` | beta | active | Add a silence rule (instance, duration). Requires a current strict registry read; stale LKG is `409` and no usable basis is `503` through the closed `fleet-error-v1` response. |
| `http:fleet.silences.remove` | `DELETE /api/fleet/silence/:name` | `src/fleet/index.ts:369` | beta | active | Remove a named silence rule; requires the same current strict registry read as add. |
| `http:fleet.auth-ticket.mint` | `POST /api/auth-ticket` | `src/fleet/index.ts:686`, `src/fleet/auth-ticket.ts` | stable | active | Mint short-lived API/SSE ticket (root Bearer, or console session cookie + same-origin proof). Loopback TCP source only (C2). |
| `http:fleet.console-session.create` | `POST /api/console-session` | `src/fleet/index.ts:649` | beta | active | Console unlock: loopback source only (C2), validates the root token + same-origin proof, sets an HttpOnly `SameSite=Strict` session cookie (24h TTL; `Secure` when the unlock arrives over TLS); the browser never holds the root token |
| `http:fleet.console-session.delete` | `DELETE /api/console-session` | `src/fleet/index.ts:673` | beta | active | Console lock/logout: revokes the presented session and clears the cookie |
| `http:fleet.ws-ticket.mint` | `POST /api/ws-ticket` | `src/fleet/index.ts:713`, `src/fleet/ws-ticket.ts` | stable | active | Mint short-lived WebSocket ticket (root Bearer, or console session cookie + same-origin proof). Loopback TCP source only (C2). |
| `http:fleet.signals.ingest` | `POST /api/signals` | `src/fleet/index.ts:730`, `src/fleet/routes/signals.ts` | alpha | active | Byte-stable idempotent incident signal ingestion. Producer Bearer credential (pre-gate carve-out: fleet root/tickets do NOT authorize production). 201 receipt / 200 + `Idempotent-Replay: true` / 409 identity conflict; 32 KiB body cap; per-producer rate limit. |
| `http:fleet.producers.register` | `POST /api/producers` | `src/fleet/index.ts:734`, `src/fleet/routes/signals.ts` | alpha | active | Register an incident producer (root Bearer). Returns the single-use enrollment secret; the plaintext appears only in this response. |
| `http:fleet.producers.credential.mint` | `POST /api/producers/:id/credential` | `src/fleet/index.ts:739`, `src/fleet/routes/signals.ts` | alpha | active | Mint a producer credential: enrollment-secret exchange, or rotation with the current credential (24h overlap window). |
| `http:fleet.producers.credential.revoke` | `DELETE /api/producers/:id/credential` | `src/fleet/index.ts:739`, `src/fleet/routes/signals.ts` | alpha | active | Revoke a producer and its credentials (root Bearer). Returns 204. |
...
For agent runtimes, the normalized `turn_capability` projection also exposes
`last_successful_turn_provider` only when it is a recognized provider ID, and
`last_successful_turn_session_current` only when the successful turn belongs to
the exact still-live session incarnation. A `false` or `null` session-current
value is intentionally insufficient for provider-progress corroboration.

| Identifier | Method + Path | Source | Stability | Status | Notes |
|---|---|---|---|---|---|
| `http:health.status` | `GET /health` | `src/core/health.ts:1959`, `src/core/database-compatibility-early.ts:172` | stable | active | Normal-runtime liveness probe. Chat instances expose identifier-free, process-local queue-admission counters at `runtime.chat.queue_admission.rejected_total` and `unowned_total`; a positive unowned count degrades health, while terminalized capacity rejections remain observable without permanently degrading the process. During a startup database-compatibility drain, the inspection-only implementation returns `503` with `service_mode: "inspection_only"`, a content-free `startup_block`, database inspection metadata, and provider and synthetic admission blocked; it does not start WhatsApp, the agent runtime, or provider sessions. If a running Chat process detects compatibility loss inside its queue or a guarded background provider, it first returns normal-shape unhealthy health with bounded `runtime.chat.database_compatibility` fields (`reason`, `observed_migration`, `required_migration`), stops enrichment and memory consolidation, blocks later Chat admission, and exposes no paths, JIDs, or message content. The launchd health watchdog can perform one transition restart into startup classification. systemd `Restart=always` restarts on process exit but does not react to HTTP `503`; that surface requires a controlled operator restart. A schema newer than the binary is never `schema_ready`. The normal-runtime `instance` block includes nullable checkout metadata (`commit`, full 40-character git SHA when known; `branch`, current branch or `HEAD-detached` when known) plus, on agent instances, provider-fallback telemetry (`effectiveProvider`, `fallbackActiveUntil`, `fallbackReason`, `fallbackModel`, `fallbackResetAt`, `fallbackRecoveryProbeRequired`, `fallbackTurnsServed`, `fallbackTurnsEmpty`, `lastFallbackTurnAt`; counters are process-local, reset on restart) and `primaryModelUsability` (`status`, `provider`, `model`, `reason`, `suggestion`, `checkedAt`, `probeInFlight`) from the startup primary model usability probe. The top-level `turn_capability` block exposes `model_usable`, `model_usability_status`, `model_usable_stale`, `model_usable_checked_at`, `last_successful_turn_at`, `last_successful_turn_provider`, `last_successful_turn_model`, `last_successful_turn_session_current`, `primary_model`, `last_turn_error_class`, `last_turn_error_at`, `periodic_probe_expected`, `periodic_probe_backoff_multiple`, `model_usable_freshness_ms`, and `next_probe_due_at` (`model_usable_stale` flags a usability verdict whose probe is past its freshness window — treated as unusable so a stale "usable" reading cannot keep health green; `model_usable_checked_at` is the probe timestamp; while the periodic primary-readiness probe is armed the window follows the probe scheduler — interval × backoff multiple plus the full jitter band and a 2-minute grace, hard-ceilinged at the max-backoff cycle — and `model_usable_freshness_ms` reports the window the verdict was judged against; while a timer is armed `next_probe_due_at` is its due instant and the window is derived from it, so cadence and window cannot diverge and a flat 30-minute window can no longer flag evidence stale mid-cycle); agent health degrades when runtime health is degraded, the configured primary model is unusable or its usability probe is stale, or a user-turn error has no later successful user turn (an `empty-output` error degrades only as a current, sustained stall — past a short debounce and within a staleness bound — so a transient empty turn or a stale idle one self-clears; #1433). `runtime.agent` also exposes content-free queue-halt truth (`turnQueueHalted`, boolean; `turnQueueHaltedScopes`, count only): the active shared queue contributes one and makes health unhealthy/503, any number of latched per-chat scopes makes health degraded/200 until process restart, and single mode reports false/0 because its admission path does not use the TurnQueue. It also exposes process-local `proactiveResumeIdentityRejects`, provider-state execution telemetry (`providerExecution.active`, `pending`, `oldestWaitMs`, `totalWaits`, `maxPending`, `lastWaitMs`, `abortedWaits`, `pressureActive`), live `turnFinalizationRetainedRetries` / `turnFinalizationDegradedScopes` gauges, cumulative `turnFinalizationRetryAttempts` / `turnFinalizationRetryRecoveries` / `turnFinalizationRetryExhaustions`, and durable-recovery gauges `turnRecoveryOutstanding`, `turnRecoveryPending`, `turnRecoveryLiveClaimed`, `turnRecoveryExpiredClaimed`, `turnRecoveryBlockedUnsafe`, `turnRecoveryExhausted`, `turnRecoveryOpenRecoveries`, `turnRecoveryQuarantinedDelivery`, `turnRecoveryCorruptLinks`, `turnRecoveryOrphanTransfers`, and `turnRecoveryEchoConflicts`, plus the informational actionability split of `turnRecoveryBlockedUnsafe` into `turnRecoveryBlockedUnsafeSynthetic` (synthetic self-turns, which owe no user a reply), `turnRecoveryBlockedUnsafeSuperseded` (real-source jobs whose conversation has newer inbound), and `turnRecoveryBlockedUnsafeStranded` (real-source jobs with no newer inbound — the only class that may still owe a user a reply); the three always sum to `turnRecoveryBlockedUnsafe` and do not change health status by themselves. The top-level `event_loop` block exposes `lag_p95_ms`, `sample_count`, `locally_starved`, `starvation_threshold_ms`, `discontinuity_count`, window extremes (`lag_min_ms`, `lag_median_ms`, `lag_max_ms`), the window's observation-source split (`interval_sample_count`, `snapshot_sample_count`), the latest inter-observation `elu_utilization` and `cpu_delta_ms`, `observer_cost_ms` (health-handler time inside the retained window that has ALREADY been subtracted from the lag figures above — a consumer that subtracts it again double-counts; it is a window total against per-sample statistics, so it does not reconstruct a raw per-sample delay), and `raw_samples` discovery metadata (`available`, schema, path, oldest and latest sequence); raw observations are never embedded in this general-purpose payload. Lag math uses only monotonic time; wall-clock adjustments cannot create false lag. Deadlines remain on the original interval phase and advance past missed slots, preventing ordinary callback jitter from being amplified into a false one-interval stall. Starvation warnings are emitted at health-request time and can lag the causal delayed callback by up to the retained window, so causal correlation must read the dedicated authenticated `/health/event-loop-samples` cursor endpoint, never warning timestamps (#3253). `discontinuity_count` is a saturating process-local counter, reset only by process restart, for monotonic scheduling gaps strictly greater than the retained 10-second window; exactly 10 seconds remains a normal sample, a discontinuity clears the retained window, and timer-first or snapshot-first observation consumes the same physical gap once. Authenticated responses expose aggregate-only `outbound_sends`, `tool_durability`, and `retention.database` evidence. `whatsapp.auth_bond.status` is one of `present`, `missing`, `invalid`, or `unknown`; `unknown` means no current tree evidence exists (no walk has completed, or the last one is stale past its risk bound) and is reported as an auth-bond risk rather than as a healthy tree — it is never produced by the live inspection path. The `whatsapp.auth_bond` block's tree digest (`tree_hash`, `file_count`, `total_bytes`) is computed off the request path and carries its own provenance: `digest_source` (`live` when walked inline, `cached` when served from a completed walk, `stale` when an event invalidated it and the refresh has not landed, `absent` when no walk has ever completed), `digest_age_ms`, `digest_refresh_in_flight`, and `digest_refresh_count`, plus `digest_refresh_outcome` (`fresh`, `incomplete`, `failed`, `superseded`, `none`, or `live`), `digest_refresh_reason`, `digest_refresh_scheduled` with `digest_next_refresh_eligible_ms` (a walk is queued but has not started — reported separately because `digest_refresh_outcome` describes the last COMPLETED attempt and would otherwise read as "nothing is happening"), and `digest_refresh_attempts` (walks started, including ones that did not publish; `digest_refresh_count` counts only publications, so the pair separates cost from progress). The same block carries three transient-read fields: `transient_read_persistent` (one transient credential read has persisted past the 120-second stale-risk bound), `transient_read_reason` (the single auth-bond issue that streak belongs to, or `null`), and `transient_read_age_ms` (how long that streak has run, or `null` when none is open). They exist so an escalated `auth_failure_class` can be reconciled with an issue list whose entries mean "not right now"; the streak is keyed by the reason, so a different transient reason starts a new streak and the reported reason and age always describe the same fault. All three are process-local and reset on restart, so a small age on a long-running fault means the process is young rather than the fault. Past that bound `whatsapp.connection.auth_failure_class` reports `auth_bond_read_persistent`, which is NON-TERMINAL and degrades at HTTP 200: it says the credential could not be READ, never that it is corrupt, so it is absent from every terminal set and the watchdog stays free to attempt the restart that may clear the read fault. No fleet outage record is opened by it — the mode-bucket modules that would decide one have no runtime importer, and incident opening and closing remain unimplemented follow-up work. Those three refresh fields describe the cached refresh scheduler and nothing else: the live inspection path carries no provenance, so whenever `digest_source` and `digest_refresh_outcome` are `live`, `digest_refresh_scheduled` is `false` and `digest_next_refresh_eligible_ms` and `digest_refresh_attempts` are `null` — the absence of a scheduler, not an idle one. On the cached path `digest_next_refresh_eligible_ms` reports only a QUEUED successor's due time, so it is `null` while reads are merely rate-limited by the refresh floor. An `incomplete` or `failed` refresh deliberately keeps the previous digest and lets it go on ageing, so a digest that stops advancing is how a persistent walk fault surfaces, and every such refresh queues its own retry under a doubling back-off rather than waiting for the next reader. The 120-second stale risk bound is a CLASSIFICATION bound, not a convergence bound: it guarantees only that on the first read at or after 120 seconds from the last successful observation the status reports `unknown` rather than a stale `present`. It does not promise that a refresh lands within 120 seconds — the retry schedule reaches roughly 150 seconds plus walk time before a fifth attempt, and `digest_refresh_attempts` climbing while `digest_refresh_count` stands still is how that is read off the surface. `digest_source` is the field that distinguishes null tree fields caused by an unfinished walk from null tree fields caused by an unreadable tree; an `absent` or long-stale digest is reported as an auth-bond risk rather than as a clean tree, so an unknown tree never reads as a healthy one. An unreadable durability ledger degrades with `durability_evidence_unreadable`; a failed retention lifecycle degrades with `database_retention_failed` until a later success. `providerExecution.pressureActive` degrades runtime health after a 30-second wait. `turnRecoveryOutstanding` counts admission-active pending/claimed work plus orphan transfers; blocked-unsafe receipts remain visible but are informational by themselves. Retained finalization, outstanding or exhausted recovery, an open operator catch-up, corrupt proof, or an echo conflict degrades agent health. Normal-runtime responses expose additive top-level `status_reasons`, an ordered array of bounded reason codes supporting the aggregate status; unknown or mixed degradation remains fail-closed. A recently-degraded instance whose live signals go quiet keeps `degraded` with the `degradation_silence_unproven` reason/cause pair (the silence latch) until an exact-primary-route recovery proof releases it: a successful turn on the same provider AND model as the primary route (`last_successful_turn_model` vs `primary_model`; a provider-default primary requires an absent receipt model), from the still-current session (`last_successful_turn_session_current` exactly true), strictly newer than the latch point, with no live fallback window and every latched reason turn-provable. The latch is process-local state — a restart clears it by amnesia, not proof — and Chat instances (no turn-capability evidence) have no release channel; see docs/runbook.md "Degradation silence latch". On claude-cli agent instances `runtime.agent.accountIdentity` exposes the ratified account-identity verdict (task-21, verify-only) as status classes only: `status` ∈ `disabled` (no `service.expectedAccountDigest`) / `pending` (armed, not yet verified) / `match` / `mismatch` / `unverifiable`, a bounded `reason` (`not-logged-in`, `identity-fields-missing`, `unparseable`, `binary-missing`, `probe-failed`, `probe-threw`, `expectation-malformed`, `stale-receipt`, `never-verified`), `stale`, `checkedAt`, and 12-hex `expectedDigestPrefix` / `observedDigestPrefix` — never a raw account identifier. The verdict is refreshed on startup and on every primary-usability probe (no separate poller) and judged against the same freshness window as `model_usable`; a match older than that window is `unverifiable`/`stale-receipt`, never a stale green. `mismatch` and `unverifiable` degrade with the reason/cause pairs `runtime.credential_identity_mismatch` / `credential_identity_mismatch` and `runtime.credential_identity_unverifiable` / `credential_identity_unverifiable` and page the operator with alert sources of the same names; the reasons are deliberately NOT turn-provable (a turn proves the credential works, not whose it is), so an identity reason caught by the silence latch persists until the instance restarts after the identity is corrected. The runtime never writes, mirrors, or seeds a credential from this check. `runtime.agent` also exposes content-free per-chat dispatch-ownership state: `perChatSessionsWithoutOwner` and `perChatRespawnAbandoned` are counts only, while `agentRespawnFailedClearPending` is a boolean retry obligation that degrades under the `runtime.agent_respawn_failed_clear_pending` / `agent_respawn_failed_clear_pending` reason/cause pair; none exposes chat identities. A positive `perChatSessionsWithoutOwner` degrades with the reason/cause pair `runtime.per_chat_session_without_owner` / `per_chat_session_without_owner`: a mapped chat whose dispatch ownership record is missing or names another manager, which rejects every inbound turn in that chat until the dispatch path evicts and rebuilds it. A positive `perChatRespawnAbandoned` degrades with `runtime.per_chat_respawn_abandoned` / `per_chat_respawn_abandoned`: auto-respawn stopped re-arming because provider termination was never proved, so the chat waits for the user's next message; it also pages the operator on the `agent_respawn_failed` alert source, and it settles as soon as the chat serves again — its next inbound message either respawns the session in place or is served by the session that never went inactive — or when a new owned session is indexed for it, or when the record ages out after an hour; the alert source is shared with crash exhaustion and clearing is attempted only once no chat is either exhausted or abandoned. A refused or throwing clear retains `agentRespawnFailedClearPending`; later health polls and provider-ready boundaries retry while both owner populations remain empty, and an accepted clear retires the obligation without duplicate clears. None of these three reasons arms the silence latch, because each disappears through its own repair path rather than needing a latch to remain visible. The two ownership conditions clear by different mechanisms: `perChatSessionsWithoutOwner` is recomputed from live state on every poll, while `perChatRespawnAbandoned` is a retention record settled when the chat serves again by either route, or on age. |
| `http:health.event-loop-samples` | `GET /health/event-loop-samples` | `src/core/health.ts:1927`, `src/core/loop-lag-samples-endpoint.ts` | beta | active | Privileged, content-free raw loop-lag cursor page. Requires the same server-scoped Bearer token as diagnostic health; missing/invalid auth returns `401` with no public fallback. Accepts only optional non-negative safe-integer `after` and `limit=1..160`; unknown, repeated, or malformed parameters return bounded `400` codes. Responses use `health.event-loop-samples.v1`, remain below 32 KiB, and report process identity, cursor bounds, truncation, and explicit cursor-eviction gaps. |
| `http:health.send` | `POST /send` | `src/core/health.ts:1391` | stable | active | Send a text message |
| `http:health.schedule` | `POST /schedule` | `src/core/health.ts:1460` | experimental | active | Enqueue a text/media message into `scheduled_messages` for durable delivery by the always-on MessageScheduler (≤~60 s). Media passed by file path bounded to `WHATSOUP_SCHEDULE_ROOT` (fail-closed `409` when unset); omitted `scheduledAt` lands on the next tick (`now + 2s`). Non-MCP enqueue path for the launchd loop-daily (5E-0). |
| `http:health.access` | `POST /access` | `src/core/health.ts:1733` | stable | active | Allow / block contact or group |
| `http:health.mark-read` | `POST /mark-read` | `src/core/health.ts:1841` | stable | active | Zero unread + chatModify |
| `http:health.typing` | `GET /typing` | `src/core/health.ts:1911` | stable | active | Currently-composing JIDs from presence cache |
| `http:health.heal` | `POST /heal` | `src/core/health.ts:1634` | stable | active | Inject Type-3 repair report |
| `http:health.agent-compact` | `POST /agent/compact` | `src/core/health.ts:1545` | stable | active | Out-of-band compaction; requires `chatJid` for per-chat / shared scopes |
...
| `config:instance_config.agentOptions.turnRecoveryCatchupReconcile` | config-key | [docs/configuration.md §agentOptions](configuration.md#agentoptions), [enabling guide](turn-recovery-continuity-reconciler.md#enabling-per-instance) | v1 (bootstrap) | beta | active | Default-off gate for the turn-recovery supervisor's automatic operator catch-up reconciler: `{ enabled: boolean, groupLimit?: 1–1000 }`, closed shape validated at load by `src/core/turn-recovery-catchup-config.ts`. Addition: [2026-09-25 release note](releases/2026-09-25-turn-recovery-catchup-reconcile-gate.md#public-surface-additions) |
...
| `config:instance_config.service` | config-key | [docs/configuration.md §service](configuration.md#service-launchd-render-options) | v1 (bootstrap) | beta | active | Service-context block: launchd render options `claudeConfigDir` / `pathPrepend` (macOS only) and the ratified account identity `expectedAccountDigest` (claude-cli agent instances, every platform; opaque `sha256:` digest, never a raw identifier — see [§Ratified account identity](configuration.md#ratified-account-identity-serviceexpectedaccountdigest)) |
...
| `cli:npm.setup-host-dependencies` | `npm run setup:host-dependencies` | `package.json` | stable | active | Plan-only by default; explicitly installs runtime, quality, or release host dependencies after confirmation and verifies exact or compatibility-only Node policy with the matching doctor profile |
...
| `cli:npm.loop-lag-samples` | `npm run loop-lag-samples -- collect …` | `package.json`, `scripts/collect-loop-lag-samples.ts` | beta | active | Loopback-only authenticated raw loop-lag collector. Appends a private bounded JSONL artifact; token values are accepted only through an absolute private token file. Use `npm --silent run loop-lag-samples:schema` when stdout must be pure JSON. |
| `cli:npm.loop-lag-samples-schema` | `npm run loop-lag-samples:schema` | `package.json`, `scripts/collect-loop-lag-samples.ts` | beta | active | Offline machine-readable discovery for collector inputs, effects, record versions, interval limits, and exit codes. Invoke through `npm --silent run` for banner-free JSON. |
...
| `cli:npm.record-continuity-manifest` | `npm run record-continuity-manifest` | `package.json` | beta | active | Explicitly confirmed, idempotent persistence of fingerprint-only missing/ambiguous continuity evidence; does not send, replay, admit, or close messages |
| `cli:npm.prepare-recovered-audio` | `npm run prepare-recovered-audio` | `package.json` | beta | active | Operator-selected transcription of recovered history voice notes by exact message ID: preview by default, `--apply` with a named local provider, bounded items/audio/bytes/wall time, compare-and-set writes and a hash manifest; exits non-zero unless every selected item is ready or excluded; never sends or replays |
...
| `cli:npm.turn-recovery-operator` | `npm run turn-recovery-operator` | `package.json` | beta | active | Redacted operator workflow for blocked-unsafe turn-recovery jobs (#2155): subcommands list/show/reassign/promote against `--db <instance dbPath>`; content-free paginated listing, fenced reassignment, and evidence-gated promotion through the existing durability transitions; mutations default to dry-run, one job per invocation, durable content-free audit receipts |
| `cli:npm.fleet-silence-reset` | `npm run fleet-silence-reset -- [--confirm-reset sha256:<revision>]` | `package.json` | beta | active | JSON-only local repair. The default is a genuinely read-only invalid-registry inspection; an exact confirmation preserves a private quarantine copy, publishes only if the target is absent, and verifies the empty replacement without exposing registry bytes. |
| `cli:npm.validate-private-operation-record` | `npm --silent run validate-private-operation-record` | `package.json` | beta | active | Read-only schema-v1 host-operation receipt validation; `schema` publishes closed registries and command schemas, while `validate --record <absolute-path> --format json` exits 0 valid / 1 actionable / 2 read failure with content-free JSON errors |
...
| `cli:npm.triage-priority-clusters` | `npm run triage:priority-clusters` | `package.json` | beta | active | Offline generation, validation, and deterministic rendering of the sealed numeric-only P0/P1 cluster projection |
...
| `cli:npm.reconcile-launchd-restart-policy` | `npm run reconcile-launchd-restart-policy` | `package.json` | beta | active | Dry-run by default; `--apply` validates a generated plist identity, then atomically reloads and starts/restarts one named macOS instance, failing closed when bootout, reload, or rollback fails. Both modes resolve the instance `service` render options (fail-closed on invalid config) and report governed-env drift (`CLAUDE_CONFIG_DIR`, `PATH`, `WHATSOUP_PATH_PREPEND`) by key and SHA-256 value digest only — never values; `PATH` is reported as configured-prefix-satisfied versus ambient-tail-differs, installed non-governed keys an apply would drop are listed by name, and `--apply` refuses to drop them unless `--drop-non-governed-env` is passed |
| `cli:npm.claude-account-digest` | `npm run claude-account-digest` | `package.json` | beta | active | Read-only capture of the ratified claude-cli account-identity digest for `service.expectedAccountDigest`: runs the runtime's own `claude auth status --json` probe in the invoking shell's `CLAUDE_CONFIG_DIR` context and prints exactly one opaque `sha256:` line on stdout — never a raw account identifier. **Run it as `npm run --silent claude-account-digest`**: the script itself emits only the digest, but `npm run` without `--silent` prefixes a four-line banner on stdout, so a command-substitution capture yields an unusable digest. Wrapper diagnostics go to stderr. Exit 2 not logged in, 3 identity unreadable, 4 binary/probe failure. Never logs in. |
...
| `cli:npm.provider-mcp-canary` | `npm run provider:mcp-canary -- --provider <id> --state-root <path>` | `package.json` | beta | active | Run an opt-in, no-model/no-WhatsApp provider MCP transport proof under an owned process group and write one private artifact-bound receipt; missing or unsafe proof fails closed |
...
| `cli:npm.report-shadow-gate` | `npm run report:shadow-gate` | `package.json`, `scripts/shadow-gate-report.ts` | beta | active | Read-only shadow-gate measurement over an immutable DB snapshot plus a copy of the NDJSON segment directory; prints coverage/missingness first, then conservative disagreement rates with Clopper–Pearson bounds; never prints message text, JIDs or phone numbers; exits 0 report, 64 usage, 65 invalid evidence |
| `cli:npm.forensic-reconstruct` | `npm run forensic:reconstruct` | `package.json`, `scripts/forensic-reconstruction.ts` | beta | active | Bounded, hash-pinned reconstruction CLI for three harness families; scans captured JSONL or database inputs, builds deterministic sanitized evidence packages, and verifies exact package manifests; exits 0 complete/valid or 2 inconclusive/input/tool failure |
...
| `cli:npm.release-export` | `npm run release:export` | `package.json` | beta | active | Exact-commit release export executor: stages from `git archive`, self-verifies with the standard drift checker, publishes atomically with no-clobber/rollback preservation; service repoint and restart remain separately approved host mutations |
| `cli:npm.resolve-deployment-config` | `npm run resolve:deployment-config` | `package.json`, `scripts/resolve-deployment-effective-config.ts` | beta | active | Resolve validated private deployment inputs into a bound effective-configuration record; no deployment acceptance, service activation, or live configuration change |
...
| `cli:npm.guard-pr-metadata` | `npm run guard:pr-metadata` | `package.json` | stable | active | Validate a PR body from stdin, a local file, or the authoritative pull-request event against explicit marker-delimited closing directives and the exact candidate commit range; exits 0 pass/warn, 1 policy violation, 2 incomplete input; receipts contain only bounded codes and PR/issue numbers |
...
| `cli:npm.guard-hooks-installed` | `npm run guard:hooks-installed` | `package.json` | internal | active | Verify the installed Git hooks are byte-identical to the checked-out objects: resolves the configured hooks path, refuses hooks that resolve outside this worktree, and compares each hook's observed blob OID and mode against the expected tree entry. Emits a canonical receipt with the git-executable identity digest. Exits 0 OK, 1 BLOCK, 2 INCONCLUSIVE |
| `cli:npm.ci-classify` | `npm run ci:classify` | `package.json` | internal | active | Classify a push candidate's exact-revision risk from git-object evidence: resolves the exact change facts, binds them to the control manifest digest and a bounded tool-source inventory (substitution of any inventory file changes the identity), and emits a canonical risk-classification receipt. Exits 0 OK, 1 BLOCK, 2 INCONCLUSIVE |
| `cli:npm.ci-manifest` | `npm run ci:manifest` | `package.json` | internal | active | Read and validate the canonical control manifest (`controls/ci-control-manifest.json`): `validate` checks schema, control dependencies, and stage/trust-class references; `digest` emits the manifest digest the exact-revision classifier binds to. Exits 0 OK, 1 BLOCK, 2 INCONCLUSIVE |
| `cli:npm.ci-ref-policy` | `npm run ci:ref-policy` | `package.json` | internal | active | Report-only outgoing-ref policy evaluation for a push candidate: resolves the native ref graph, refuses grafted or replaced object lineage, and preserves the originating failure cause rather than collapsing it. Exits 0 OK, 1 BLOCK, 2 INCONCLUSIVE |
...
| `cli:npm.guard-observation-contract` | `npm run guard:observation-contract` | `package.json` | internal | active | Validate the observation-plane contract data set: envelope schema, claim catalog, outcome projections, authority lattice, adapter registry, and fixtures |
...
| `cli:npm.guard-fault-taxonomy-coverage` | `npm run guard:fault-taxonomy-coverage` | `package.json` | stable | active | Fail any emitted alert source that lacks a fault-taxonomy sourceDispositions entry and is absent from the checked-in debt baseline (growth), any stale debt entry (now registered or no longer emitted), and any runtime-computed emit site not claimed with file provenance; #2147 completes when the debt ledger is empty |
...
| `cli:npm.guard-platform-patterns` | `npm run guard:platform-patterns` | `package.json` | stable | active | Enforce cross-platform portability: block hardcoded platform binaries, unguarded /proc///sys///run/ access, systemctl without launchctl fallback, and GNU-only shell flags; 6 mechanical rule classes with 73 baselined violations |
...
| `cli:npm.guard-resolved-override` | `npm run guard:resolved-override` | `package.json` | internal | active | Fail the build on an `ExecutingSessionContext.resolved` override outside the count-pinned test-only inventory (#3435); the override flips an all-undefined executing context to `'resolved'` and bypasses the fail-closed branch #3438 added, so a production caller reopens the empty-context fail-open; a zero-file scan, an unreadable candidate, or an allowlisted site that stops matching is INCONCLUSIVE (exit 2), never a pass |
...
| `cli:npm.guard-hardcoded-tmpdir` | `npm run guard:hardcoded-tmpdir` | `package.json` | stable | active | Reject new hardcoded `/tmp/` path literals in src/scripts, grandfathered via `.claude/fitness/tmpdir-baseline.json` (Linux-only path breaks on macOS/NixOS; use `os.tmpdir()`) |
...
| `cli:npm.preflight-instructions-path` | `npm run preflight:instructions-path` | `package.json` | alpha | active | Fail-closed instance agent-instructions path validation (salvaged fleet-runtime guard; blocks on blank/home-escaping/missing/non-regular/unreadable instruction paths) |
...
| `cli:npm.guard-guard-test-coverage` | `npm run guard:guard-test-coverage` | `package.json` | stable | active | AST-backed meta-guard: every guard-family script (`scripts/*guard*.ts`, `scripts/check-*.ts`) must ship a companion test wired into `verify:push:branch` with a real failure-path proof, or carry a `// meta-guard:no-test <reason>` opt-out. Supports explicit semantic mode and output format options plus bounded `--verbose` diagnostics; exits 0 for pass/advisory, 1 for actionable findings, and 2 for inconclusive evidence. Branch and release gates invoke semantic enforcement explicitly. |
...
| `deploy:wrapper.whatsoup-reply-guarantee-drain` | `whatsoup-reply-guarantee-drain` launcher | [`deploy/scripts/reply-guarantee-drain.sh`](../deploy/scripts/reply-guarantee-drain.sh) | stable | active | Repo-relative scheduler entrypoint for draining queued stuck replies and independently observing WAL-backed durable reply obligations |
...
| `deploy:systemd.reply-guarantee` | `whatsoup-reply-guarantee.{service,timer}` | [`deploy/whatsoup-reply-guarantee.service`](../deploy/whatsoup-reply-guarantee.service), [`deploy/whatsoup-reply-guarantee.timer`](../deploy/whatsoup-reply-guarantee.timer) | stable | active | systemd timer for queue drain plus separate active-breach and recovery-debt observation |
...
| `deploy:launchd.generated` | macOS plist generation behavior | [docs/runbooks/macos-launchd-deployment.md](runbooks/macos-launchd-deployment.md), [src/fleet/platform.ts](../src/fleet/platform.ts) | stable | active | Generated launchd plists, rendered from typed per-instance render options (config.json `service` block → `CLAUDE_CONFIG_DIR`, service `PATH` prepend; omitted options render byte-identical output); per §4, regen non-destructively is non-breaking |
| `deploy:launchd.timers` | `com.whatsoup.{harness-maintenance,reply-guarantee,release-drift-check}.plist` | [`deploy/com.whatsoup.harness-maintenance.plist`](../deploy/com.whatsoup.harness-maintenance.plist), [`deploy/com.whatsoup.reply-guarantee.plist`](../deploy/com.whatsoup.reply-guarantee.plist), [`deploy/com.whatsoup.release-drift-check.plist`](../deploy/com.whatsoup.release-drift-check.plist) | stable | active | launchd timer templates for maintenance, reply durability, and independent release-integrity / advisory-currency observations; rendered into `~/Library/LaunchAgents` by operator action on macOS, never auto-loaded (`RunAtLoad=false`; loading is a deployment step) |
...
| `artifact:fleet.silences` | `<configRoot>/fleet-silences.json` | [`src/fleet/silence-manager.ts`](../src/fleet/silence-manager.ts) | beta | active | Operator-managed alert silence rules; mode `0600`. A missing registry is explicitly uninitialized; unreadable, invalid, or non-regular state is unavailable and ordinary mutations are refused. `cli:npm.fleet-silence-reset` is the only explicit corrupt-registry reset path. |
| `artifact:fleet.silence-generation` | `<configRoot>/fleet-silence-registry-state.json` | [`src/fleet/silence-manager.ts`](../src/fleet/silence-manager.ts) | beta | active | Private metadata-only lifecycle marker (mode `0600`) used to distinguish durable first-run absence from a populated registry disappearing after restart; contains no rule values. |
| `artifact:fleet.silence-quarantine` | `<configRoot>/fleet-silence-quarantine/` | [`src/fleet/silence-manager.ts`](../src/fleet/silence-manager.ts) | beta | active | Private `0700` directory holding the original bytes preserved by an explicitly confirmed corrupt-registry reset; it is never returned by the API or CLI. |
| `artifact:fleet.silence-repair-receipts` | `<configRoot>/fleet-silence-repair-receipts.jsonl` | [`src/fleet/silence-manager.ts`](../src/fleet/silence-manager.ts) | beta | active | Private metadata-only repair receipt ledger (mode `0600`); records revisions and state transitions, never registry bytes or paths. |
| `artifact:fleet.silence-registry-episode` | `$XDG_STATE_HOME/whatsoup/fleet/silence-registry-episode.json` | [`src/fleet/silence-registry-episode-store.ts`](../src/fleet/silence-registry-episode-store.ts) | beta | active | Private restart-safe lifecycle state for the one registry-unavailable alert episode. State advances only after local durable queueing; it does not claim exactly-once remote delivery. |
| `artifact:fleet.silence-registry-episode-failover` | `$XDG_STATE_HOME/whatsoup/fleet/silence-registry-episode-failover.json` | [`src/fleet/silence-registry-episode-store.ts`](../src/fleet/silence-registry-episode-store.ts) | beta | active | Private sticky lifecycle ledger used only when the primary episode journal is unreadable. It preserves the primary untouched and remains authoritative even if that primary later appears readable; it is never automatically reconciled. |
...
| `artifact:loop-lag-evidence` | Operator-selected absolute private JSONL path | [`scripts/collect-loop-lag-samples.ts`](../scripts/collect-loop-lag-samples.ts), [collector runbook](runbooks/loop-lag-forensic-collector.md) | beta | active | Versioned content-free sample/gap/run records; parent `0700`, file `0600`, whole-record bounded retention (50 MiB default). The path is never inferred from instance state. |
...
| `console:settings.silences` | Settings alert-mute controls | [`console/src/components/settings/SettingsSections.tsx`](../console/src/components/settings/SettingsSections.tsx) | beta | active | Shows unavailable, stale, uninitialized, and empty registry states separately; add/remove controls stay disabled until a fresh current read. |
