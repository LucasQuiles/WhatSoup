...
| `cli:npm.guard-platform-patterns` | `npm run guard:platform-patterns` | `package.json` | stable | active | Enforce cross-platform portability: block hardcoded platform binaries, Linux-path access without a dominating non-Linux return, systemctl without launchctl fallback, and GNU-only shell flags; 6 mechanical rule classes with 66 baseline records |
...
| `cli:npm.guard-process-tree-diagnostics` | `npm run guard:process-tree-diagnostics` | `package.json` | internal | active | Fail closed when a production process-tree termination caller lacks retained root authority, a canonical diagnostic source, an outcome observer, or bounded cgroup-divergence telemetry; the lexical AST inventory covers aliases without confusing block/function/switch shadows, namespace imports, bounded-static awaited dynamic imports (unresolved results must be discarded), resolver-normalized local barrels, bound/native invocation, static computed and rest destructuring, options-spread shifts, and unsupported canonical-function, TypeScript-namespace/import-equals, or container exports/value escapes |
