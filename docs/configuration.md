...
| 45 | Adds `outbound_op_message_ids` — every historical `wa_message_id` per outbound op (PK `(op_id, wa_message_id)`, index on `wa_message_id`, backfilled from existing non-NULL `outbound_ops.wa_message_id` rows) so echo reconciliation survives resends — plus `idx_outbound_ops_chat_hash_status` on `(chat_jid, payload_hash, status)` for the M1 durable duplicate-suppression check; no-op if `outbound_ops` is absent (`src/core/database-migration-45.ts`) |
