...
| `WHATSOUP_REPO_ROOT` | path | `process.cwd()` | Repository root scanned for the ARC binding file reported under the `arc` key of `GET /health`. A trimmed, non-empty explicit value takes precedence over the process working directory. If that explicit root is missing or invalid, health reports `{loaded:false,reason}` and does not silently search the working directory. Generated launchd jobs instead pin `WorkingDirectory` to their reviewed checkout, so the variable is normally unnecessary there. |
