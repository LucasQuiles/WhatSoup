...
| 45 | Adds `recovery_runs.status` (`'running'`/`'completed'`/`'failed'`, `NOT NULL DEFAULT 'running'`, CHECK-enforced) plus nullable `error_kind`/`error_message` (idempotent ALTERs, no-op if `recovery_runs` is absent). Backfills every pre-existing row from `completed_at` (`NOT NULL` → `'completed'`, `NULL` → `'failed'`, since a null `completed_at` at migration time is always durable recovery debt, never an in-progress run). Before this migration, a genuinely failed run and one interrupted before even its incomplete receipt could be written were both indistinguishable `completed_at IS NULL` rows (#1786; see `docs/durability.md` §5.3) (`runMigration45`). |
