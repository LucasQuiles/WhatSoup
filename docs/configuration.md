...
| `BOT_ERRORS_CONVERSATION_SCOPE_RETENTION_SECONDS` | positive integer | `604800` (7 days) | How long a conversation stays recorded as represented. Past the window that conversation can force a notification again. Enforced on the state save lifecycle, not only when the key next sees traffic, so a quiet instance cannot retain digests indefinitely. |
...
| `BOT_ERRORS_CONVERSATION_SCOPE_MAX_KEYS` | positive integer | `128` | Incident keys carrying a conversation-scope sidecar at once. Bounds the state file against a long tail of historical keys. |
