...
| `WHATSOUP_SYSTEM_TURN_TIMEOUT_MS` | integer (ms) | `240000` (4m, matches the auto-compact timeout) | Absolute wall bound for every non-auto provider request the runtime owns (`src/runtimes/agent/runtime-tunables.ts` `SYSTEM_TURN_TIMEOUT_MS`). The previously hardcoded bound was implicated in group-turn failures where an injected context full of unanswered asks provokes a long, effect-suppressed system turn that blows the timeout. Parsed as a positive integer (invalid/≤0 → default). Values well above the default extend how long a wedged system turn can hold its lane, so tune with care; keep it below the silent-compact TTL (`auto-compact-controller.ts`) so a silent `/compact` cannot expire mid-compaction. |
...
| `EMIT_ALERT_THROTTLE_MS` | number (ms) | `300000` (5 min) | In-process dedup window for the legacy alert helper (the spawn-based fallback path used when the durable outbox write fails). Alerts with the same `(instance, source, summary)` key within the window are suppressed. Set to `0` to disable; negatives clamp to `0`; non-finite values use the default. Resolved at call time by a single validated getter in `src/lib/emit-alert.ts` (no longer frozen at import). Fronted by the typed `emitAlertThrottleMs` instance-config field (#2192): an instance-config value is published back to the env at config load so the call-time reader honors it. |
| `BOT_ERRORS_SAFE_SHAPE_CRED_PATH` | `1`/`true`/`yes`/`on` enables | off | Rollout flag for safe-shape credential-path redaction in the bot-errors outbox (`src/lib/bot-errors-outbox.ts`): when on, matched credential-bearing paths are collapsed to a `[REDACTED]`-suffixed known-safe prefix before the event is written. Fronted by the typed `botErrorsSafeShapeCredPath` instance-config field (#2192), published like the routing trio; env-only and unset deployments unchanged. |
| `BOT_ERRORS_RUNTIME_TOOL_FAILURE_ALERTS` | `0` disables | enabled | Master switch for the agent runtime's per-tool-failure operator alerts. When exactly `0`, `maybeEmitToolFailureAlert` returns early and no `runtime_tool_failure` alert is emitted regardless of the failure. Any other value (including unset) leaves the alerts enabled, still subject to the downstream noise gate that pages only on infra/provider-health signatures. Fronted by the typed `toolFailureAlertsEnabled` instance-config field (#2192): config.ts resolves it and the runtime threads the boolean through `ToolFailureAlertDeps` — `src/runtimes/agent/tool-failure-alert.ts` itself reads no env. |
...
| `shadowGate` | object | `{ mode: 'off', eventsDir: null }` | Logged-only reply-worthiness gate (`src/core/shadow-gate-adapter.ts`). `mode` is `off` or `shadow`; there is no enforcing mode. In `shadow`, each message that passes the access policy and reaches dispatch gets an advisory SPAWN/SUPPRESS verdict appended as NDJSON (`shadow-gate-events.*.ndjson`, metadata and closed codes only — no message text; ids that look like a JID or a standalone phone number are rejected) under `eventsDir`, default `~/.config/whatsoup/instances/<name>/`. Each record carries the recorded instance id (`botName` with characters outside `A-Za-z0-9._:-` replaced by `_`), a database-lineage hash, a boot id, the message id and inbound seq, the verdict, rule id, status/reason code, evaluation time and the gate/rules/feature versions; `armed` and 10-minute `counts` coverage markers carry recorder counters. Dispatch is never changed. Both fields are read once at startup, so a change takes effect only after a restart. Measure with `npm run report:shadow-gate` — see [runbook §Shadow Gate](runbook.md#shadow-gate). |
...
**Policy path is explicit, not discovered.** `writeSandboxArtifacts` wires the `PreToolUse` hook with the absolute path of the policy it writes (passed as an argument), so the hook enforces exactly that file regardless of the agent's working directory — a mid-turn `cd` into a subdirectory can neither miss the policy nor pick up a `.claude/sandbox-policy.json` an agent plants in a writable subdirectory. A hook wired with no path argument falls back to a cwd-relative `.claude/sandbox-policy.json`; if that is absent, the missing-policy behaviour below applies.
...
- **Fail-closed:** an empty/corrupt/unreadable policy denies all egress; a proxy that cannot start aborts instance start rather than running unconfined. `WHATSOUP_SANDBOX_FAIL_OPEN=1` is the single documented escape hatch (relaxes both not-on-allowlist and unreadable-policy denials, logged). The in-process proxy channel is fronted by the typed `sandboxFailOpen` instance-config field (#2192, env fallback preserved); the shell-hook channel reads the env var directly and is unchanged — one env knob, two independently documented consumers.
...
| 54 | Adds `completed_delivery_identity_admissions`, a content-free, single-automatic-attempt admission ledger for missing, invalid, or scope-mismatched completed-delivery identity. It backfills active, suspended, and orphaned legacy checkpoints with incomplete proof, excludes unresolved targets from every resume selector, and records whether fresh inbound work or an operator owns repair (`src/core/database-migration-54.ts`). |
| 55 | Adds typed online enrichment-cycle receipts to `enrichment_runs`: bounded status/cause/stage/retryability/evidence fields, separate last-success timestamp, non-negative aggregate counters, source-scoped receipt-reader indexes, and legacy-unclassified defaults for prior rows (`src/core/database-migration-55.ts`). |
| 56 | Rebuilds `inbound_events` with a `CHECK` constraint closing `processing_status` to the canonical five-state `InboundStatus` union (`pending`, `processing`, `turn_done`, `complete`, `failed`), so malformed values can no longer land silently. A fail-closed pre-flight scan aborts before mutation if any existing row is out of union; dependent triggers and views are captured and recreated verbatim around the table swap (`src/core/database-migration-56.ts`, #2250). |
| 57 | `trigger_occurrences` table — durable pre-execution trigger-occurrence lifecycle: fenced lease (`lease_owner`, `lease_generation`, `lease_expires_at`), `UNIQUE(trigger_id, scheduled_for, attempt)` stable occurrence identity, and `stale` reconciliation state. Committed by the poller BEFORE the executor runs so a hang or crash leaves observable evidence; `trigger_runs` unchanged (`src/core/database-migration-57.ts`, #2566 slice 1). |
