...
| `BOT_ERRORS_CONVERSATION_SCOPE_MAX_KEYS` | positive integer | `128` | Incident keys carrying a conversation-scope sidecar at once. Bounds the state file against a long tail of historical keys. Evicting past this cap records a bounded top-level marker (`conversationScopesOverflow`: last eviction time and cumulative count, updated in place). The admission gate reads it the way it already reads the per-key overflow marker: once eviction has happened, a conversation with no record may be one that was evicted rather than one never seen, so it is treated as represented instead of forcing a fresh notification. Without that marker an evicted conversation would page again on its next rejection, re-add its key and evict another, paging forever above the cap. |
