...
**Current workflow/dependency reconciliation (2026-07-22):** merge commit
`c6b7540ad3fb969c544997b132975a0b97159fa2` preserves the evidence-bearing first parent
`62c0523e79b906239b276900adb41217f1e40036` and incorporates locally observed
`origin/main` `abe2572e5eaac315234b7fedc5975427acca180c` as its second parent without
rebasing. Its first-parent delta is exactly four paths, 268 insertions, and 151 deletions:
`quality.yml`, the quality-guardrails checklist, `package.json`, and `package-lock.json`.
The reviewed package resolution preserves every script key from both parents, the topic's
semantic receipt validation and control-plane test set, and current main's
`typecheck:scripts`, Sharp, and protobuf updates. Source inspection proves `quality.yml`
now handles `merge_group` without cancelling queue events and that all 14 direct external
action references are full-SHA pinned. These facts invalidate older workflow-gap reviews;
they do not close protected policy, dependency provenance, exact-set merge authority,
native portability, or hosted queue evidence. The preserved Task 7 candidate was reapplied
after this merge and later expanded through the admitted file-size prerequisite; the combined
...
- [ ] **Step 10 (CP-F2f): Add the versioned agent-recovery result and remediation closure contract**

**Frozen CP-F2f admission packet**

- Evidence: **Proven** at current source. `ControlResultV1` remains the current schema-1
  authorizing envelope and `ReasonDefinitionV2` remains the active taxonomy record. They
  already provide five leaf outcomes, three-state aggregation, evidence state, severity,
  exact bindings, preconditions, terminal attempts, causal graphs, patch limits, closed
  retry/remediation values, one renderer, exact-set aggregation, warning governance, and
  historical non-authorizing reads. They do not provide the rider's independently versioned
  taxonomy binding, aggregate effect, closed agent directive and authority, protected
  task/lease binding, structured recovery protocol, or remediation-closure receipt.
- Reachable failure: a schema-valid result can still force an agent to infer whether to stop,
  patch, reconcile, preserve, invalidate, reuse, rerun, request approval, quarantine, or
  escalate. The current `manual-recovery` value is also ambiguous, and no canonical receipt
  proves that remediation closed the original finding under the current policy.
- Unsafe fixtures: caller-selected authority or directive; stale lease generation; V1 receipt
  used for current aggregate authorization after cutover; V1 rewritten into V2; warning used
  as passing evidence; incomplete preserve/invalidate/reuse/rerun sets; unchanged deterministic
  retry; closure receipt bound to the wrong finding, candidate, policy, taxonomy, attempt, or
  reviewer evidence.
- Safe neighbors: exact V1 golden bytes remain readable as historical/report-only evidence;
  a regenerated V2 result matches the protected task/lease context and V3 reason defaults;
  a complete remediation receipt binds the original finding, corrected candidate, unsafe
  fixture, safe neighbor, focused and surrounding tests, review, and authoritative closure.
- Unavailable evidence: unknown schema/taxonomy version, absent protected authority/lease,
  incomplete recovery graph, missing terminal attempt, or unproven closure is
  `INCONCLUSIVE/2`, never a synthesized default or pass.
- Smallest repair: extend the existing result/reason owners and sole renderer; add one
  remediation-receipt module. Do not create a second message catalog, exception system,
  lease owner, execution engine, native-cause translator, or aggregate gate.
- Stop conditions: any changed V1 or V2 golden bytes, in-place `ReasonDefinitionV2` semantic
  change, caller-authored authority, alternate renderer, native-cause collapse, V1-to-V2
  evidence promotion, unbounded output, or plan/HEAD/lease drift.
- Rollback: stop V2 emission and authorization, retain V2 parsing as historical/report-only
  evidence, preserve every receipt, and ordinarily revert the additive source changes.
  Never reinterpret V2 bytes as V1.

**Files:**

- Modify: `scripts/lib/ci-control/result.ts`
- Modify: `scripts/lib/ci-control/reasons.ts`
- Create: `scripts/lib/ci-control/remediation.ts`
- Modify: `package.json` only to add the remediation companion test beside the existing
  reason/result tests in `verify:push:branch`
- Modify: `tests/scripts/ci-control-result.test.ts`
- Modify: `tests/scripts/ci-control-reasons.test.ts`
- Create: `tests/scripts/ci-control-remediation.test.ts`

**Additive interfaces:**

```ts
export type AgentDirective =
  | 'CONTINUE' | 'CONTINUE_WITH_REVALIDATION' | 'STOP_AND_PRESERVE'
  | 'REPAIR_PRECONDITION' | 'PATCH_SOURCE' | 'RECONCILE_LINEAGE'
  | 'RECONSTRUCT_CANDIDATE' | 'REGENERATE_EVIDENCE'
  | 'RETRY_TRANSIENT' | 'REQUEST_APPROVAL' | 'QUARANTINE'
  | 'ROLLBACK' | 'ESCALATE';

export type AgentAuthority =
  | 'observe-only' | 'repair-preconditions' | 'patch-within-approved-scope'
  | 'reconcile-lineage' | 'request-approval'
  | 'manual-recovery-only' | 'never-auto';

export type RetryClassV2 =
  | Exclude<RetryClass, 'manual-recovery'>
...
export interface RecoveryProtocolV1 {
  class: 'stop-preserve' | 'precondition-repair' | 'source-patch'
    | 'lineage-reconciliation' | 'quarantine-recovery';
  preserve: string[];
  invalidate: string[];
  reuse: string[];
  rerun: string[];
  ...
  escalation: { owner: string; condition: string } | null;
  retryCondition: string | null;
}

export interface ControlResultV2
  extends Omit<ControlResultV1, 'schemaVersion' | 'retryClass' | 'aggregateDecision'> {
  schemaVersion: 2;
  taxonomyVersion: 'ci-agent-errors-v2';
  aggregateEffect: AggregateDecision;
  directive: AgentDirective;
  task: TaskBindingV1;
  retryClass: RetryClassV2;
  recovery: RecoveryProtocolV1;
}

export interface ReasonDefinitionV3
  extends Omit<ReasonDefinitionV2, 'schemaVersion' | 'retryClass'> {
  schemaVersion: 3;
  taxonomyVersion: 'ci-agent-errors-v2';
  defaultDirective: AgentDirective;
  allowedAgentAuthorities: readonly AgentAuthority[];
  recoveryClass: RecoveryProtocolV1['class'];
  retryClass: RetryClassV2;
}

export type AnyControlResult = ControlResultV1 | ControlResultV2;
```

`taxonomyVersion` and wire `schemaVersion` are independent. `ReasonDefinitionV3` is an
additive schema rather than an in-place change to immutable V2 semantics. Protected
validation context supplies the task, authority, and lineage-lease expectations; a result
cannot self-authorize. V2 directive, retry, recovery, and authority must agree with the V3
reason definition and the protected task context. `aggregateEffect` is the sole V2
authorization mapping; the exact-key validator rejects the legacy `aggregateDecision`
field rather than allowing two decision fields to diverge. It is `pass` only for a
leaf `pass` or a classifier-proven `not-applicable`, `block` only for a proven leaf block,
and `inconclusive` for warning or inconclusive evidence. Advisory warning observations stay
visible outside the required exact set and never satisfy it.

`RemediationReceiptV1` is non-authorizing evidence. It binds the original finding and
result digest, before/after OIDs, current policy and taxonomy, exact changed locations,
admitted patch scope, unsafe fixture, safe neighbor, focused and surrounding terminal test
attempts, renderer/redaction/self-bypass proof, reviewer evidence digest, authoritative
closure decision, residual risk, and closure time. CP-WA1/CP-GL1 remain the task/lease
owners and CP-F4 remains the executor/agent-action owner; CP-F2f only validates their
protected inputs and recovery instructions.

Migration is explicit: preserve `ControlResultV1` and `ReasonDefinitionV2` byte semantics;
add schema-dispatched readers with independent exact-key sets; record
`manual-recovery -> manual-recovery-only` only as a non-authorizing compatibility mapping;
and make V1 historical/report-only after V2 cutover. V1 cannot enter current aggregation,
and V2 evidence must be regenerated rather than upgraded. V2 becomes the current emission
schema only after exact-byte dual-read, renderer-parity, redaction, and rollback tests pass.

- [ ] **Step 10a: Prove RED for version, authority, recovery, and closure boundaries**

Add V1 golden-byte preservation and V1-authorization rejection; V2 exact-key/version/
taxonomy tests; all five aggregate-effect mappings; every directive/authority/retry/
recovery value and invalid substitution; protected task/lease mismatch and stale generation;
legacy `aggregateDecision` injection and divergent-decision rejection;
preserve/invalidate/reuse/rerun completeness; unchanged deterministic retry; stable renderer
ordering/parity/redaction/budgets; remediation closure binding; native-cause preservation;
historical-code non-emission; unknown/repurposed taxonomy; and incomplete recovery.

- [ ] **Step 10b: Implement the additive V2/V3 readers, renderer projection, and receipt**

Keep one renderer and one reason registry. Human, JSON, annotation, and summary projections
use the ordered directive, situation, cause, dependent symptoms, required action, allowed
scope, prohibited changes, recovery, retry, verification, and escalation fields from the
same validated V2 object. Public output continues to pass the existing all-channel
publication boundary and byte limits.

- [ ] **Step 10c: Verify and independently review the migration**

```bash
bash scripts/run-with-pinned-npm.sh test -- \
  tests/scripts/ci-control-reasons.test.ts \
  tests/scripts/ci-control-result.test.ts \
  tests/scripts/ci-control-remediation.test.ts \
...
Dependency contract: `CP-F2e -> CP-F2f`; `CP-F2f + CP-F3 -> CP-H1c`; and CP-F2f precedes
H1d, CP-F4, CP-F5, and every later authoritative consumer. Completed CP-F3 and additive
H1a/H1b source do not replay merely because the schema is added; their future neutral-result
integration consumes V2.
...
- Evidence: **Proven** at `.husky/pre-push` and `scripts/pre-push-guard.ts:52-74`.
  The absolute foreign `core.hooksPath` was a historical observation that motivated H1a;
  current reconciled configuration resolves to repository-relative `.husky` and passes the
  installed-byte guard, without closing H1d authority.
...
receipt integration waits for CP-F2f, which consumes CP-F2e. Each strict native receipt—not
`ControlResultV1`—owns
...
precondition, CP-F2f taxonomy/result, CP-F3 claimed-scope, CP-WA1 supervisor/terminal-attempt, and H1c
...
and the three equivalent existing call sites through that helper before
`expectedForArtifact()` so no duplicate blind-cast path remains and the companion test
stays below the repository line-count threshold. Keep
...
The program ordering is exact: `CP-WA1 + CP-F3 -> CP-GL1`; then
`CP-H1c + CP-GL1 -> CP-H1d -> CP-F4 -> CP-F5 -> remote review and workflow canaries`.
CP-GL1 is source-only
until a later transport packet is authorized; it must be added to the program dependency
graph and ledger during Task 7 Step 1a before H1d can claim readiness.
...
| before merge | exact proposed-merge OID/tree/parents, candidate plus integration lanes, current aggregate evidence, and topology-applicable merge-group receipt or explicit unavailable status |
...
merge-sensitive evidence is always regenerated for the final proposed merge. On a supported,
separately authorized queue topology, merge-queue `merge_group` execution is the future
authoritative freshness boundary. Until then, exact local proposed-merge validation plus
synthetic/source merge-group fixtures remain distinct evidence and cannot be described as a
live queue canary. Neither path authorizes a hosted merge or ruleset change in this plan.
...
export interface CoverageThresholdReceiptV1 {
  schemaVersion: 1;
  nativeDisposition: 'pass' | 'block';
  nativeCauseCodes: readonly (
    | 'coverage.threshold.satisfied'
    | 'coverage.threshold.no-matching-files'
    | 'coverage.threshold.below-threshold'
  )[];
  coverageSummaryDigest: string;
...
export interface CoverageExecutionValidationContextV1 {
  ...
  expectedTerminalStageLeases: Readonly<
    Partial<Record<CoverageStageName, SupervisorLeaseExpectationsV1>>
  >;
  commandDigest: string;
  cwdDigest: string;
  environmentDigest: string;
...
result/reason owner and owns the sole exactly-four-row ordered `stageEvidence` tuple.
Derive the terminal-stage key set from that tuple and exact-key validate
`expectedTerminalStageLeases`: its keys equal the terminal-stage set exactly and each
expectation's attempt ID equals its row. Missing, extra, unknown, swapped, duplicate-attempt,
or `not-run` lease entries are invalid. Every terminal row requires its exact trusted-store
attempt and lease. A terminal `collect`, `vitest`, or `coverage-threshold` row may be
interpreted as PASS or BLOCK only when its matching schema-valid native receipt binds the
row's attempt ID/digest, revision, selection/config/policy inputs, and emitted bytes. Direct
nonzero status without that receipt is `INCONCLUSIVE`, never BLOCK. Signal, timeout,
cancellation, crash, missing close proof, live descendants, malformed/stale evidence, or
failed atomic terminal publication is also `INCONCLUSIVE`; preserve a provisional native
violation only as related causal evidence. Cleanup never runs a later stage or overwrites
the primary direct status.

The stage/nullability matrix is authoritative:

| Stage/state | Attempt and lease | Native receipt | Decision and downstream state |
|---|---|---|---|
| `strip-types` terminal/0 | required | none by design | continue |
| `strip-types` terminal/nonzero or abnormal | required | none | `INCONCLUSIVE`; all later rows `not-run`; all three receipts null |
| `collect` terminal/valid passing receipt | required | selection receipt required | continue |
| `collect` terminal/valid receipt proving a registered collection violation | required | selection receipt required | `BLOCK`; Vitest and threshold `not-run`; run/threshold receipts null |
| `collect` terminal/nonzero without valid native receipt | required | null | `INCONCLUSIVE`; Vitest and threshold `not-run`; downstream receipts null |
| `vitest` terminal/valid passing receipt | required | run receipt required | continue |
| `vitest` terminal/valid failing receipt | required | run receipt required | `BLOCK`; threshold `not-run`; threshold receipt null |
| `vitest` terminal/nonzero without valid receipt | required | null | `INCONCLUSIVE`; threshold `not-run` |
| threshold terminal/valid passing receipt | required | threshold receipt required | eligible for coverage PASS |
| threshold terminal/valid failing receipt | required | threshold receipt required | `BLOCK` |
| threshold terminal/nonzero without valid receipt | required | null | `INCONCLUSIVE` |
| any `not-run` | forbidden | that stage and every downstream receipt null | never satisfies coverage |

Each `not-run.causedByStage` names the earliest preceding terminal stage that stopped
execution, and every later row remains `not-run`. Valid upstream receipts remain present;
for example, a `vitest:not-run` does not null an already valid selection receipt. The
threshold receipt's closed parser-owned disposition/cause projection binds the canonical
threshold-report bytes and preserves the existing threshold checker as decision owner.
`coverage.threshold.satisfied` is the sole cause for `nativeDisposition: 'pass'`; a block
preserves every observed `no-matching-files` and `below-threshold` cause exactly once in
canonical order. Missing, extra, reordered, duplicate, or disposition/report-mismatched
causes are invalid. The adapter validates and projects those native fields but does not
recompute threshold policy. No upstream receipt may be reconstructed from
later output. The run receipt does not duplicate the stage array. The caller invokes the complete coverage command through the
...
process close; missing/wrong outer attempt/store/lease; exact terminal lease-key mismatch;
extra `not-run` lease; swapped or duplicate attempt; each stage/nullability matrix row;
deterministic collect/Vitest/threshold nonzero with missing, malformed, stale, or mismatched
native receipt; matching native collection/Vitest/threshold failure receipt; threshold
`no-matching-files` only, `below-threshold` only, both causes, and missing/extra/reordered/
duplicate/disposition-mismatched cause sets; strip-types nonzero with three `not-run`
rows; receipt on a `not-run` row; incorrectly nulled upstream receipt; threshold execution
after Vitest block/inconclusive; cleanup/finalization failure after provisional pass or
native violation; altered persisted stdout; and raw failure/private-value output. Preserve existing
...
- Create: `scripts/lib/repo-hygiene-policy.ts`
- Modify: `scripts/publication-guard.ts`
- Modify: `scripts/repo-hygiene-guard.ts`
- Modify: `scripts/safeguard-diagnostics.ts`
- Modify: `tests/scripts/ci-control-git-input.test.ts`
- Create: `tests/scripts/ci-control-git-object-input.test.ts`
- Modify: `tests/scripts/ci-control-result.test.ts`
...
by the current validators. It changes no production result policy or authorization
behavior.

The repository-hygiene source/test pair is a separate owner-approved repair in this
planning/bootstrap reconciliation, not part of CP-TC1. The proven false positive treated inert `.md` and
`.markdown` prose that describes an unbounded-suppression token as executable policy.
The smallest canonical-owner repair exempts only those two inert prose extensions from
that one finding. Its companion fixture proves `.md` and `.markdown` safe neighbors pass
while `.mdx` and test-source suppressions still emit `unbounded-suppression`; executable
sources and every other repository-hygiene rule remain unchanged.

Current-main integration also made the authoritative file-size identity test reachable from
the local branch gate. Its RED receipt reported three new, unregistered over-budget files:
`scripts/repo-hygiene-guard.ts` (2,312 lines), `scripts/lib/ci-control/git-input.ts` (2,433),
and `tests/scripts/ci-control-git-input.test.ts` (3,702). This is a blocking integration
finding, not permission to enlarge the fitness baseline. The admitted prerequisite split
extracts one repository-hygiene policy module, one exact-object Git core, one shared test
fixture module, and one exact-object test suite. It must preserve the facade export set, the
90-test Git contract, hygiene unsafe/safe behavior, tool-digest closure, and classifier/
publication source bindings. Every resulting source or test file must remain below 2,000
lines and the warning-identity test must pass without a baseline change.
...
During this pre-CP-GL1 local source reconciliation, the root coordinator instead supervises one
...
```bash
PATH=/opt/homebrew/opt/python@3.12/libexec/bin:/usr/bin:/bin:/usr/sbin:/sbin:/opt/homebrew/bin /bin/bash scripts/run-with-pinned-npm.sh test -- \
  tests/scripts/ci-control-manifest.test.ts \
  tests/scripts/ci-control-result.test.ts \
  tests/scripts/ci-control-reasons.test.ts \
  tests/scripts/ci-control-attempt.test.ts \
...
The ordered allowlisted suite PATH is a tested host precondition: it selects the audited
Python 3.12 toolchain, whose `tomllib`, `plistlib`, and XML extension load successfully,
before an ambient package-manager Python whose XML extension is unloadable on this host,
while still admitting the exact GNU timeout executable required by boundary-run fixtures.
...
`CP-WA1 + CP-F3 -> CP-GL1`; then
`CP-H1c + CP-GL1 -> CP-H1d -> CP-F4 -> CP-F5 -> CP-W1/workflow canaries` ordering and a separate
...
staging, the hook-bound reconciliation commit, and the exact postcommit replay. Hosted settings,
...
- [ ] **Step 3: Run the existing branch gate and present control-precursor regression bundle**

```bash
PATH=/opt/homebrew/opt/python@3.12/libexec/bin:/usr/bin:/bin:/usr/sbin:/sbin:/opt/homebrew/bin /bin/bash scripts/run-with-pinned-npm.sh run verify:push:branch
bash scripts/run-with-pinned-npm.sh run guard:test-integrity:required
bash scripts/run-with-pinned-npm.sh test -- \
  tests/scripts/ci-control-manifest.test.ts \
  tests/scripts/ci-control-result.test.ts \
...
disclosure. The explicit test list covers only present control-precursor files; it does not
...
```bash
PATH=/opt/homebrew/opt/python@3.12/libexec/bin:/usr/bin:/bin:/usr/sbin:/sbin:/opt/homebrew/bin /bin/bash scripts/run-with-pinned-npm.sh run coverage:check -- --pool=forks --fileParallelism=false
```

Expected: the existing complete suite and coverage thresholds pass with a directly captured
terminal exit. CP-TR1's bounded machine-readable selected/executed/passed/failed/skipped/todo
receipt owner is absent, so this command cannot close CP-TR1 or provide authoritative
exact-set execution evidence. Any truncated, skipped, todo, masked, or nonterminal result is
inconclusive; the static test-integrity gate cannot substitute for runtime evidence.
...
Do not reuse the precommit suite or coverage result: the reconciliation commit changed HEAD and
invalidated it.
...
CP-TR1, CP-F5, and later trust layers remain incomplete.
