...
- Preserve existing required GitHub checks until replacement gates pass real PR and merge-group canaries and a separate hosted-settings change is authorized and read back.
...
- **Current reconciled lease:** after earlier reconciliations at `d7a443cbd329bd2d71bfe7df704f103179f59c57` and `c26cc8b2091d6b39abd0350b535c8ed551d9ab93`, live SSH readback proved remote `main` had advanced from `b2de0cad47bfc68b6291e3a49767cda4fdea5b74` to `e68e969319b08c07c5587cee65fc206a0bb6a9e6` while the remote design branch remained `9fc8a640…`. The exact seven-path packet was stashed and verified, the disjoint two-file design-system delta was merged locally, and the packet was restored at local merge commit `f30052f11a86cc3ece3026ff8e8798ec207831ea`. Its parents are exactly `c26cc8b…` and `e68e9693…`; no path changed between the prior and current remote `main` revisions overlaps the packet.
- **Current planning state:** both ignored plan files are force-admitted by exact path alongside the design, publication audit, current-program record, and generated work indexes. Their digests and every prerequisite receipt are refreshed on the reconciled lease before review, mutation, promotion, and closeout; a changed digest invalidates dependent evidence.
...
- **Proven:** no `controls/ci-control-manifest.json`, neutral `ControlResultV1`, `verify:fast`, `verify:pr`, `verify:portability`, `verify:deploy`, `portability-gate`, or `policy-gate` exists.
...
- **Proven:** the active `core.hooksPath` points to another worktree's absolute `.husky` path. Current bytes matched at audit time, but revision identity is not enforced.
- **Proven:** GitHub workflows lack `merge_group`, use mutable major action tags, provide only narrow native macOS coverage, upload browser failure screenshots without a publication scan, and place a private test-integrity deploy key in a candidate-controlled pull-request job.
- **Proven:** hosted main protection currently requires Linux Node 24/25 quality and CodeQL; native macOS is not required. Repository-level action policy permits all actions. No repository `CODEOWNERS`, environment, self-hosted runner, or tag-target ruleset was observed.
...
| CP-W2 | `ci.aggregate-gate.owner` | `ci-control/cp-w2` | admitted workflow/gate validators, immutable dependency declarations, manifest registrations, and tests | Exact-set PR and merge-group canaries / stop on skipped/missing/duplicate result or mutable dependency |
...
9. Commit through hooks and produce local source evidence. Stop before any remote write unless the current owner request explicitly authorizes the exact branch/repository action; after authorization, fetch/reconcile, push through the exact-ref guard, and bind remote evidence to the pushed OID.
...
- Fetch `origin` before mutation, before final verification, and before push. Any changed `origin/main` invalidates the frozen base; inspect incoming paths and reconcile with an ordinary merge or fast-forward as appropriate.
