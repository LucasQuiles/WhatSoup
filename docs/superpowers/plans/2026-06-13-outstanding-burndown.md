...
> **POST-MERGE CONSOLIDATION (2026-06-13 22:45 ET):** PR #830 landed the provider-hardening compose as `97e60757`. PR CI was green after the BOT ERRORS D22 drill assertion was aligned with the structured credential-redaction output, and post-merge main CI finished 5/5 green. The compose branch/worktree/remote `integration/provider-hardening-compose-refresh-20260613T194657Z` are gone. The pre-merge runbook below is retained as historical proof only; do not run it for #830 again. Draft PRs #828/#829 remain open, peer-owned, and patch-positive. Older local compose/full-compose worktrees are preserved because they still contain unmerged local-only work and cannot be deleted as superseded.
...
>
> **Historical compose contents:** `integration/provider-hardening-compose-refresh-20260613T194657Z` composed the validated hardening branches that landed through #830: private-fs chmod, provider errPreview redaction, heal stale reconciliation, detailed API retry shape, D18 health-poller policy, Python deploy redaction SSOT, guardrail residuals, verification reliability, process-lock ownership, pending-poll timeout normalization/clamping, provider credit taxonomy, release drift CLI, sandbox opencode provider config, and D20 keychain observe-only/opt-in diagnostics. The latest code-bearing commit before handoff-only/docs commits was `6dcae2ea`; D20 implementation landed locally at `56086d44`; final squash landed on main as `97e60757`. Pre-cleanup cherry proof caught two branch-tip commits that were not yet represented in the compose (`184d264a` credit-exhaustion false-positive tightening and `66e5cae3` fallback-opencode sandbox-config coverage); both were included before merge. Compose-only fixes recorded in this branch: require BOT ERRORS helper manifest paths, classify the verification residuals plan in `docs/publication-audit.md`, preserve Python redaction contracts for `Authorization: Bearer ...` and `whatsoup@<phone>` service units, import the canonical `isRecord` helper in the runtime-manifest guard, sanitize private host/domain/tailnet examples in public test fixtures by constructing the values dynamically, and make provider keychain unlock mutation opt-in.
>
...
>
> **Do next:** do not reopen the compose landing work. Keep draft PRs #828 and #829 read-only/peer-owned; their topics overlap current main but remain non-equivalent follow-ups, so do not delete or mark superseded without `range-diff`/owner confirmation. Preserve/triage dirty parallel worktrees before cleanup. **Do not** treat the historical task checklist below as active work without first rechecking current main.

## Distilled Goal Evidence Matrix

| Goal | Current evidence | Status | Remaining gate |
|---|---|---|---|
| Land current burndown branch safely | PR #830 landed as `97e60757`; PR CI and post-merge main CI are green; branch/worktree/remote were removed | Landed | Draft #828/#829 remain peer-owned follow-ups |
| Close ready residual fixes | `bot-errors-emit.py` `atomic_write_json()` calls `ensure_private_dir(path.parent)`; `scripts/import-boundary-check.ts` is in the push-battery boundary guard; `scripts/source-runtime-drift-check.ts` documents its narrower helper contracts | Landed via #830 | Watch real-host runtime evidence; no compose follow-up needed |
| Python deploy-script architecture decision package | `docs/superpowers/plans/2026-06-13-python-deploy-redaction-ssot.md` chooses manifest-tracked `deploy/scripts/lib/` and documents fallback N-copy equivalence guard if real-host shared imports fail | Landed; decision documented | Real-host deployment validation if runtime import issues appear |
| D5/R8 secret-redaction SSOT | Shared `deploy/scripts/lib/bot_errors_redaction.py`; seven BOT ERRORS consumers migrated; redaction parity/SSOT tests pass; TypeScript hygiene scanner explicitly remains separate | Landed via #830 | Watch for real-host import issues |
| Decision-gated cleanup D1-D4/H2 | Main already has D1/D3/D4/H2 through #814; #830 adds D2 chmod defense in `src/lib/private-fs.ts` | Landed | No compose follow-up |
| Collector cooldown flake | #830 adds deterministic fake-clock coverage in `deploy/scripts/tests/test_bot_errors_collector_backoff.py`; no retry/skip/quarantine | Landed | Monitor required quality jobs for recurrence |
| Older high-risk runtime decisions | #830 includes heal stale reconciliation, process-lock ownership/TOCTOU hardening, detailed API retry-shape work, D18 health-poller policy, D20 keychain observe-only/opt-in behavior, and provider errPreview sanitization | Landed | Watch runtime evidence; do not reopen without a new failing signal |
| Verification infrastructure hardening | #830 adds `guard:bot-errors-runtime-manifest`, branch-diff repo hygiene, file-size identity budget, coverage headroom diagnostic, and deploy Python fixture hygiene coverage | Landed | Coverage target itself remains open in RR-013 |
...
## Historical Approval-To-Merge Runbook (Complete)

PR #830 has already completed this runbook. The commands are retained for auditability only; do not rerun them for the deleted compose worktree.
...
| D5/R8 Python secret-redaction SSOT | Landed via #830 | Main uses manifest-tracked `deploy/scripts/lib/bot_errors_redaction.py`, migrates seven BOT ERRORS deploy consumers, adds parity/SSOT tests, and keeps TS source-diff hygiene vocabulary separate for now. |
| Python standalone deploy architecture | Landed via #830; decision package retained | Recommended direction adopted: a manifest-tracked `deploy/scripts/lib/` module with runtime-manifest coverage. Fallback remains N-copy equivalence guard if real-host deployment rejects shared imports. |
| Collector cooldown flake | Landed via #830 | Main has deterministic fake-clock coverage in `deploy/scripts/tests/test_bot_errors_collector_backoff.py`; do not retry/skip/quarantine this class. |
| Older runtime decisions | Landed via #830 | Main includes heal stale reconciliation, process-lock ownership/TOCTOU hardening, detailed API retry-shape work, D18 health-poller policy, provider errPreview sanitization, and D20 keychain observe-only/opt-in unlock policy. D20 is captured in `docs/superpowers/plans/2026-06-13-d20-provider-keychain-unlock-policy.md`; reverify against current main before reopening. |
