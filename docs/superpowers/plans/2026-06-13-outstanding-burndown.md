...
|---|---|---|---|
| P0 | `guard:repo:staged` is not a PR/base diff gate in CI or post-commit pre-push contexts | `.github/workflows/quality.yml` ran the staged guard, but the guard reads `git diff --cached`; a clean CI index made added-line hygiene rules a smoke/no-op rather than PR-diff enforcement | Local branch `chore/guardrail-residuals-20260613` adds `guard:repo:branch-diff`, merge-base scanning, CI wiring, branch push wiring, public-surface docs, and chain self-tests. |
