...
| Older runtime decisions | Partly implemented in local compose; D20 keychain policy remains a separate decision | Compose includes heal stale reconciliation, process-lock ownership/TOCTOU hardening, detailed API retry-shape work, D18 health-poller policy, and provider errPreview sanitization. Reverify each against branch `HEAD` before publishing; do not claim D20 closed. |
