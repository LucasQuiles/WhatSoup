...
**Status:** completed/superseded — original implementation branch landed through #810, with follow-on closures in #814, #808, #813, #804, #816, #817, and #818. Do not restart `chore/outstanding-specs-and-burndown`.

> **CURRENT STATE REFOCUS (2026-06-13):** `origin/main` is `e1f07ffe2898d3dba839bb9d67eb851e09cd3eed`. The original hardening branch is closed:
...
> - #804, #813, #816, #817, and #818 have since landed on main. #816 post-merge main CI on `218dfacd` finished 5/5 green; #817 post-merge main CI on `558730b0` finished 5/5 green; #818 post-merge main CI on `e1f07ffe` was still running at this refresh.
>
> **Do next:** finish observing #818 post-merge CI, then continue the provider-outage hardening follow-up PRs for config/model consistency, primary-model entitlement probing, health turn capability, outbound dedupe, and release drift detection. Preserve/triage dirty parallel worktrees before cleanup. **Do not** treat the historical task checklist below as active work without first rechecking current main.
...
| PR #818: mini1 health profile personal runtime | Landed in current main as `e1f07ffe`; post-merge main CI running | Local isolated validation passed Test Integrity, `tests/scripts/bot-errors-health-check.test.ts` (110 tests), `guard:doc-drift`, and `verify:push:branch`; wait for post-merge main CI before treating deployment handoff as settled. |
...
| Older runtime decisions | Need current-main revalidation | Recheck heal restart suppression, lock TOCTOU, detailed APIs wire-or-excise, HTTP-error degraded-forever, keychain unlock behavior, and errPreview sanitization against landed #807/#813/#815/#816/#818. |
