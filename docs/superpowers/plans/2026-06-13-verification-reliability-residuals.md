...
**Status:** completed - landed via PR #830 (`97e60757`). Collector cooldown stabilization, file-size warning identity pinning, coverage-headroom guard availability, branch-diff hygiene, and deploy Python fixture coverage are on `main`; coverage-headroom remains intentionally advisory until measured coverage has enough headroom.
