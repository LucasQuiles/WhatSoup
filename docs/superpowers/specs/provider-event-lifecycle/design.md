...
- **Rationale:** Migration 64 adds one empty activation marker table plus nine
...
- **Rationale:** The existing turn replay context remains the authority, but gains
  ...
  `failed_permanent` is not. Migration 63 enables aggregate no-send only for proved
  single-op answers; pre-lifecycle multi-op answers remain uncertain because they lack
  a sealed expected set. Migration 64 may prove multi-op no-send only from an immutable
