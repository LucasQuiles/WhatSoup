...
- **Rationale:** Migration 59 adds one empty activation marker table plus nine
...
- **Rationale:** The existing turn replay context remains the authority, but gains
  ...
  `failed_permanent` is not. Migration 58 enables aggregate no-send only for proved
  single-op answers; pre-lifecycle multi-op answers remain uncertain because they lack
  a sealed expected set. Migration 59 may prove multi-op no-send only from an immutable
