...
**Status:** Active — docs-only successor on canonical main `56e232223132d33c347cd2d2521620f911d4f4b6`; migrations 41-43 and PR #1770 are merged, while the schema-ceiling guard and migration 44 remain pending; no implementation or deployment authorized
...
2. Schema-lineage canonicalization is merged through PR #1768
   ...
   migrations 41/42 and adding forward repair migration 43. The current repository
   schema ceiling is 43. This merge did not add the production schema-ceiling
   drain/read-only guard, which remains a separate prerequisite before schema 44.
3. Treat #1744/#1749 as required terminal-recovery prerequisites and #1750 as an open
   ...
   delivery truth. Implement that prerequisite as forward migration 44 only after
   migration 43 is canonical; do not alter historical migrations or merge the stale
   DGX recovery branch wholesale.
4. Land and fleet-verify the schema-ceiling guard on the canonical schema-43 base,
   then land migration 44. Rebase the implementation branch onto both resulting
   canonical merges; migration 45 is then available to the provider-event ledger.
...
- **Rollback:** After activation the runtime is roll-forward-only; before activation,
  rollback permits the immutable migration-45 marker but requires no activation row,
  ...
  otherwise requires full v45 write compatibility or drain/read-only rejection of
  ...
  data and schema history to the exact backup's v44 fingerprint and removes the source
  v45 row as part of that atomic state replacement. Unknown fleet fingerprints stop rollout.
...
- **Status:** pending — PR #1770 recovery-health semantics are merged; canonical `not_sent`, bounded terminal closure, and migration 44 remain absent
...
- **Steps:**
  ...
  - [ ] Reserve `outbound_ops.status='not_sent'` for typed pre-send rejection and keep
    the exact op ID/status in terminal `deliveryKind=not_sent`. Migration 44 permits
    terminal aggregate no-send only for positively proved single-op answers; 1:N stays
    partial/uncertain until migration 45's immutable sealed expected set proves every
  ...
  - [ ] Base this work on the merged migration-43 canonicalization and allocate forward
    migration 44 for canonical outbound/terminal `not_sent`, immutable transfer
  ...
  - [ ] Add a declared closure-eligibility index and migration-44 storage accounting.
...
- **Status:** in progress — PR #1768 merged migrations 41-43; the schema-ceiling guard and CHK-071 remain pending
...
- **Steps:**
  ...
  - [x] Restore the exact final historical migration-41 source and the exact
    ...
    coverage. `CURRENT_SCHEMA_MIGRATION` is 43 on this branch base.
  - [ ] Add or relocate the marked `CHK-001` and `CHK-071` plan-stage conformance
    cases under `tests/spec-conformance/provider-event-lifecycle/` so the merged
    migration evidence and remaining schema-ceiling behavior are executable against
    this spec without duplicating the existing migration unit coverage.
  - [ ] Add the marked `CHK-071` RED cases, then extend
    ...
    preserve backup/inspection but reject every provider turn. Deploy and verify this
    guard fleet-wide before any schema-44/45 writer; record and prohibit older pre-guard
    binary fingerprints as rollback targets.
  ...
  - [ ] After TSK-010 also merges migration 44, rebase this lifecycle branch onto both
    exact prerequisite merge commits before TSK-002; do not begin fixtures on the
    intermediate migration-43-only base.
...
#### TSK-003: Add migration 45 and the durable receipt ledger
- **Status:** pending
...
- **Steps:**
  ...
  - [ ] Add `src/core/database-migration-45.ts` and register it in
...
- **Steps:**
  ...
  - [ ] Extend the existing deploy preflight/cutover path and managed compatibility
    ...
    runtime artifact, it proves that artifact is v45 write-compatible; otherwise it
  ...
  - [ ] Document receipt inspection, quarantine/tombstone meaning, replay veto,
    ...
    exact verified pre-v45 backup while still quiesced with no activation marker,
    ...
    the migration-45 marker may exist in the source; the whole-database restore returns
    schema history to the backup's exact v44 fingerprint and is the sole allowed removal
    ...
    downgrade to be fully v45 write-compatible or drain/read-only with all new
