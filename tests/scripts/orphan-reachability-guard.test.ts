...
const TRACKED_UNREACHABLE: readonly TrackedEntry[] = [
  ...
  // transition-controller/resolver subtree with it (HealthPoller now instantiates the
  // controller and feeds it health samples). The mode-bucket modules remain unwired;
  // nothing production imports them yet.
  ...
  { path: 'src/core/observability/lifecycle-event-store.ts', issue: 'FLOS Stage 1 (plan §3; design §10 Contract B)', reason: 'bounded private store lands unwired by design; the emission slice is the runtime importer and graduates this entry' },
  ...
  // #3057 RESOLVED: recovery-authority-store is now wired into health-poller
  // emit/clear/startup-scan — entry removed; the guard graduation test at :359
  // confirms the module is reachable from the production import graph.
