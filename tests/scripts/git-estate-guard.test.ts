...
function gitInspectionFailureEnvironment(root: string): NodeJS.ProcessEnv {
  const resolvedGit = spawnSync('sh', ['-c', 'command -v git'], { encoding: 'utf8' });
  expect(resolvedGit.status, resolvedGit.stderr).toBe(0);
...
function stashChangeEnvironment(root: string): { env: NodeJS.ProcessEnv; marker: string } {
  const resolvedGit = spawnSync('sh', ['-c', 'command -v git'], { encoding: 'utf8' });
  expect(resolvedGit.status, resolvedGit.stderr).toBe(0);
...
function baselineChangeEnvironment(
  root: string,
  baselinePath: string,
): { env: NodeJS.ProcessEnv; marker: string } {
  const resolvedGit = spawnSync('sh', ['-c', 'command -v git'], { encoding: 'utf8' });
...
function finalClosureBaselineChangeEnvironment(
  root: string,
  baselinePath: string,
): { env: NodeJS.ProcessEnv; marker: string } {
  const resolvedGit = spawnSync('sh', ['-c', 'command -v git'], { encoding: 'utf8' });
