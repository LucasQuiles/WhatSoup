...
function targetReceipt(
  ...
  snapshotMode: "changed" | "no-op" = operationResult === "no-op"
    ? "no-op"
    : "changed",
): Record<string, unknown> {
...
function oneTargetRows(
  target: Record<string, unknown>,
): Array<Record<string, unknown>> {
  const batchId = String(target.batch_id);
  const operationId = String(target.operation_id);
  const rows = [
    batchStarted(batchId, operationId, [Number(target.issue_number)]),
    target,
  ];
  if (
    target.receipt_type === "target_verified" &&
    target.operation_result !== "planned"
  ) {
    rows.push(
      completed(
        batchId,
...
describe("open issue registry schema", () => {
  it("canonically serializes and hashes the complete registry envelope", () => {
    ...
    expect(
      (
        JSON.parse(
          canonicalRegistryJson(parseRegistry(padded)),
        ) as typeof registry
  ...
  it("pins the exact live and addable label catalogues", () => {
    expect(LIVE_LABELS).toHaveLength(44);
  ...
  it("represents privacy-withheld pull request refs as null, never as fake refs", () => {
    ...
    expect(() => parseRegistry(ambiguousPlaceholder)).toThrow(
      /withheld.*null/i,
    );
  });
  ...
  it("rejects malformed, unsorted, and duplicate pull request overlap evidence", () => {
    ...
    unsafeRef.issues[0].pull_request_overlaps = [
      { ...overlap, head_ref: "../private" },
    ];
    ...
    unknownField.issues[0].pull_request_overlaps = [
      { ...overlap, body: "forbidden" },
    ];
  ...
  it("rejects unsafe issue URLs, repository paths, and unsorted or duplicate lists", () => {
    ...
    unsafeUrl.issues[0].url =
      "https://example.com/LucasQuiles/WhatSoup/issues/101";
    expect(() => parseRegistry(unsafeUrl)).toThrow(/URL/i);

    const unsafePath = cloneRegistry();
    unsafePath.issues[0].affected_paths = [
      ["", "Users", "operator", "project", "file.ts"].join("/"),
    ];
    ...
    const duplicateObligations = cloneRegistry();
    duplicateObligations.issues[0].lead_verification_obligations = [
      "Re-read the decisive source before mutation.",
      "Re-read the decisive source before mutation.",
    ];
    expect(() => parseRegistry(duplicateObligations)).toThrow(
      /sorted|duplicates/i,
    );
  ...
  it.each([0x01, 0x1f, 0x7f])(
    "rejects repository paths containing control U+%s",
    (codePoint) => {
      const invalid = cloneRegistry();
      invalid.issues[0].affected_paths = [
        `src/${String.fromCharCode(codePoint)}file.ts`,
      ];
  ...
  it("rejects whitespace-only required registry text and repository paths", () => {
    const invalidRegistries = [
      ...
      (() => {
        const invalid = cloneRegistry();
        invalid.issues[0].pull_request_overlaps = [
          { ...overlap, title: " \n\t " },
        ];
        return invalid;
      })(),
  ...
  it("rejects duplicate issues, self-references, URL drift, and inventory drift deterministically", () => {
    ...
    urlDrift.issues[0].url =
      "https://github.com/LucasQuiles/WhatSoup/issues/102";
    expect(
      validateRegistry(parseRegistry(urlDrift)).map((finding) => finding.code),
  ...
  it("preserves current labels and refuses new live-but-non-addable labels", () => {
    ...
    expect(
      validateRegistry(parseRegistry(missingCurrent)).map(
        (finding) => finding.code,
      ),
    ).toContain("current-label-not-recommended");

    const nonAddable = cloneRegistry();
    nonAddable.issues[0].recommended_labels = [
      "bug",
      "good first issue",
      "reliability",
    ];
    expect(
      validateRegistry(parseRegistry(nonAddable)).map(
        (finding) => finding.code,
      ),
  ...
  it("detects cycles among open implementation-order relationships", () => {
    ...
    expect(
      validateRegistry(parseRegistry(cyclic)).map((finding) => finding.code),
  ...
  it("rejects every direct self-reference", () => {
    ...
    expect(
      validateRegistry(parseRegistry(selfDependency)).map(
        (finding) => finding.code,
      ),
    ...
    expect(
      validateRegistry(parseRegistry(selfDuplicate)).map(
        (finding) => finding.code,
      ),
    ...
    expect(
      validateRegistry(parseRegistry(selfOrder)).map((finding) => finding.code),
    ...
    expect(
      validateRegistry(parseRegistry(selfPartial)).map(
        (finding) => finding.code,
      ),
...
describe("registry classification and cohort semantics", () => {
  it("requires actionable leaf boundaries, paths, falsifier, and acceptance criteria", () => {
    for (const field of [
      ...
      else if (field === "falsifier_or_remaining_gap")
        invalid.issues[0][field] = "";
      else invalid.issues[0][field] = [];
      expect(
        validateRegistry(parseRegistry(invalid)).map((finding) => finding.code),
  ...
  it("enforces stale, measurement-only, and partial evidence coherence", () => {
    ...
    expect(
      validateRegistry(parseRegistry(stale)).map((finding) => finding.code),
    ...
    expect(
      validateRegistry(parseRegistry(wrongMeasurement)).map(
        (finding) => finding.code,
      ),
    ...
    partial.issues[0].partial_findings = [];
    expect(
      validateRegistry(parseRegistry(partial)).map((finding) => finding.code),
    ...
    expect(
      validateRegistry(parseRegistry(unexpected)).map(
        (finding) => finding.code,
      ),
  ...
  it("keeps trackers out of cohorts and requires leaf cohorts to have an owner", () => {
    ...
    expect(
      validateRegistry(parseRegistry(tracker)).map((finding) => finding.code),
    ).toEqual(
      expect.arrayContaining([
        "tracker-cohort-forbidden",
        "tracker-pr-owner-forbidden",
      ]),
    );
    ...
    expect(
      validateRegistry(parseRegistry(missingOwner)).map(
        (finding) => finding.code,
      ),
  ...
  it("rejects cohorts for duplicate, stale, and measurement-only records", () => {
    for (const [classification, evidenceState] of [
      ...
      if (classification === "duplicate")
        invalid.issues[0].duplicate_of_issue_number = 999;
      expect(
        validateRegistry(parseRegistry(invalid)).map((finding) => finding.code),
  ...
  it("rejects a cohort with an open owning overlap and compares owner against overlap.number", () => {
    ...
    owningOverlap.issues[0].pull_request_overlaps = [
      { ...overlap, assessment: "owns" },
    ];
    expect(
      validateRegistry(parseRegistry(owningOverlap)).map(
        (finding) => finding.code,
      ),
    ...
    expect(
      validateRegistry(parseRegistry(ownerOverlap)).map(
        (finding) => finding.code,
      ),
...
describe("PUBLIC registry safety", () => {
  it.each([
    [
      [["", "Users", "privateoperator", "project"].join("/")],
      /public|private|path/i,
    ],
    [[["operator", "real-company.com"].join("@")], /public|private|email/i],
    [
      [["120363123456789", "g.us"].join("@")],
      /public|private|secret|redaction/i,
    ],
    [
      [["15551234567", "s.whatsapp.net"].join("@")],
      /public|private|secret|redaction/i,
    ],
  ...
  it("refuses secret-like values and forbidden attribution assembled at runtime", () => {
    ...
    secret.issues[0].evidence_summary = ["ghp", "AbCdEf0123456789AbCdEf"].join(
      "_",
    );
...
describe("mutation receipt schema and ledger lifecycle", () => {
  it("hashes and parses all four strict receipt variants without body or title text", () => {
    ...
    expect(parseLedger(ledgerText(unknownRows)).at(-1)?.receipt_type).toBe(
      "target_unknown",
    );
  });
  ...
  it("rejects recovery drift in result, identity, snapshots, or deltas", () => {
    const mutations: Array<
      [string, (receipt: Record<string, unknown>) => void]
    > = [
      [
        "result",
        (receipt) => {
      ...
      [
        "expected snapshot",
        (receipt) => {
          (receipt.expected_after as Record<string, unknown>).body_sha256 =
            HASH_E;
        },
      ],
      [
        "title delta",
        (receipt) => {
          (receipt.expected_after as Record<string, unknown>).title_sha256 =
            HASH_A;
          receipt.title_delta = {
            before_sha256: HASH_F,
            after_sha256: HASH_A,
  ...
  it.each([
    [
      "wrong transition",
      [batchStarted(HASH_E, "op-101", [101]), completed(HASH_E, "op-101", 1)],
    ],
    [
      "wrong batch ID",
      [
  ...
  it("rejects a non-start first row and an empty or unsorted start target list", () => {
    ...
    expect(() => ledgerText([batchStarted(HASH_E, "op-101", [])])).toThrow(
      /nonempty/i,
    );
    expect(() =>
      ledgerText([batchStarted(HASH_E, "op-101", [102, 101])]),
    ).toThrow(/sorted/i);
  });
  ...
  it("rejects wrong global sequence and previous hash linkage", () => {
    ...
    const { receipt_sha256: _sequenceHash, ...sequenceWithoutHash } =
      wrongSequence[1];
    wrongSequence[1] = {
      ...sequenceWithoutHash,
    ...
    expect(() =>
      parseLedger(
        `${wrongSequence.map((row) => JSON.stringify(row)).join("\n")}\n`,
      ),
    ).toThrow(/sequence/i);

    const wrongPrevious = structuredClone(rows);
    wrongPrevious[1].previous_receipt_sha256 = HASH_F;
    const { receipt_sha256: _previousHash, ...previousWithoutHash } =
      wrongPrevious[1];
    wrongPrevious[1] = {
      ...previousWithoutHash,
    ...
    expect(() =>
      parseLedger(
        `${wrongPrevious.map((row) => JSON.stringify(row)).join("\n")}\n`,
      ),
  ...
  it("rejects receipt hash tampering and complete body or title text fields", () => {
    ...
    expect(() =>
      parseLedger(`${tampered.map((row) => JSON.stringify(row)).join("\n")}\n`),
    ...
    for (const field of ["body", "title"]) {
      ...
      expect(() =>
        parseLedger(
          `${forbidden.map((row) => JSON.stringify(row)).join("\n")}\n`,
        ),
...
describe("mutation result timestamp and diagnostic coherence", () => {
  ...
  it.each(validCases)(
    "accepts coherent $operation_result receipts",
    (target) => {
      expect(() =>
        parseLedger(ledgerText(oneTargetRows(target))),
      ).not.toThrow();
    },
  );
  ...
  it("requires no-op snapshots and deltas to be exact equality", () => {
    const mutations: Array<(receipt: Record<string, unknown>) => void> = [
      ...
      (receipt) => {
        (receipt.expected_after as Record<string, unknown>).labels = [
          "bug",
          "reliability",
        ];
  ...
  it("rejects planned targets and incoherent aggregate results in completed batches", () => {
    ...
    expect(() => parseLedger(ledgerText(planned))).toThrow(
      /planned|completion/i,
    );
    ...
    expect(() => parseLedger(ledgerText(allNoOpClaimedApplied))).toThrow(
      /result|no-op/i,
    );
    ...
    expect(() => parseLedger(ledgerText(appliedClaimedNoOp))).toThrow(
      /result|no-op/i,
    );
  });
  ...
  it.each([
    [
      "planned with applied timestamp",
      validCases[0],
      { applied_at: APPLIED_AT },
    ],
    ["no-op without verification", validCases[1], { verified_at: null }],
    [
      "applied-verified without application",
      validCases[2],
      { applied_at: null },
    ],
    [
      "ambiguous verification without diagnostic",
      validCases[3],
      { diagnostic_code: null },
    ],
    [
      "refused update without diagnostic",
      validCases[4],
      { diagnostic_code: null },
    ],
    [
      "failed-before-write with application timestamp",
      validCases[5],
      { applied_at: APPLIED_AT },
    ],
    [
      "unknown write without application timestamp",
      validCases[6],
      { applied_at: null },
    ],
    [
      "post-write mismatch with verification timestamp",
      validCases[7],
