...
function makeLowControlsAvailable(manifest: ControlManifestV1): void {
  // Flip every checked-in unavailable control (planned/quarantined/deprecated)
  // to report-only so the fixture is independent of which controls the live
  // manifest currently quarantines or stages.
  for (const record of manifest.controls) {
    if (['planned', 'quarantined', 'deprecated'].includes(record.availability)) {
      record.availability = 'report-only';
    }
  }
}
