...
describe("open issue triage CLI", () => {
  ...
  it("withholds repository-hygiene PR refs before exact reconciliation", async () => {
    const root = fixtureRoot({ generatedView: true });
    ...
    const overlap: OpenIssueRegistry["issues"][number]["pull_request_overlaps"][number] =
      {
        number: 88,
        title: "Private-host alignment",
    ...
    source.inventory.open_pull_request_count = 1;
    source.inventory.draft_pull_request_count = 1;
    source.issues[0]!.pull_request_overlaps = [overlap];
    writeFileSync(
      join(root, "docs/triage/open-issue-registry.json"),
    ...
    writeFileSync(
      ...
      `${JSON.stringify({
        ...
        retained_issue_states: [],
        retained_overlap_states: [
          { issue_number: 101, pull_request_number: 88, overlap },
        ],
      })}\n`,
    );
    const client = new FakeClient();
    client.capture.pullRequests = [
      {
        ...
        body: "",
        url: overlap.url,
        updatedAt: overlap.updated_at,
        disposition: "open",
        isDraft: true,
        headRefName: withheldRef,
        baseRefName: "main",
        changedFiles: 1,
    ...
    expect(
      await run(reconcileCheckArgs(MAIN_SHA), root, client, io.runtime),
    ).toBe(0);
    expect(io.stderr.join("")).not.toContain(withheldRef);
    expect(io.stdout.join("")).not.toContain(withheldRef);
  });
