...
describe('ImessageAdapter — inbound reaction routing', () => {
  ...
  it('does not dedupe a malformed reaction before a corrected same-GUID redelivery', async () => {
    const invalidPayloads: Array<Partial<InboundImessage>> = [
    ...
    for (const invalidPayload of invalidPayloads) {
      ...
      expect(adapter.handleInboundRecord(reactionRecord(invalidPayload))).toBe(false);
      expect(adapter.handleInboundRecord(reactionRecord())).toBe(true);
      expect(events).toEqual([
        expect.objectContaining({
          target: expect.objectContaining({ id: 'orig-1' }),
          emoji: '❤️',
          removed: false,
        }),
      ]);
