...
describe('BlueBubblesPort — send', () => {
  it('sends to a 1:1 recipient by constructing the DM chat guid', async () => {
    ...
    const result = await port.send({ recipient: 'friend@icloud.com', body: 'hi' });
    expect(result.guid).toBe('g-1');
    expect(mock.calls[0]?.body).toEqual({
      chatGuid: 'iMessage;-;friend@icloud.com',
      message: 'hi',
    });
  });
  ...
  it('throws MalformedResponse when the server returns no guid', async () => {
    ...
    await expect(port.send({ recipient: 'a@b.com', body: 'x' }))
...
describe('BlueBubblesPort — listInboundSince', () => {
  const bbMsg = (overrides: Record<string, unknown> = {}) => ({
    ...
    text: 'hello',
    isFromMe: false,
    handle: { address: 'friend@icloud.com' },
    dateCreated: 1000,
    ...overrides,
  });
...
describe('BlueBubblesPort — sendReaction', () => {
  it('maps tapback emojis to reactionType (1:1)', async () => {
    ...
    await port.sendReaction({ targetGuid: 'g-1', conversation: 'friend@icloud.com', emoji: '❤️', remove: false });
    expect(mock.calls[0]?.body).toEqual({
      chatGuid: 'iMessage;-;friend@icloud.com',
  ...
  it('rejects unsupported emojis', async () => {
    const { port } = makePort();
    await expect(port.sendReaction({ targetGuid: 'g', conversation: 'a@b.com', emoji: '🔥', remove: false }))
...
describe('BlueBubblesPort — sendReadReceipts + sendTypingIndicator', () => {
  it('marks a conversation read with the message guids', async () => {
    const { port, mock } = makePort();
    mock.on('POST', '/chat/iMessage%3B-%3Bfriend%40icloud.com/read', () => ({}));
    await port.sendReadReceipts({ conversation: 'friend@icloud.com', guids: ['g-1', 'g-2'] });
    expect(mock.calls[0]?.body).toEqual({ messageGuids: ['g-1', 'g-2'] });
  });

  it('sends typing start (POST) and stop (DELETE)', async () => {
    const { port, mock } = makePort();
    const p = '/chat/iMessage%3B-%3Bfriend%40icloud.com/typing';
    mock.on('POST', p, () => ({}));
    mock.on('DELETE', p, () => ({}));
    await port.sendTypingIndicator({ conversation: 'friend@icloud.com', composing: true });
    await port.sendTypingIndicator({ conversation: 'friend@icloud.com', composing: false });
