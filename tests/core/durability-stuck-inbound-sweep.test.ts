...
describe('sweepStuckInbound — live stuck-inbound reconciler (W2b)', () => {
  ...
  it('completes a stale processing inbound whose terminal op is echoed (recovered_response_sent)', () => {
    ...
    expect(res).toEqual({ completedEchoed: 1, completedTurnDone: 0, failedStale: 0, reclaimedRecoveryOwned: 0, closedFromTerminalRecord: 0 });
  });
  ...
  it('leaves a fresh (<5 min) open inbound with an echoed terminal op untouched', () => {
    ...
    expect(status(seq)).toBe('processing');
    expect(res).toEqual({ completedEchoed: 0, completedTurnDone: 0, failedStale: 0, reclaimedRecoveryOwned: 0, closedFromTerminalRecord: 0 });
  });
  ...
  it('completes a stale turn_done inbound with no outbound op (recovered_turn_done)', () => {
    ...
    expect(res).toEqual({ completedEchoed: 0, completedTurnDone: 1, failedStale: 0, reclaimedRecoveryOwned: 0, closedFromTerminalRecord: 0 });
  });
  ...
  it('fails a stale processing inbound with no outbound op (error)', () => {
    ...
    expect(res).toEqual({ completedEchoed: 0, completedTurnDone: 0, failedStale: 1, reclaimedRecoveryOwned: 0, closedFromTerminalRecord: 0 });
  });
  ...
  it('a second sweep makes no further changes (idempotent)', () => {
    ...
    expect(first).toEqual({ completedEchoed: 1, completedTurnDone: 1, failedStale: 1, reclaimedRecoveryOwned: 0, closedFromTerminalRecord: 0 });

    const second = engine.sweepStuckInbound();
    expect(second).toEqual({ completedEchoed: 0, completedTurnDone: 0, failedStale: 0, reclaimedRecoveryOwned: 0, closedFromTerminalRecord: 0 });
  });
