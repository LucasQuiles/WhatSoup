...
describe('atomic linked turn recovery jobs', () => {
  ...
  it('retains a corroborated unknown-delivery job without claimability or admission blocking', () => {
    const transfer = createTransfer('corroborated-retained');
    ...
    const selectedBefore = db.raw.prepare('SELECT * FROM outbound_ops WHERE id = ?').get(selectedOpId);
    const terminalBefore = db.raw.prepare(
      'SELECT * FROM turn_terminal_records WHERE id = ?',
    ).get(receipt.recordId);
    const jobBefore = db.raw.prepare('SELECT * FROM turn_recovery_jobs WHERE id = ?').get(jobId);
