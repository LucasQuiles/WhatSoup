/**
 * Migration 66: the append-only `continuity_gap_closures` table. Closure is a
...
import { runMigration66 } from '../../src/core/database-migration-66.ts';
...
const tmp = trackTmpDirs('whatsoup-migration-66-');
...
describe('migration 66 through the registry', () => {
  it('a fresh database opens at schema 66 with the closure table and its guards', () => {
    expect(CURRENT_SCHEMA_MIGRATION).toBe(66);
    const db = new Database(':memory:');
    db.open();
    try {
      const raw = db.raw;
      expect(ledgerVersions(raw)).toEqual(Array.from({ length: 66 }, (_, i) => i + 1));
  ...
  it('upgrades a schema-65 file in place and preserves recorded continuity gaps', () => {
    ...
    // Emulate a schema-65 file: remove exactly what migration 66 adds.
    first.raw.exec('DROP TABLE continuity_gap_closures');
    first.raw.prepare('DELETE FROM schema_migrations WHERE version = 66').run();
    first.close();
  ...
  it('a binary at the previous ceiling refuses the migrated file as future_schema', () => {
    // A schema-65 binary compares the ledger maximum with its own ceiling.
    // Recording one version above this binary's ceiling exercises the same
    // comparison a 65 binary performs against a 66 file.
