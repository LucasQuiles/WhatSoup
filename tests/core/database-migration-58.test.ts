/**
 * Migration 58 — capability-obligation replay schema (D3/D4/D5/D6/D7).
 *
...
describe('migration 58 — registration and idempotency', () => {
  ...
  it('re-running the migration function on an already-migrated db is safe', () => {
    expect(() => runMigration58(db.raw)).not.toThrow();
  });
});
      ...
      db.raw
        .prepare(
          `INSERT INTO capability_attestations
           ...
           VALUES ('test-host', 'test-user', 'relsha', 58, 'claude-cli', 'persistent_session',
