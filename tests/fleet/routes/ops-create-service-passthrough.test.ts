 ...
 * same temp dirs, real handler, config.json readback. type: 'chat' is used so
 * the render-options-only block (claudeConfigDir + pathPrepend, no
 * expectedAccountDigest) exercises validateLaunchdServiceConfig without the
 * agent-only account-identity path.
 */
...
describe('handleCreateLine — service block passthrough (issue #3401 item 2)', () => {
  ...
  it('persists a valid service block to config.json on CREATE', async () => {
    const service = {
      claudeConfigDir: '/opt/whatsoup-test/claude-agent',
