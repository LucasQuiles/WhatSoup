...
describe('AgentRuntime.getHealthSnapshot — per_chat shape', () => {
  ...
  it('latches a delayed per-chat halt across queue deletion and scope rekey', async () => {
    ...
    expect(JSON.stringify(snapshot.details)).not.toContain(lidScope);
...
describe('AgentRuntime.getHealthSnapshot — single-session shape', () => {
  ...
  it('reports a halted shared turn queue as unhealthy', () => {
    capturedTurnQueues.length = 0;
    const runtime = new AgentRuntime(makeDb(), makeMessenger(), 'test', {
      sessionScope: 'shared',
    });
  ...
  it('does not report the inactive turn queue in single mode', () => {
    capturedTurnQueues.length = 0;
    const runtime = new AgentRuntime(makeDb(), makeMessenger(), 'test');
    capturedTurnQueues.at(-1)!.halt();
