...
describe('TurnQueueHaltLatch', () => {
  it('preserves a halt through an alias chain and keeps one scope', () => {
  ...
  it('coalesces halted aliases when their scopes collide', () => {
    const latch = new TurnQueueHaltLatch();
    latch.halt('jid');
    latch.halt('lid');
  ...
  it('reports shared state from the live queue and never reports it in single mode', () => {
    const latch = new TurnQueueHaltLatch();
