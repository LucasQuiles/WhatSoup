...
describe('NL routing handlers (nlRouting flag)', () => {
  ...
  describe('bare keep promotes a temporary route pin to permanent', () => {
    ...
    // Q-CANARY model-pin `keep` contract (2026-07-23), actor policy: only the
    // sender who set the chat's current winning preference may confirm it —
    // NOT any group member. This test used to assert the opposite (the
    // defect GAP-CHECK flagged: "any group member may promote"); it now
    // proves the refusal instead of the resurrection.
