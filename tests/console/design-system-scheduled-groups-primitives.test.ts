...
describe('scheduled/groups design-system primitives', () => {
  ...
  it('removes raw fallback colors and hardcoded numeric style literals from the new surfaces', () => {
    ...
    expect(scheduledRow).not.toContain("borderTop: 'var(--bw) solid var(--border-hairline)'")
    expect(groupDetail).not.toContain("borderBottom: 'var(--bw) solid var(--border-hairline)'")
    ...
    expect(contactPicker).not.toContain("border: 'var(--bw) solid var(--border-hairline)'")
    expect(contactPicker).not.toContain("maxHeight: '200px'")
    expect(chatPicker).not.toContain("border: 'var(--bw) solid var(--border-hairline)'")
  ...
  it('keeps the updated modal surfaces on shared dialog/input/tab primitives', () => {
    ...
    expect(scheduleComposer).toContain('className="text-text-2"')
