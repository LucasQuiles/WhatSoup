 ...
 *
 * Source surprises pinned inline (see §SOURCE SURPRISES).
 *
...
// Stub MessageBubble: renders message content + fromMe flag so tests can
// assert which messages are visible without coupling to bubble markup.
vi.mock('../../console/src/components/MessageBubble', () => ({
  default: ({ msg, animate }: { msg: { pk: number; content: string | null; fromMe: boolean }; animate?: boolean }) => (
    <div
      data-testid="message-bubble"
      data-pk={msg.pk}
      data-from-me={String(msg.fromMe)}
      data-animate={String(animate ?? false)}
    >
      {msg.content ?? ''}
    </div>
  ),
...
// Stub ChatListItem so we can assert selection state + clicks without
// depending on its internal markup (covered by its own tests).
vi.mock('../../console/src/components/ChatListItem', () => ({
  default: ({ chat, isSelected, onClick, isTyping }: {
    chat: { conversationKey: string; name: string }
    isSelected: boolean
    onClick: () => void
    isTyping: boolean
  }) => (
    <div
      ...
      data-key={chat.conversationKey}
      data-selected={String(isSelected)}
      data-typing={String(isTyping)}
      onClick={onClick}
    >
      {chat.name}
    </div>
...
describe('HistoryTab — chat list', () => {
  it('renders a ChatListItem for each chat in the list', () => {
    ...
    expect(items[0]!.getAttribute('data-key')).toBe(CHAT_A.conversationKey)
    expect(items[1]!.getAttribute('data-key')).toBe(CHAT_B.conversationKey)
  ...
  it('marks the selected chat with data-selected=true and all others as false', () => {
    ...
    const items = screen.getAllByTestId('chat-list-item')
    const aliceItem = items.find(el => el.getAttribute('data-key') === CHAT_A.conversationKey)
    const bobItem = items.find(el => el.getAttribute('data-key') === CHAT_B.conversationKey)

    expect(aliceItem?.getAttribute('data-selected')).toBe('false')
    expect(bobItem?.getAttribute('data-selected')).toBe('true')
  })
  ...
  it('passes isTyping=true to ChatListItem when the jid is in typingJids', () => {
    ...
    expect(aliceItem?.getAttribute('data-typing')).toBe('true')
    expect(bobItem?.getAttribute('data-typing')).toBe('false')
  })

  it('handles group JIDs correctly alongside phone JIDs', () => {
    render(withProviders(<HistoryTab {...defaultProps({ chats: [CHAT_A, GROUP_CHAT] })} />))

    const items = screen.getAllByTestId('chat-list-item')
    expect(items).toHaveLength(2)
    expect(items[1]!.getAttribute('data-key')).toBe('120363001@g.us')
    expect(items[1]!.textContent).toBe('Team Chat')
  })
})
...
describe('HistoryTab — message rendering', () => {
  ...
  it('passes fromMe correctly to each bubble', () => {
    render(withProviders(<HistoryTab {...defaultProps()} />))

    const bubbles = screen.getAllByTestId('message-bubble')
    const pks = bubbles.map(b => b.getAttribute('data-pk'))
    // Messages are reversed before virtualization so newest (fromMe=true, pk=102) comes first in reversed[]
    // SOURCE SURPRISE §3: messages prop is reversed() before passing to virtualizer —
    // MSG_2 (pk=102, newer) appears at index 0 in the reversed array.
    expect(pks).toContain('101')
    expect(pks).toContain('102')

    const aliceMsg = bubbles.find(b => b.getAttribute('data-pk') === '101')
    const myMsg = bubbles.find(b => b.getAttribute('data-pk') === '102')
    expect(aliceMsg?.getAttribute('data-from-me')).toBe('false')
    expect(myMsg?.getAttribute('data-from-me')).toBe('true')
  })
...
describe('HistoryTab — outgoing background by mode', () => {
  // SOURCE SURPRISE §4: outgoingBg is computed from mode prop and passed to
  ...
  it('uses agent soft color for agent mode (source pin)', () => {
    ...
    expect(src).toContain("mode === 'agent' ? 'var(--m-agt-soft)' : 'var(--m-cht-soft)'")
  })
})
...
describe('HistoryTab — load older messages', () => {
  ...
  it('deduplicates messages with matching pks (no phantom rows)', async () => {
    ...
    await waitFor(() => {
      ...
      expect(uniquePks.size).toBe(pks.length)
      expect(pks.filter(p => p === '101')).toHaveLength(1)
    })
  })
