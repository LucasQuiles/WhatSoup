 ...
 *
 * Asserts that the "Polling" connection-status badge carries text-text-2 (AA ink)
...
// Option B: container span text-text-3 → text-text-2.
// ---------------------------------------------------------------------------

describe('Nav DD-8 — Polling badge carries text-text-2 not text-text-3 (Option B)', () => {
  it('Polling span has text-text-2 class (AA ink — package §2.1)', () => {
    wsConnected = false
    ...
    // AA ink assertion: text-text-2 must be present on the badge.
    expect(badgeSpan.className).toContain('text-text-2')
  })

  it('Polling span does NOT carry text-text-3 (ghost tier absent — DD-8 §2.1)', () => {
    wsConnected = false
    renderNav()
    ...
    // Ghost-tier absence: regression to text-text-3 is caught here.
    expect(badgeSpan.className).not.toContain('text-text-3')
  })
  ...
  it('Live badge retains text-s-ok (Polling promotion must not affect Live state)', () => {
    ...
    expect(liveBadge.className).not.toContain('text-text-3')
    expect(liveBadge.className).not.toContain('text-text-2')
  })

  it('Polling badge does not render when connected (positive-control for Polling scope)', () => {
    // When wsConnected=true the Polling badge is absent — proves the text-text-2
